...
/**
 * @author RonanLana
 *     <p>The objective of this program is to uncover all maker data from the ItemMaker.wz.xml files
 *     and generate a SQL file with every data info for the Maker DB tables.
 */
    ...
    private static void translateToken(String token) {
        ...
        if (token.contains("/imgdir")) {
            ...
            if (status == 2) { // close item maker data
                generateUpdatedItemFee(); // for equipments, this will try to update reqMeso to be
                // conformant with the client.
                makerList.add(
                        new MapleMakerItemEntry(
                                id,
                                itemid,
                                reqLevel,
                                reqMakerLevel,
