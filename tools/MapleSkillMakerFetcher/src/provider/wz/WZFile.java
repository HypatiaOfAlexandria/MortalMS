...
public class WZFile implements MapleDataProvider {
                ...
                case 0x02:
                    name =
                            WZTool.readDecodedStringAtOffsetAndReset(
                                    slea, lea.readInt() + this.headerSize + 1);
    ...
    public WZIMGFile getImgFile(String path) throws IOException {
        ...
        String fullPath =
                wzfile.getName().substring(0, wzfile.getName().length() - 3).toLowerCase()
                        + '/'
                        + path;
        return new WZIMGFile(
                this.wzfile, entry, provideImages, ListWZFile.isModernImgFile(fullPath));
    }
