...
/**
 * @author RonanLana
 *     <p>This application finds itemids with inexistent name and description from within the
 *     server-side XMLs, then identify them on a report file along with a XML excerpt to be appended
 *     on the String.wz xml nodes. This program assumes all equipids are depicted using 8 digits and
 *     item using 7 digits.
 *     <p>Estimated parse time: 2 minutes
 */
public class MapleNoItemNameFetcher {
        ...
        for (MapleData md : subdirData.getChildren()) {
            try {
                ...
                if (nameData != null && descData != null) {
                    ...
                    System.out.println(
                            "Found itemid on String.wz with no full property: "
                                    + subdirPath
                                    + subdirData.getName()
                                    + '/'
                                    + md.getName());
    ...
    private static void readStringSubdirectoryData(
            MapleData subdirData, int depth, String subdirPath) {
        if (depth > 0) {
    ...
    private static void readEquipNodeData(
            MapleDataProvider data,
            MapleDataDirectoryEntry mDir,
            String wzFileName,
            String dirName) {
    ...
    private static void writeEquipSubdirectoryHeader(EquipType eType) {
        printWriter.println(
                "    <imgdir name=\"" + getStringDirectoryNameFromEquipType(eType) + "\">");
    }
    ...
    private static void writeMissingStringWZNode(
            String nodePath, List<Integer> missingNames, boolean isEquip) {
        if (!missingNames.isEmpty()) {
            if (!isEquip) {
                Collections.sort(missingNames);
                ...
                List<List<Integer>> equips = new ArrayList<>(arraySize);
                for (int i = 0; i < arraySize; ++i) {
                    equips.set(i, new ArrayList<>(42));
                }

                for (int itemid : missingNames) {
                    equips.get(equipTypes.get(itemid).ordinal()).add(itemid);
                }
                ...
                for (EquipType eType : EquipType.values()) {
                    writeMissingEquipWZNode(eType, equips.get(eType.ordinal()));
                }
    ...
    private static void writeMissingStringWZNames(Map<String, List<Integer>> missingNames)
            throws Exception {
        System.out.println("Writing remaining 'String.wz' names...");

        printWriter = new PrintWriter(xmlFile, StandardCharsets.UTF_8);
        printOutputFileHeader();

        String[] nodePaths = {
            "Cash.img", "Consume.img", "Eqp.img", "Etc.img", "Ins.img", "Pet.img"
        };
    ...
    public static void main(String[] args) {
        try {
            ...
            readItemWZData();
            readStringWZData(); // calculates the diff and effectively holds all items with no name
            // property on the WZ
