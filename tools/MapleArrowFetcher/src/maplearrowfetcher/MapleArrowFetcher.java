...
/**
 * @author RonanLana
 *     <p>This application traces arrow drop data on the underlying DB (that must be defined on the
 *     DatabaseConnection file of this project) and generates a SQL file that proposes updated arrow
 *     quantitty on drop entries for the drop_data table.
 *     <p>The arrow quantity range is calculated accordingly with the target mob stats, such as
 *     level and if it's a boss or not.
 */
public class MapleArrowFetcher {

    private static PrintWriter printWriter;
    private static final String newFile = "lib/arrow_drop_data.sql";

    private static final int minArrowId = 2060000;
    private static final int maxArrowId = 2061004;

    private static final float correctionFactor = 2.2f;
    ...
    private static List<int[]> getArrowEntryValues(Map<Integer, List<Integer>> existingEntries) {
        ...
        List<Entry<Integer, List<Integer>>> listEntries =
                new ArrayList<>(existingEntries.entrySet());
        ...
        for (Entry<Integer, List<Integer>> ee : listEntries) {
            ...
            for (int itemid : ee.getValue()) {
                ...
                values[2] = (mr.getLeft() + itemWeight - 1) / itemWeight; // weighted min quantity
                values[3] = (mr.getRight() + itemWeight - 1) / itemWeight; // weighted max quantity
        ...
        try {
            ...
            PreparedStatement ps =
                    con.prepareStatement(
                            "SELECT dropperid, itemid FROM drop_data WHERE itemid >= "
                                    + minArrowId
                                    + " AND itemid <= "
                                    + maxArrowId
                                    + " ORDER BY itemid;");
            ...
            if (rs.isBeforeFirst()) {
                while (rs.next()) {
                    ...
                    if (mobRange.containsKey(mobid)) {
                        List<Integer> em =
                                existingEntries.computeIfAbsent(mobid, k -> new ArrayList<>(2));
