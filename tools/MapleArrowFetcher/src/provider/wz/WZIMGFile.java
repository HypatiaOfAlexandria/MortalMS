...
public class WZIMGFile {
            ...
            case 1:
                {
                    String name =
                            WZTool.readDecodedStringAtOffsetAndReset(
                                    slea, file.getOffset() + slea.readInt());
                    entry.setName(name);
                    break;
                }
            default:
        ...
        switch (marker) {
            ...
            case 8:
                ...
                if (iMarker == 0) {
                    ...
                    entry.setData(
                            WZTool.readDecodedStringAtOffsetAndReset(
                                    slea, slea.readInt() + file.getOffset()));
                } else {
    ...
    private void parseExtended(
            WZIMGEntry entry, SeekableLittleEndianAccessor slea, long endOfExtendedBlock) {
        byte marker = slea.readByte();
        String type;
        switch (marker) {
            case 0x73:
        ...
        switch (type) {
            ...
            case "Canvas":
                {
                    ...
                    if (provideImages) {
                        ...
                        entry.setData(
                                new PNGMapleCanvas(
                                        width, height, dataLength, format + format2, pngdata));
                    } else {
                        entry.setData(
                                new PNGMapleCanvas(
                                        width, height, dataLength, format + format2, null));
            ...
            case "UOL":
                ...
                switch (uolmarker) {
                    ...
                    default:
                        System.out.println(
                                "Unknown UOL marker: " + uolmarker + ' ' + entry.getName());
                }
                break;
            default:
