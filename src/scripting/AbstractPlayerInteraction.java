...
public class AbstractPlayerInteraction {
    ...
    private static int getMarketPortalId(MapleMap map) {
        return (map.findMarketPortal() != null)
                ? map.findMarketPortal().getId()
    ...
    public EventManager getEventManager(String event) {
        return c.getEventManager(event);
    }
    ...
    private boolean canHoldAll(List<Integer> itemids, List<Integer> quantity, boolean isInteger) {
        ...
        List<Pair<Item, MapleInventoryType>> addedItems = new ArrayList<>();
    ...
    public boolean canHold(int itemid, int quantity, int removeItemid, int removeQuantity) {
        return canHoldAllAfterRemoving(
                Collections.singletonList(itemid),
                Collections.singletonList(quantity),
                Collections.singletonList(removeItemid),
                Collections.singletonList(removeQuantity));
    }

    private static List<Pair<Item, MapleInventoryType>> prepareProofInventoryItems(
    ...
    private static List<List<Pair<Integer, Integer>>> prepareInventoryItemList(
            List<Integer> itemids, List<Integer> quantity) {
        int size = Math.min(itemids.size(), quantity.size());

        List<List<Pair<Integer, Integer>>> invList = new ArrayList<>(6);
        for (int i = MapleInventoryType.UNDEFINED.getType();
                i < MapleInventoryType.CASH.getType();
                i++) {
            invList.add(new ArrayList<>());
        }

        for (int i = 0; i < size; i++) {
            int itemid = itemids.get(i);
            invList.get(ItemConstants.getInventoryType(itemid).getType())
                    .add(new Pair<>(itemid, quantity.get(i)));
        }
    ...
    public boolean canHoldAllAfterRemoving(
            List<Integer> toAddItemids,
            List<Integer> toAddQuantity,
            List<Integer> toRemoveItemids,
            List<Integer> toRemoveQuantity) {
        List<List<Pair<Integer, Integer>>> toAddItemList =
        ...
        MapleInventoryProof prfInv =
                (MapleInventoryProof) this.getInventory(MapleInventoryType.CANHOLD);
        prfInv.lockInventory();
        try {
            for (int i = MapleInventoryType.EQUIP.getType();
                    i < MapleInventoryType.CASH.getType();
                    i++) {
                ...
                if (!toAdd.isEmpty()) {
                    ...
                    List<Pair<Item, MapleInventoryType>> addItems =
                            prepareProofInventoryItems(toAdd);
    ...
    public void resetAllQuestProgress(int qid) {
        ...
        c.announce(
                MaplePacketCreator.updateQuest(
                        getPlayer().getQuest(MapleQuest.getInstance(qid)), false));
    }
    ...
    public Item evolvePet(byte slot, int afterId) {
        ...
        */
        ...
        MapleInventoryManipulator.removeFromSlot(
                c, MapleInventoryType.CASH, target.getPosition(), (short) 1, false);
    ...
    public void gainItem(
            int id, short quantity, boolean show) { // this will fk randomStats equip :P
    ...
    public Item gainItem(
            int id, short quantity, boolean randomStats, boolean showMessage, long expires) {
    ...
    public Item gainItem(
        ...
        if (quantity >= 0) {
            if (ItemConstants.isPet(id)) {
                ...
                // MapleInventoryManipulator.addById(c, id, (short) 1, null, petId, expires == -1 ?
                    ...
                    if (ServerConstants.USE_ENHANCED_CRAFTING && c.getPlayer().getCS()) {
                        ...
                        item =
                                MapleItemInformationProvider.getInstance()
                                        .scrollEquipWithId(
                                                item, 2049100, true, 2049100, c.getPlayer().isGM());
                    }
                }
            } else {
            ...
            if (expires >= 0) item.setExpiration(System.currentTimeMillis() + expires);
                ...
                c.getPlayer()
                        .dropMessage(
                                ...
                                "Your inventory is full. Please remove an item from your "
                                        + ItemConstants.getInventoryType(id).name()
                                        + " inventory.");
                return null;
            }
            if (ItemConstants.getInventoryType(id) == MapleInventoryType.EQUIP) {
                if (randomStats) {
                    MapleInventoryManipulator.addFromDrop(
                            c,
                            MapleItemInformationProvider.randomizeStats((Equip) item),
                            false,
                            petId);
                } else {
            ...
            MapleInventoryManipulator.removeById(
                    c, ItemConstants.getInventoryType(id), id, -quantity, true, false);
        }
        if (showMessage) {
    ...
    public MapleGuild getGuild() {
        try {
            return Server.getInstance()
                    .getGuild(getPlayer().getGuildId(), getPlayer().getWorld(), null);
        } catch (Exception e) {
            e.printStackTrace();
        }
        return null;
    ...
    public boolean isPartyLeader() {
        return getParty() != null && getParty().getLeaderId() == getPlayer().getId();
    }
    ...
    public static void givePartyItems(int id, short quantity, List<MapleCharacter> party) {
        for (MapleCharacter chr : party) {
            ...
            if (quantity >= 0) {
                ...
                MapleInventoryManipulator.removeById(
                        cl, ItemConstants.getInventoryType(id), id, -quantity, true, false);
            }
    ...
    public void givePartyExp(String PQ, boolean instance) {
        ...
        for (MaplePartyCharacter member : party.getMembers()) {
            ...
            if (ServerConstants.PQ_BONUS_EXP_RATE > 0
                    && System.currentTimeMillis() <= ServerConstants.EVENT_END_TIMESTAMP) {
    ...
    public void removeFromParty(int id, List<MapleCharacter> party) {
        for (MapleCharacter chr : party) {
            ...
            if (possesed > 0) {
                MapleInventoryManipulator.removeById(
                        c, ItemConstants.getInventoryType(id), id, possesed, true, false);
    ...
    public static void removeAll(int id, MapleClient cl) {
        ...
        if (invType == MapleInventoryType.EQUIP) {
            if (cl.getPlayer().getInventory(MapleInventoryType.EQUIPPED).countById(id) > 0) {
                MapleInventoryManipulator.removeById(
                        cl, MapleInventoryType.EQUIPPED, id, 1, true, false);
    ...
    public void teachSkill(int skillid, byte level, byte masterLevel, long expiration) {
        getPlayer()
                .changeSkillLevel(SkillFactory.getSkill(skillid), level, masterLevel, expiration);
    }
    ...
    public void removeEquipFromSlot(short slot) {
        ...
        MapleInventoryManipulator.removeFromSlot(
                c, MapleInventoryType.EQUIPPED, slot, tempItem.getQuantity(), false, false);
    }
    ...
    public void gainAndEquip(int itemid, short slot) {
        ...
        if (old != null) {
            MapleInventoryManipulator.removeFromSlot(
                    c, MapleInventoryType.EQUIPPED, slot, old.getQuantity(), false, false);
        }
        ...
        c.announce(
                MaplePacketCreator.modifyInventory(
                        false, Collections.singletonList(new ModifyInventory(0, newItem))));
    }
    ...
    public List<MaplePet> getDriedPets() {
        List<MaplePet> list = new ArrayList<>();
