...
public class ReactorActionManager extends AbstractPlayerInteraction {
    ...
    public void dropItems(
        ...
        List<ReactorDropEntry> items =
                generateDropList(
                        getDropChances(), c.getPlayer().getDropRate(), meso, mesoChance, minItems);
            ...
            for (ReactorDropEntry d : items) {
                ...
                if (d.itemId == 0) {
                    ...
                    reactor.getMap()
                            .spawnMesoDrop(
                                    mesoDrop,
                                    reactor.getMap().calcDropPos(dropPos, reactor.getPosition()),
                                    reactor,
                                    client.getPlayer(),
                                    false,
                                    (byte) 2);
                    ...
                    drop =
                            ItemConstants.getInventoryType(d.itemId) != MapleInventoryType.EQUIP
                                    ? new Item(d.itemId, (short) 0, (short) 1)
                                    : MapleItemInformationProvider.randomizeStats(
                                            (Equip) ii.getEquipById(d.itemId));

                    reactor.getMap()
                            .dropFromReactor(
                                    getPlayer(), reactor, drop, dropPos, (short) d.questid);
                }
            }
        } else {
                            ...
                            .register(
                                    () -> {
                                        ...
                                        if (d.itemId == 0) {
                                            ...
                                            if (ItemConstants.getInventoryType(d.itemId)
    ...
    public void hitMonsterWithReactor(
            int id, int hitsToKill) { // until someone comes with a better solution, why not?
        ...
        if (mm != null) {
            int damage = (mm.getMaxHp() + hitsToKill - 1) / hitsToKill;
    ...
    public void spawnFakeMonster(int id) {
        reactor.getMap()
                .spawnFakeMonsterOnGroundBelow(MapleLifeFactory.getMonster(id), getPosition());
    }
