...
public class EventInstanceManager {
    ...
    private final ReentrantReadWriteLock lock =
            new MonitoredReentrantReadWriteLock(MonitoredLockType.EIM, true);
    private final ReadLock rL = lock.readLock();
    private final WriteLock wL = lock.writeLock();

    private MonitoredReentrantLock pL =
            MonitoredReentrantLockFactory.createLock(MonitoredLockType.EIM_PARTY, true);
    private MonitoredReentrantLock sL =
            MonitoredReentrantLockFactory.createLock(MonitoredLockType.EIM_SCRIPT, true);
    ...
    private final Map<Integer, List<Integer>> collectionSet =
            new HashMap<>(ServerConstants.MAX_EVENT_LEVELS);
    private final Map<Integer, List<Integer>> collectionQty =
            new HashMap<>(ServerConstants.MAX_EVENT_LEVELS);
    private final Map<Integer, Integer> collectionExp =
            new HashMap<>(ServerConstants.MAX_EVENT_LEVELS);
    ...
    // registers all opened gates on the event. Will help late characters to encounter next stages
    // gates already opened
    private final Map<Integer, Pair<String, Integer>> openedGates = new HashMap<>();

    // forces deletion of items not supposed to be held outside of the event, dealt on a player's
    // leaving moment.
                                    ...
                                    try {
                                        ...
                                        Logger.getLogger(EventManager.class.getName())
                                                .log(
                                                        ...
                                                        "Event '"
                                                                + em.getName()
                                                                + "' does not implement scheduledTimeout function.",
                                                        ex);
                                    }
                                },
                                time);
    ...
    private void registerExpeditionTeam(MapleExpedition exped, int recruitMap) {
        ...
        for (MapleCharacter chr : exped.getMembers()) {
            if (chr.getMapId() == recruitMap) registerPlayer(chr);
        }
    }
        ...
        try {
            ...
            Logger.getLogger(EventManager.class.getName())
                    .log(
                            ...
                            "Event '"
                                    + em.getName()
                                    + "' does not implement playerUnregistered function.",
                            ex);
        }
    ...
    public void dispose() {
        ...
        Thread t = new Thread(() -> dispose(false));

        t.start();
    }

    public synchronized void dispose(
            boolean shutdown) { // should not trigger any event script method after disposed
        if (disposed) return;
    ...
    public static boolean isLeader(MapleCharacter chr) {
    ...
    public final void setEventRewards(
            int eventLevel, List<Double> rwds, List<Double> qtys, int expGiven) {
    ...
    private boolean hasRewardSlot(MapleCharacter player, int eventLevel) {
        byte listReq =
                getRewardListRequirements(
                        eventLevel); // gets all types of items present in the event reward list

        // iterating over all valid inventory types
        for (byte type = 1; type <= 5; type++) {
            if ((listReq >> type) % 2 == 1 && !player.hasEmptySlot(type)) return false;
        }
    ...
    public final boolean checkEventTeamLacking(boolean leavingEventMap, int minPlayers) {
        ...
        return !eventCleared && leavingEventMap && !isEventTeamLeaderOn()
                || getPlayerCount() < minPlayers;
    }

    public final boolean isEventTeamLackingNow(
            boolean leavingEventMap, int minPlayers, MapleCharacter quitter) {
        if (eventCleared) {
            return leavingEventMap && getPlayerCount() <= 1;
        } else {
            return leavingEventMap && getLeaderId() == quitter.getId()
                    || getPlayerCount() <= minPlayers;
        }
    }
    ...
    public final void warpEventTeam(int warpFrom, int warpTo) {
        ...
        for (MapleCharacter chr : players) {
            if (chr.getMapId() == warpFrom) chr.changeMap(warpTo);
        }
    }
    ...
    public final void warpEventTeamToMapSpawnPoint(int warpFrom, int warpTo, int toSp) {
        ...
        for (MapleCharacter chr : players) {
            if (chr.getMapId() == warpFrom) chr.changeMap(warpTo, toSp);
        }
    }
    ...
    public final void recoverOpenedGate(MapleCharacter chr, int thisMapId) {
        ...
        if (gateData != null) {
            chr.announce(
                    MaplePacketCreator.environmentChange(gateData.getLeft(), gateData.getRight()));
        }
    }
    ...
    public final void giveEventPlayersStageReward(int thisStage) {
        List<Integer> list =
                getClearStageBonus(
                        thisStage); // will give bonus exp & mesos to everyone in the event
    ...
    public final void linkPortalToScript(
            int thisStage, String portalName, String scriptName, int thisMapId) {
        giveEventPlayersStageReward(thisStage);
        thisStage--; // stages counts from ONE, scripts from ZERO
    ...
    public static boolean activatedAllReactorsOnMap(
            MapleMap map, int minReactorId, int maxReactorId) {
