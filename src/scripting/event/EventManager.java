...
public class EventManager {
    ...
    private final Queue<Integer> queuedGuilds = new ArrayDeque<>();
    private final Map<Integer, Integer> queuedGuildLeaders = new HashMap<>();
    private final List<Boolean> openedLobbys;
    private final List<EventInstanceManager> readyInstances = new ArrayList<>();
    private Integer readyId = 0, onLoadInstances = 0;
    private final Properties props = new Properties();
    private final String name;
    private MonitoredReentrantLock lobbyLock =
            MonitoredReentrantLockFactory.createLock(MonitoredLockType.EM_LOBBY);
    private MonitoredReentrantLock queueLock =
            MonitoredReentrantLockFactory.createLock(MonitoredLockType.EM_QUEUE);
    private MonitoredReentrantLock startLock =
            MonitoredReentrantLockFactory.createLock(MonitoredLockType.EM_START);

    private final Set<Integer> playerPermit = new HashSet<>();
    private final Semaphore startSemaphore = new Semaphore(7);

    private static final int maxLobbys =
            8; // an event manager holds up to this amount of concurrent lobbys
    ...
    public void cancel() { // make sure to only call this when there are NO PLAYERS ONLINE to mess
        // around with the event manager!
        ess.dispose();
    ...
    private List<Integer> getLobbyRange() {
        try {
            return convertToIntegerArray(
                    (List<Double>) iv.invokeFunction("setLobbyRange", (Object) null));
        } catch (ScriptException | NoSuchMethodException ex) { // they didn't define a lobby range
            List<Integer> defaultRange = new ArrayList<>(2);
            defaultRange.add(0);
    ...
    public EventScheduledFuture schedule(
            final String methodName, final EventInstanceManager eim, long delay) {
        Runnable r =
                () -> {
                    try {
        ...
        // hate to do that, but those schedules can still be cancelled, so well... Let GC do it's
        // job
    ...
    public boolean startInstance(int lobbyId, MapleExpedition exped, MapleCharacter leader) {
        ...
        try {
            if (!playerPermit.contains(leader.getId())
                    && startSemaphore.tryAcquire(7777, TimeUnit.MILLISECONDS)) {
                    ...
                    try {
                        ...
                        EventInstanceManager eim =
                                (EventInstanceManager)
                                        (iv.invokeFunction(
                                                "setup", leader.getClient().getChannel()));
                        if (eim == null) {
                            if (lobbyId > -1) {
    ...
    public boolean startInstance(
            int lobbyId, MapleCharacter chr, MapleCharacter leader, int difficulty) {
        if (this.isDisposed()) return false;
    ...
    public boolean startInstance(
            int lobbyId, MapleParty party, MapleMap map, MapleCharacter leader) {
        if (this.isDisposed()) return false;
    ...
    // PQ method: starts a PQ with a difficulty level, requires function setup(difficulty, leaderid)
    // instead of setup()
    ...
    public boolean startInstance(
            int lobbyId, MapleParty party, MapleMap map, int difficulty, MapleCharacter leader) {
        if (this.isDisposed()) return false;
    ...
    public boolean startInstance(int lobbyId, EventInstanceManager eim, String ldr) {
        return startInstance(
                ...
                eim.getEm()
                        .cserv
                        .getPlayerStorage()
                        .getCharacterByName(ldr)); // things they make me do...
    }

    public boolean startInstance(
            int lobbyId, EventInstanceManager eim, String ldr, MapleCharacter leader) {
        if (this.isDisposed()) return false;
        ...
        try {
            ...
            if (p != null) {
                ...
                List<MaplePartyCharacter> lmpc =
                        new ArrayList<>((Collection<? extends MaplePartyCharacter>) p);
    ...
    public static void startQuest(MapleCharacter chr, int id, int npcid) {
        try {
    ...
    public static void completeQuest(MapleCharacter chr, int id, int npcid) {
        try {
    ...
    public int getTransportationTime(int travelTime) {
        return wserv.getTransportationTime(travelTime);
    }

    private void fillEimQueue() {
        Thread t =
                new Thread(
                        new EventManagerWorker()); // call new thread to fill up readied instances
        // queue
        t.start();
    }
    ...
    private void instantiateQueuedInstance() {
        ...
        try {
            if (this.isDisposed()
                    || readyInstances.size() + onLoadInstances
                            >= Math.ceil((double) maxLobbys / 3.0d)) {
                return;
            }
