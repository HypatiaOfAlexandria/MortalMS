...
public class MaplePacketCreator {
    ...
    private static final long DEFAULT_TIME = 150842304000000000L; // 00 80 05 BB 46 E6 17 02
    public static final long ZERO_TIME = 94354848000000000L; // 00 40 E0 FD 3B 37 4F 01
    private static final long PERMANENT = 150841440000000000L; // 00 C0 9B 90 7D E5 17 02
    ...
    private static void addRemainingSkillInfo(
    ...
    protected static void addCharLook(
            final MaplePacketLittleEndianWriter mplew, MapleCharacter chr, boolean mega) {
    ...
    private static void addAreaInfo(final MaplePacketLittleEndianWriter mplew, MapleCharacter chr) {
        ...
        for (Entry<Short, String> shortStringEntry : areaInfos.entrySet()) {
            mplew.writeShort(shortStringEntry.getKey());
            mplew.writeMapleAsciiString(shortStringEntry.getValue());
        }
    }
    ...
    private static void addCharEquips(
        ...
        Collection<Item> ii =
                MapleItemInformationProvider.getInstance().canWearEquipment(chr, equip.list());
    ...
    private static void addCharEntry(
            final MaplePacketLittleEndianWriter mplew, MapleCharacter chr, boolean viewall) {
        addCharStats(mplew, chr);
    ...
    protected static void addItemInfo(
            final MaplePacketLittleEndianWriter mplew, Item item, boolean zeroPosition) {
        ...
        if (isCash) {
            mplew.writeLong(
                    isPet ? item.getPetId() : isRing ? equip.getRingId() : item.getCashId());
        }
        ...
        if (isCash) {
            ...
            long expNibble =
                    (ExpTable.getExpNeededForLevel(ii.getEquipLevelReq(item.getItemId()))
                            * equip.getItemExp());
    ...
    private static void addSkillInfo(
            final MaplePacketLittleEndianWriter mplew, MapleCharacter chr) {
        mplew.write(0); // start of skills
        Map<Skill, MapleCharacter.SkillEntry> skills = chr.getSkills();
        int skillsSize = skills.size();
        // We don't want to include any hidden skill in this, so subtract them from the size list
        // and ignore them.
     ...
     *
     * <p>Possible values for {@code reason}:<br>
    ...
    public static byte[] getAfterLoginError(int reason) { // same as above o.o
        ...
        System.out.println("getAfterLoginError: reason=" + reason);
        assert reason < 0;
        return mplew.getPacket();
    }
    ...
    public static byte[] getTempBan(long timestampTill, byte reason) {
        ...
        mplew.write(reason);
        mplew.writeLong(
                getTime(timestampTill)); // Tempban date is handled as a 64-bit long, number of
        // 100NS intervals since 1/1/1601. Lulz.
    ...
    public static byte[] getAuthSuccess(MapleClient c) {
        Server.getInstance()
                .loadAccountCharacters(
                        c); // locks the login session until data is recovered from the cache or the
        // DB.
        ...
        boolean canFly = Server.canFly(c.getAccID());
        mplew.writeBool(
                (ServerConstants.USE_ENFORCE_ADMIN_ACCOUNT || canFly)
                        && c.getGMLevel()
                                > 1); // thanks Steve(kaito1410) for pointing the GM account boolean
        // here
        mplew.write(
                ((ServerConstants.USE_ENFORCE_ADMIN_ACCOUNT || canFly) && c.getGMLevel() > 1)
                        ? 0x80
     ...
     *
     * <p>Possible values for {@code mode}:<br>
     ...
     */
    public static byte[] getServerList(
            int serverId, String serverName, int flag, String eventmsg, List<Channel> channelLoad) {
        final MaplePacketLittleEndianWriter mplew = new MaplePacketLittleEndianWriter();
     ...
     *
     * <p>Possible values for {@code status}:<br>
     * 0 - Normal<br>
    ...
    public static byte[] getCharList(MapleClient c, int serverId) {
        ...
        mplew.writeInt(
                ServerConstants.COLLECTIVE_CHARSLOT
     ...
     */
    public static byte[] sendTV(
            MapleCharacter chr, List<String> messages, int type, MapleCharacter partner) {
        final MaplePacketLittleEndianWriter mplew = new MaplePacketLittleEndianWriter();
    ...
    public static byte[] getCharInfo(MapleCharacter chr) {
        ...
        for (int i = 0; i < 3; i++) {
            mplew.writeInt(Randomizer.nextInt()); // 14
        }
    ...
    public static byte[] updatePlayerStats(
            List<Pair<MapleStat, Integer>> stats, boolean enableActions, MapleCharacter chr) {
        final MaplePacketLittleEndianWriter mplew = new MaplePacketLittleEndianWriter();
     ...
     *
     * @param to The {@code MapleMap} to warp to.
    ...
    public static byte[] getWarpToMap(
            MapleMap to, int spawnPoint, Point spawnPosition, MapleCharacter chr) {
        final MaplePacketLittleEndianWriter mplew = new MaplePacketLittleEndianWriter();
        mplew.writeShort(SendOpcode.SET_FIELD.getValue());
        mplew.writeInt(chr.getClient().getChannel() - 1);
        mplew.writeInt(0); // updated
    ...
    public static byte[] spawnSummon(MapleSummon summon, boolean animated) {
        ...
        mplew.skip(3);
        mplew.write(
                summon.getMovementType()
                        .getValue()); // 0 = don't move, 1 = follow (4th mage summons?), 2/4 = only
        // tele follow, 3 = bird follow
        mplew.write(
                summon.isPuppet()
                        ? 0
                        : 1); // 0 and the summon can't attack - but puppets don't attack with 1
        // either ^.-
    ...
    public static byte[] spawnKite(
            int oid, int itemid, String name, String msg, Point pos, int ft) {
        MaplePacketLittleEndianWriter mplew = new MaplePacketLittleEndianWriter();
     ...
     *
     * <p>Possible values for {@code type}:<br>
     * 0: [Notice]<br>
     * 1: Popup<br>
     * 2: Megaphone<br>
     ...
     * @return The server notice packet.
     */
    private static byte[] serverMessage(
            int type,
            int channel,
            String message,
    ...
    private static void encodeParentlessMobSpawnEffect(
            MaplePacketLittleEndianWriter mplew, boolean newSpawn, int effect) {
        if (effect > 0) {
            mplew.write(effect);
            mplew.write(0);
            mplew.writeShort(0);
     ...
     */
    private static byte[] spawnMonsterInternal(
            MapleMonster life,
        ...
        /**
         * -4: Fake -3: Appear after linked mob is dead -2: Fade in 1: Smoke 3: King Slime spawn 4:
         * Summoning rock thing, used for 3rd job? 6: Magical shit 7: Smoke shit 8: 'The Boss' 9/10:
         * Grim phantom shit? 11/12: Nothing? 13: Frankenstein 14: Angry ^ 15: Orb animation thing,
    ...
    public static byte[] moveMonsterResponse(
            int objectid, short moveid, int currentMp, boolean useSkills) {
     ...
     */
    public static byte[] getShowExpGain(
            int gain, int equip, int party, boolean inChat, boolean white) {
        final MaplePacketLittleEndianWriter mplew = new MaplePacketLittleEndianWriter();
        ...
        int mod =
                ServerConstants.PARTY_EXPERIENCE_MOD != 1
                        ? ServerConstants.PARTY_EXPERIENCE_MOD * 100
                        : 0;
    ...
    public static byte[] dropItemFromMapObject(
            boolean recvrInParty, MapleMapItem drop, Point dropfrom, Point dropto, byte mod) {
        final MaplePacketLittleEndianWriter mplew = new MaplePacketLittleEndianWriter();
        mplew.writeShort(SendOpcode.DROP_ITEM_FROM_MAPOBJECT.getValue());
        mplew.write(mod);
        mplew.writeInt(drop.getObjectId());
        ...
        mplew.writeInt(
                !drop.isFFADrop()
                        ? (recvrInParty ? drop.getPartyOwnerId() : drop.getOwnerId())
                        : 0); // owner charid/partyid :)
        mplew.write(drop.getDropType()); // 0 = timeout for non-owner, 1 = timeout for non-owner's
        // party, 2 = FFA, 3 = explosive/FFA
    ...
    private static byte[] spawnPlayerMapObject(MapleCharacter chr, boolean enteringField) {
        ...
        if (bv != null) {
            if (bv.equals(Corsair.BATTLE_SHIP)) {
                ...
                final Item mount =
                        chr.getInventory(MapleInventoryType.EQUIPPED).getItem((short) -18);
                if (mount != null) {
        ...
        mplew.writeInt(
                ItemConstants.getInventoryType(chr.getChair()) == MapleInventoryType.SETUP
                        ? chr.getChair()
                        : 0);
    ...
    private static void encodeNewYearCardInfo(
            MaplePacketLittleEndianWriter mplew, MapleCharacter chr) {
    ...
    public static byte[] onNewYearCardRes(
            MapleCharacter user, NewYearCardRecord newyear, int mode, int msg) {
        MaplePacketLittleEndianWriter mplew = new MaplePacketLittleEndianWriter();
        mplew.writeShort(SendOpcode.NEW_YEAR_CARD_RES.getValue());
        mplew.write(mode);
        switch (mode) {
            case 4: // Successfully sent a New Year Card" + System.lineSeparator() + " to %s.
            ...
            case 9:
            case 0xB:
                // 0x10: You have no free slot to store card." + System.lineSeparator() + "try later
                // on please.
    ...
    private static void encodeNewYearCard(
            NewYearCardRecord newyear, MaplePacketLittleEndianWriter mplew) {
        mplew.writeInt(newyear.getId());
        mplew.writeInt(newyear.getSenderId());
        mplew.writeMapleAsciiString(newyear.getSenderName());
    ...
    private static void addRingLook(
            final MaplePacketLittleEndianWriter mplew, MapleCharacter chr, boolean crush) {
        List<MapleRing> rings;
        rings = crush ? chr.getCrushRings() : chr.getFriendshipRings();
        boolean yes = false;
    ...
    private static void addAnnounceBox(
            final MaplePacketLittleEndianWriter mplew, MaplePlayerShop shop, int availability) {
        mplew.write(4);
    ...
    private static void spawnAnnounceBox(
            final MaplePacketLittleEndianWriter mplew,
            MapleMiniGame game,
            int type,
            int ammount,
    ...
    private static void updateHiredMerchantBoxInfo(
            MaplePacketLittleEndianWriter mplew, MapleHiredMerchant hm) {
    ...
    private static void updatePlayerShopBoxInfo(
    ...
    private static void serializeMovementList(
            LittleEndianWriter lew, List<LifeMovementFragment> moves) {
    ...
    public static byte[] moveSummon(
            int cid, int oid, Point startPos, List<LifeMovementFragment> moves) {
        final MaplePacketLittleEndianWriter mplew = new MaplePacketLittleEndianWriter();
    ...
    */
    ...
    public static byte[] summonAttack(
            int cid, int summonOid, byte direction, List<SummonAttackEntry> allDamage) {
        final MaplePacketLittleEndianWriter mplew = new MaplePacketLittleEndianWriter();
    ...
    */
    ...
    private static void addAttackBody(
        ...
        for (Entry<Integer, List<Integer>> integerListEntry : damage.entrySet()) {
            List<Integer> onedList = integerListEntry.getValue();
            if (onedList != null) {
                lew.writeInt((integerListEntry.getKey()).intValue());
                lew.write(0x0);
                if (skill == 4211006) {
    ...
    // someone thought it was a good idea to handle floating point representation through packets
    // ROFL
    ...
    public static byte[] getScrollEffect(
            int chr, ScrollResult scrollSuccess, boolean legendarySpirit) {
        final MaplePacketLittleEndianWriter mplew = new MaplePacketLittleEndianWriter();
    ...
    public static byte[] selectWorld(int world) {
        ...
        mplew.writeInt(world); // According to GMS, it should be the world that contains the most
        // characters (most active)
        return mplew.getPacket();
    }
    ...
    public static byte[] charInfo(MapleCharacter chr) {
        // 3D 00 0A 43 01 00 02 00 00 00 00 00 00 00 00 00 00 00 00 00 01 00 00 00 00 00 00 00 00 00
        ...
        Item mount; // mounts can potentially crash the client if the player's level is not
        // properly checked
        if (chr.getMount() != null
                && (mount = chr.getInventory(MapleInventoryType.EQUIPPED).getItem((short) -18))
                        != null
                && MapleItemInformationProvider.getInstance().getEquipLevelReq(mount.getItemId())
                        <= chr.getLevel()) {
        ...
        mplew.writeInt(
                chr.getMonsterBookCover() > 0
                        ? MapleItemInformationProvider.getInstance()
                                .getCardMobId(chr.getMonsterBookCover())
                        : 0);
    ...
    /**
     * It is important that statups is in the correct order (see declaration order in MapleBuffStat)
     * since this method doesn't do automagical reordering.
     *
     * @param buffid
     * @param bufflength
     * @param statups
     * @return
     */
    // 1F 00 00 00 00 00 03 00 00 40 00 00 00 E0 00 00 00 00 00 00 00 00 E0 01 8E AA 4F 00 00 C2 EB
    // 0B E0 01 8E AA 4F 00 00 C2 EB 0B 0C 00 8E AA 4F 00 00 C2 EB 0B 44 02 8E AA 4F 00 00 C2 EB 0B
    // 44 02 8E AA 4F 00 00 C2 EB 0B 00 00 E0 7A 1D 00 8E AA 4F 00 00 00 00 00 00 00 00 03
    public static byte[] giveBuff(
            int buffid, int bufflength, List<Pair<MapleBuffStat, Integer>> statups) {
        final MaplePacketLittleEndianWriter mplew = new MaplePacketLittleEndianWriter();
    ...
    public static byte[] showMonsterRiding(
            int cid, MapleMount mount) { // Gtfo with this, this is just giveForeignBuff
        final MaplePacketLittleEndianWriter mplew = new MaplePacketLittleEndianWriter();
    ...
    public static byte[] addQuestTimeLimit(final short quest, final int time) {
        ...
        mplew.write(6);
        mplew.writeShort(
                1); // Size but meh, when will there be 2 at the same time? And it won't even
        // replace the old one :)
        mplew.writeShort(quest);
        mplew.writeInt(time);
        return mplew.getPacket();
    ...
    private static void writeLongMaskD(
            final MaplePacketLittleEndianWriter mplew, List<Pair<MapleDisease, Integer>> statups) {
        long firstmask = 0;
        long secondmask = 0;
    ...
    public static byte[] giveForeignDebuff(
            int cid, List<Pair<MapleDisease, Integer>> statups, MobSkill skill) {
        // Poison damage visibility and missing diseases status visibility, extended through map
    ...
    private static void writeLongMask(
            final MaplePacketLittleEndianWriter mplew, List<Pair<MapleBuffStat, Integer>> statups) {
        long firstmask = 0;
        long secondmask = 0;
        for (Pair<MapleBuffStat, Integer> statup : statups) {
            if (statup.getLeft().isFirst()) {
    ...
    private static void writeLongMaskFromList(
            final MaplePacketLittleEndianWriter mplew, List<MapleBuffStat> statups) {
        long firstmask = 0;
        long secondmask = 0;
    ...
    public static byte[] giveForeignSlowDebuff(
            int cid, List<Pair<MapleDisease, Integer>> statups, MobSkill skill) {
        final MaplePacketLittleEndianWriter mplew = new MaplePacketLittleEndianWriter();
        mplew.writeShort(SendOpcode.GIVE_FOREIGN_BUFF.getValue());
        mplew.writeInt(cid);
    ...
    /**
     * Possible values for {@code speaker}:<br>
    ...
    public static byte[] getNPCTalk(
            int npc, byte msgType, String talk, String endBytes, byte speaker) {
        final MaplePacketLittleEndianWriter mplew = new MaplePacketLittleEndianWriter();
    ...
    public static byte[] takeOutStorage(
            byte slots, MapleInventoryType type, Collection<Item> items) {
        final MaplePacketLittleEndianWriter mplew = new MaplePacketLittleEndianWriter();
        mplew.writeShort(SendOpcode.STORAGE.getValue());
        mplew.write(0x9);
        mplew.write(slots);
    ...
    public static byte[] showBossHP(
            int oid, int currHP, int maxHP, byte tagColor, byte tagBgColor) {
        final MaplePacketLittleEndianWriter mplew = new MaplePacketLittleEndianWriter();
    ...
    public static byte[] customShowBossHP(
            byte call, int oid, long currHP, long maxHP, byte tagColor, byte tagBgColor) {
    ...
    /**
     * 10: A beginner can't create a party. 1/11/14/19: Your request for a party didn't work due to
     * an unexpected error. 13: You have yet to join a party. 16: Already have joined a party. 17:
     * The party you're trying to join is already in full capacity. 19: Unable to find the requested
     * character in this channel.
     *
     * @param message
     * @return
     */
    ...
    private static void addPartyStatus(
            int forchannel, MapleParty party, LittleEndianWriter lew, boolean leaving) {
    ...
    public static byte[] updateParty(
            int forChannel, MapleParty party, PartyOperation op, MaplePartyCharacter target) {
        final MaplePacketLittleEndianWriter mplew = new MaplePacketLittleEndianWriter();
        mplew.writeShort(SendOpcode.PARTY_OPERATION.getValue());
        switch (op) {
            case DISBAND:
    ...
    private static void writeIntMask(
            final MaplePacketLittleEndianWriter mplew, Map<MonsterStatus, Integer> stats) {
        int firstmask = 0;
        int secondmask = 0;
    ...
    public static byte[] applyMonsterStatus(
            final int oid, final MonsterStatusEffect mse, final List<Integer> reflection) {
    ...
    public static byte[] getClock(int time) { // time in seconds
        ...
        mplew.write(
                2); // clock type. if you send 3 here you have to send another byte (which does not
        ...
        mplew.writeInt(
                mist.isMobMist()
                        ...
                        : mist.isPoisonMist()
                                ...
                                : mist.isRecoveryMist()
                                        ? 4
                                        : 2); // mob mist = 0, player poison = 1, smokescreen = 2,
        // unknown = 3, recovery = 4
    ...
    public static byte[] triggerReactor(MapleReactor reactor, int stance) {
        ...
        mplew.write(0);
        mplew.write(
                5); // frame delay, set to 5 since there doesn't appear to be a fixed formula for it
        return mplew.getPacket();
    }
    ...
    public static void addThread(final MaplePacketLittleEndianWriter mplew, ResultSet rs)
            throws SQLException {
    ...
    public static byte[] skillEffect(
            MapleCharacter from, int skillId, int level, byte flags, int speed, byte direction) {
        final MaplePacketLittleEndianWriter mplew = new MaplePacketLittleEndianWriter();
    ...
    */
    ...
    private static void addPetInfo(
    ...
    public static final byte[] loadExceptionList(
            final int cid, final int petId, final byte petIdx, final List<Integer> data) {
        final MaplePacketLittleEndianWriter mplew = new MaplePacketLittleEndianWriter();
    ...
    public static byte[] skillBookSuccess(
    ...
    public static byte[] showAllCharacter(int chars, int unk) {
        ...
        mplew.write(chars > 0 ? 1 : 5); // 2: already connected to server, 3 : unk error
        // (view-all-characters), 5 : cannot find any
        mplew.writeInt(chars);
        mplew.writeInt(unk);
        return mplew.getPacket();
    }

    public static byte[] showAllCharacterInfo(
            int worldid, List<MapleCharacter> chars, boolean usePic) {
        final MaplePacketLittleEndianWriter mplew = new MaplePacketLittleEndianWriter();
        mplew.writeShort(SendOpcode.VIEW_ALL_CHAR.getValue());
        mplew.write(0);
        mplew.write(worldid);
    ...
    public static byte[] getMiniGame(
            MapleClient c, MapleMiniGame minigame, boolean owner, int piece) {
        final MaplePacketLittleEndianWriter mplew = new MaplePacketLittleEndianWriter();
        mplew.writeShort(SendOpcode.PLAYER_INTERACTION.getValue());
        mplew.write(PlayerInteractionHandler.Action.ROOM.getCode());
        mplew.write(1);
     ...
     * 4 = Can't do while dead 5 = Can't do while middle event 6 = This character unable to do it 7,
     * 20 = Not allowed to trade anymore 9 = Can only trade on same map 10 = May not open store near
     * portal 11, 14 = Can't start game here 12 = Can't open store at this channel 13 = Can't
     * estabilish miniroom 15 = Stores only an the free market 16 = Lists the rooms at FM (?) 17 =
     * You may not enter this store 18 = Owner undergoing store maintenance 19 = Unable to enter
     * tournament room 21 = Not enough mesos to enter 22 = Incorrect password
     *
     * @param status
     * @return
     */
    ...
    private static byte[] getMiniGameResult(
            MapleMiniGame game, int win, int lose, int tie, int result, int forfeit, boolean omok) {
        final MaplePacketLittleEndianWriter mplew = new MaplePacketLittleEndianWriter();
        mplew.writeShort(SendOpcode.PLAYER_INTERACTION.getValue());
        ...
        mplew.writeInt(
                game.getOwner().getMiniGamePoints(MiniGameResult.LOSS, omok) + lose); // losses
        mplew.writeInt(2000); // points
        mplew.writeInt(1); // start of visitor; unknown
        mplew.writeInt(
                game.getVisitor().getMiniGamePoints(MiniGameResult.WIN, omok) + lose); // wins
        mplew.writeInt(game.getVisitor().getMiniGamePoints(MiniGameResult.TIE, omok) + tie); // ties
        mplew.writeInt(
                game.getVisitor().getMiniGamePoints(MiniGameResult.LOSS, omok) + win); // losses
        mplew.writeInt(2000); // points
    ...
    public static byte[] getMatchCard(
            MapleClient c, MapleMiniGame minigame, boolean owner, int piece) {
        final MaplePacketLittleEndianWriter mplew = new MaplePacketLittleEndianWriter();
        mplew.writeShort(SendOpcode.PLAYER_INTERACTION.getValue());
        mplew.write(PlayerInteractionHandler.Action.ROOM.getCode());
    ...
    public static byte[] getMatchCardSelect(
            MapleMiniGame game, int turn, int slot, int firstslot, int type) {
    ...
    public static byte[] getFredrick(MapleCharacter chr) {
        ...
        try {
            List<Pair<Item, MapleInventoryType>> items =
                    ItemFactory.MERCHANT.loadItems(chr.getId(), false);
            mplew.write(items.size());
    ...
    // 18: The owner of the store is currently undergoing store maintenance. Please try again in a
    // bit.
    ...
    public static byte[] owlOfMinerva(
            MapleClient c,
            int itemid,
    ...
    public static byte[] getHiredMerchant(
            MapleCharacter chr, MapleHiredMerchant hm, boolean firstTime) { // Thanks Dustin
        final MaplePacketLittleEndianWriter mplew = new MaplePacketLittleEndianWriter();
        mplew.writeShort(SendOpcode.PLAYER_INTERACTION.getValue());
        mplew.write(PlayerInteractionHandler.Action.ROOM.getCode());
        mplew.write(0x05);
        ...
        if (hm.isOwner(chr)) {
            ...
            for (Pair<String, Byte> aMsgList : msgList) {
                mplew.writeMapleAsciiString(aMsgList.getLeft());
                mplew.write(aMsgList.getRight());
            }
        } else {
            mplew.writeShort(0);
        }
    ...
    public static byte[] getPlayerNPC(MaplePlayerNPC npc) { // thanks to Arnah
        ...
        Map<Short, Integer> myEquip = new LinkedHashMap<>();
        Map<Short, Integer> maskedEquip = new LinkedHashMap<>();
        for (Entry<Short, Integer> shortIntegerEntry : equip.entrySet()) {
            short pos = (byte) (shortIntegerEntry.getKey() * -1);
            if (pos < 100 && myEquip.get(pos) == null) {
                myEquip.put(pos, shortIntegerEntry.getValue());
                ...
                maskedEquip.put(pos, shortIntegerEntry.getValue());
            }
        }
        for (Entry<Short, Integer> entry : myEquip.entrySet()) {
            mplew.write(entry.getKey());
    ...
    public static byte[] givePirateBuff(
        ...
        final MaplePacketLittleEndianWriter mplew = new MaplePacketLittleEndianWriter();
        boolean infusion =
                buffid == Buccaneer.SPEED_INFUSION
    ...
    public static byte[] giveForeignPirateBuff(
            int cid, int buffid, int time, List<Pair<MapleBuffStat, Integer>> statups) {
        final MaplePacketLittleEndianWriter mplew = new MaplePacketLittleEndianWriter();
        boolean infusion =
                buffid == Buccaneer.SPEED_INFUSION
                        || buffid == ThunderBreaker.SPEED_INFUSION
    ...
    public static byte[] sendMTS(List<MTSItemInfo> items, int tab, int type, int page, int pages) {
        ...
        for (MTSItemInfo item : items) {
            ...
            mplew.writeMapleAsciiString(
                    item.getSeller()); // account name (what was nexon thinking?)
    ...
    public static byte[] showNotes(ResultSet notes, int count) throws SQLException {
        ...
        for (int i = 0; i < count; i++) {
            ...
            mplew.writeMapleAsciiString(
                    notes.getString("from") + ' '); // Stupid nexon forgot space lol
    ...
    */
    ...
    public static byte[] giveFinalAttack(
            int skillid, int time) { // packets found thanks to lailainoob
        final MaplePacketLittleEndianWriter mplew = new MaplePacketLittleEndianWriter();
        mplew.writeShort(SendOpcode.GIVE_BUFF.getValue());
        mplew.writeLong(0);
        mplew.writeShort(0);
     ...
     * 69: The character you wish to add as" + System.lineSeparator() + "a Junior must be in the
     * same map.<br>
     * 70: This character is already a Junior of another character.<br>
     * 71: The Junior you wish to add" + System.lineSeparator() + "must be at a lower rank.<br>
     * 72: The gap between you and your" + System.lineSeparator() + "junior must be within 20
     * levels.<br>
     * 73: Another character has requested to add this character." + System.lineSeparator() +
     * "Please try again later.<br>
     * 74: Another character has requested a summon." + System.lineSeparator() + "Please try again
     * later.<br>
     ...
     * 77: The Junior you wish to add" + System.lineSeparator() + "must be over Level 10.<br>
     * 78: You cannot add a Junior " + System.lineSeparator() + "that has requested to change
     * worlds.<br>
     * 79: You cannot add a Junior " + System.lineSeparator() + "since you've requested to change
     * worlds.<br>
     * 80: Separation is not possible due to insufficient Mesos." + System.lineSeparator() + "You
     * will need %d Mesos to" + System.lineSeparator() + "separate with a Senior.<br>
     * 81: Separation is not possible due to insufficient Mesos." + System.lineSeparator() + "You
     * will need %d Mesos to" + System.lineSeparator() + "separate with a Junior.<br>
    ...
    /**
     * Sends a UI utility. 0x01 - Equipment Inventory. 0x02 - Stat Window. 0x03 - Skill Window. 0x05
     * - Keyboard Settings. 0x06 - Quest window. 0x09 - Monsterbook Window. 0x0A - Char Info 0x0B -
     * Guild BBS 0x12 - Monster Carnival Window 0x16 - Party Search. 0x17 - Item Creation Window.
     * 0x1A - My Ranking O.O 0x1B - Family Window 0x1C - Family Pedigree 0x1D - GM Story Board
     * /funny shet 0x1E - Envelop saying you got mail from an admin. lmfao 0x1F - Medal Window 0x20
     * - Maple Event (???) 0x21 - Invalid Pointer Crash
     *
     * @param ui
     * @return
     */
    ...
    /**
     * 0 = Levelup 6 = Exp did not drop (Safety Charms) 7 = Enter portal sound 8 = Job change 9 =
     * Quest complete 10 = Recovery 11 = Buff effect 14 = Monster book pickup 15 = Equipment levelup
     * 16 = Maker Skill Success 17 = Buff effect w/ sfx 19 = Exp card [500, 200, 50] 21 = Wheel of
     * destiny 26 = Spirit Stone
     *
     * @param effect
     * @return
     */
     ...
     * 0x13 with Mode 0: + mapid 0x13 with Mode 1: + ch (FF = Unable to find merchant)
     *
     * @param type The type
     * @param mode The mode
    ...
    public static byte[] removeGuildFromAlliance(
            MapleAlliance alliance, int expelledGuild, int worldId) {
        final MaplePacketLittleEndianWriter mplew = new MaplePacketLittleEndianWriter();
        mplew.writeShort(SendOpcode.ALLIANCE_OPERATION.getValue());
        mplew.write(0x10);
        mplew.writeInt(alliance.getId());
    ...
    public static byte[] sendMesoLimit() {
        final MaplePacketLittleEndianWriter mplew = new MaplePacketLittleEndianWriter();
        mplew.writeShort(
                SendOpcode.TRADE_MONEY_LIMIT
                        .getValue()); // Players under level 15 can only trade 1m per day
        return mplew.getPacket();
    }
    ...
    public static byte[] updateDojoStats(MapleCharacter chr, int belt) {
        ...
        mplew.writeMapleAsciiString(
                "pt="
                        ...
                        + belt
                        + ";tuto="
                        + (chr.getFinishedDojoTutorial() ? "1" : "0"));
        return mplew.getPacket();
    }
     ...
     * - The Reps you have received from ? will be reduced in half. 1: <Family> ? has reached Lv. ?.
     * <br>
    ...
    private static void addRingInfo(final MaplePacketLittleEndianWriter mplew, MapleCharacter chr) {
        ...
        if (chr.getPartnerId() > 0) {
            ...
            if (marriageRing != null) {
                ...
                mplew.writeInt(
                        1112803); // Engagement Ring's Outcome (doesn't matter for engagement)
                mplew.writeInt(
                        1112803); // Engagement Ring's Outcome (doesn't matter for engagement)
            }
            mplew.writeAsciiString(
                    StringUtil.getRightPaddedStr(
                            chr.getGender() == 0
                                    ? chr.getName()
                                    : MapleCharacter.getNameById(chr.getPartnerId()),
                            '\0',
                            13));
            mplew.writeAsciiString(
                    StringUtil.getRightPaddedStr(
    ...
    public static byte[] rollSnowBall(
            boolean entermap, int state, MapleSnowball ball0, MapleSnowball ball1) {
        final MaplePacketLittleEndianWriter mplew = new MaplePacketLittleEndianWriter();
     ...
     * Possible values for {@code message}:<br>
    ...
    public static byte[] customPacket(byte[] packet) {
        final MaplePacketLittleEndianWriter mplew =
                new MaplePacketLittleEndianWriter(packet.length);
        mplew.write(packet);
        return mplew.getPacket();
    }
    ...
    public static void addCashItemInformation(
        ...
        mplew.writeLong(
                item.getPetId() > -1
                        ? item.getPetId()
                        : isRing ? equip.getRingId() : item.getCashId());
        if (!isGift) {
    ...
    public static byte[] startCPQ(MapleCharacter chr, MonsterCarnivalParty enemy) {
        ...
        mplew.writeShort(
                chr.getCarnivalParty().getAvailableCP()); // Obtained CP - Used CP of the team
    ...
    public static byte[] pyramidScore(
            byte score, int exp) { // Type cannot be higher than 4 (Rank D), otherwise you'll crash
        final MaplePacketLittleEndianWriter mplew = new MaplePacketLittleEndianWriter(7);
    ...
    public static byte[] moveDragon(
            MapleDragon dragon, Point startPos, List<LifeMovementFragment> res) {
        final MaplePacketLittleEndianWriter mplew = new MaplePacketLittleEndianWriter();
    ...
    /**
     * Changes the current background effect to either being rendered or not. Data is still missing,
     * so this is pretty binary at the moment in how it behaves.
     *
