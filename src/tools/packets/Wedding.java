...
/**
 * CField_Wedding, CField_WeddingPhoto, CWeddingMan, OnMarriageResult, and all Wedding/Marriage
 * enum/structs.
 *
 * @author Eric
 */
public class Wedding extends MaplePacketCreator {
    ...
    */
    ...
    public class GW_WeddingWishList {
        public static final int WEDDINGWL_MAX = 0xA; // enum WEDDINGWL
        public int dwReservationNo;
        public byte nGender;
    ...
    /**
     * A quick rundown of how (I think based off of enough BMS searching) WeddingPhoto_OnTakePhoto
     * works: - We send this packet with (first) the Groom / Bride IGNs - We then send a fieldId
     * (unsure about this part at the moment, 90% sure it's the id of the map) - After this, we
     * write an integer of the amount of characters within the current map (which is the Cake Map --
     * exclude users within Exit Map) - Once we've retrieved the size of the characters, we begin to
     * write information about them (Encode their name, guild, etc info) - Now that we've Encoded
     * our character data, we begin to Encode the ScreenShotPacket which requires a TemplateID, IGN,
     * and their positioning - Finally, after encoding all of our data, we send this packet out to a
     * MapGen application server - The MapGen server will then retrieve the packet byte array and
     * convert the bytes into a ImageIO 2D JPG output - The result after converting into a JPG will
     * then be remotely uploaded to /weddings/ with ReservedGroomName_ReservedBrideName to be
     * displayed on the web server.
     *
     * <p>- Will no longer continue Wedding Photos, needs a WvsMapGen :(
     *
     ...
     * @param m_dwUsers The List of all MapleCharacter guests within the current cake map to be
     *     encoded
     * @return mplew (MaplePacket) Byte array to be converted and read for byte[]->ImageIO
     */
    public static byte[] OnTakePhoto(
            String ReservedGroomName,
            String ReservedBrideName,
            int m_dwField,
        ...
        for (MapleCharacter guest : m_dwUsers) {
            ...
            mplew.writeMapleAsciiString(
                    guest.getGuildId() > 0 && guest.getGuild() != null
                            ? guest.getGuild().getName()
                            : "");
            mplew.writeShort(
                    guest.getGuildId() > 0 && guest.getGuild() != null
                            ? guest.getGuild().getLogoBG()
                            : 0);
            mplew.write(
                    guest.getGuildId() > 0 && guest.getGuild() != null
                            ? guest.getGuild().getLogoBGColor()
                            : 0);
            mplew.writeShort(
                    guest.getGuildId() > 0 && guest.getGuild() != null
                            ? guest.getGuild().getLogo()
                            : 0);
            mplew.write(
                    guest.getGuildId() > 0 && guest.getGuild() != null
                            ? guest.getGuild().getLogoColor()
                            : 0);
    ...
    /**
     * The wedding packet to display Pelvis Bebop and enable the Wedding Ceremony Effect between two
     * characters CField_Wedding::OnWeddingProgress - Stages CField_Wedding::OnWeddingCeremonyEnd -
     * Wedding Ceremony Effect
     *
     ...
     * @return mplew
     */
    public static byte[] OnWeddingProgress(
            boolean SetBlessEffect, int groom, int bride, byte step) {
        MaplePacketLittleEndianWriter mplew = new MaplePacketLittleEndianWriter();
        mplew.writeShort(SetBlessEffect ? WEDDING_CEREMONY_END : WEDDING_PROGRESS);
        if (!SetBlessEffect) { // in order for ceremony packet to send, byte step = 2 must be sent
            // first
            mplew.write(step);
        }
    ...
    public static byte[] OnWeddingGiftResult(byte mode, List<String> itemnames, List<Item> items) {
        ...
        switch (mode) {
            ...
            case 0xB:
                ...
                // 11 : You have sent a gift | 12 : You cannot give more than one present for
                // each wishlist | 13 : Failed to send the gift. | 14 : Failed to send the gift.
                if (mode == 0xB) {
                    mplew.write(itemnames.size());
                    for (String names : itemnames) {
                        mplew.writeMapleAsciiString(names);
                ...
                switch (items.get((items.size() - 1)).getInventoryType()) {
                    ...
                    default: // impossible flag, cash item can't be sent
                        if (items.get((items.size() - 1)).getInventoryType()
