...
// NOTE: this tool is currently unsupported since HS started using HTTPS. Missing proper SSL
// certificates to access Hidden-Street's website.
public class Main {

    private static final ArrayList<DropEntry> drop_entries = new ArrayList<>();
    private static final HashMap<String, Errors> problems = new HashMap<>();
    //    private static final String TEST_STRING = " <a href=\"/items/leftover/ligator-skin\"
    // alt=\"/tip.php?nid=2138\">Ligator Skin</a>, <a href=\"/items/leftover/the-magic-rock\"
    // alt=\"/tip.php?nid=3954\">The Magic Rock</a>, <a href=\"/items/quest/witch-grass-leaves\"
    // alt=\"/tip.php?nid=6129\">Witch Grass Leaves</a>    </td> ";
    private static final String BASE_URL = "https://bbb.hidden-street.net";
    private static final int VERSION = 83;
    private static final String[] pages = {
        "1-10", "11-20", "21-30", "31-40", "41-50", "51-60", "61-70", "71-80", "81-90", "91-100"
    };
    private static final String[] additionalPages88 = {"101-150", "151-200"};
    private static final String[] additionalPagesBB = {
    ...
    private static void crawlPage(String url) { // recursive method
        try {
            ...
            s.close();
            is.close();
            while (temp_data.toString().contains("class=\"monster\">")) {
                String monster_section =
                        getStringBetween(temp_data.toString(), "class=\"monster\">", "</table>");
                parseMonsterSection(monster_section);
                temp_data = new StringBuilder(trimUntil(temp_data.toString(), "</table>"));
            }
            if (temp_data.toString().contains("Go to next page")) {
                String next_url_segment =
                        getStringBetween(
                                temp_data.toString(),
    ...
    private static void parseMonsterSection(String html_data) {
        ...
        // parse useable drop
        parseItemSection(
                getStringBetween(html_data, "Useable drop:</strong>", "</td>"), monster_name);
        ...
        // parse equips
        parseItemSection(
                getStringBetween(html_data, "Common equipment:</strong>", "</div>"), monster_name);
        parseItemSection(
                getStringBetween(html_data, "Warrior equipment:</strong>", "</div>"), monster_name);
        parseItemSection(
                getStringBetween(html_data, "Magician equipment:</strong>", "</div>"),
                monster_name);
        parseItemSection(
                getStringBetween(html_data, "Bowman equipment:</strong>", "</div>"), monster_name);
        parseItemSection(
                getStringBetween(html_data, "Thief equipment:</strong>", "</div>"), monster_name);
        parseItemSection(
                getStringBetween(html_data, "Pirate equipment:</strong>", "</div>"), monster_name);
    ...
    private static void parseItemSection(String html_data, String monster_name) {
        ...
        while (temp_data.contains("<a href")) {
            ...
            // Process scrolls, nexon doesn't have the % on most of the scrolls. So we need to
            // remove it
            ...
            if (item_name.contains("100%")) {
                ...
                item_name =
                        item_name
                                .replaceAll("\\(\\)", "")
                                .trim(); // Hidden Street has a few scroll %'s with ()s around
                // them.. sigh
                ...
                item_name = item_name.replaceAll("\\(\\)", "").trim();
                // f(item_name.contains(" ()")) item_name = item_name.substring(0,
                // item_name.lastIndexOf(" ("));
            ...
            // drop entry
            var monster_ids = DataTool.monsterIdsFromName(monster_name);
            // DataTool.addMonsterIdsFromHardcodedName(monster_ids, monster_name);

            var item_ids = DataTool.itemIdsFromName(item_name);
            ...
            if (!monster_ids.isEmpty() && !item_ids.isEmpty()) {
                ...
                System.out.println(
                        "Error parsing item " + item_name + " dropped by " + monster_name + '.');
                ...
                // System.out.println("Monster ids size: " + monster_ids.size() + ", Item IDs size:
                // " + item_ids.size());
            }
        }
    }
    ...
    public static String getStringBetween(String line, String start, String end) {
        ...
        return line.substring(
                start_offset, line.substring(start_offset).indexOf(end) + start_offset);
    }
    ...
    public static void dumpErrors() {
        ...
        try {
            ...
            var bw = new BufferedWriter(new OutputStreamWriter(f, StandardCharsets.UTF_8));
            var pw = new PrintWriter(bw);

            for (var err : problems.values()) {
    ...
    public static void dumpQuery() {
        ...
        try {
            ...
            var bw = new BufferedWriter(new OutputStreamWriter(output, StandardCharsets.UTF_8));
            var pw = new PrintWriter(bw);
            pw.write("TRUNCATE TABLE `drop_data`;" + System.lineSeparator());
            pw.write(
                    "INSERT INTO `drop_data` (`dropperid`, `itemid`, `minimum_quantity`, `maximum_quantity`, `questid`, `chance`) VALUES ");
            for (var i = drop_entries.iterator(); i.hasNext(); ) {
                DropEntry de = i.next();
