...
public class MapleAlliance {
    private final List<Integer> guilds = new ArrayList<>();
    ...
    public static boolean canBeUsedAllianceName(String name) {
        ...
        try {
            ...
            try (PreparedStatement ps =
                    con.prepareStatement("SELECT name FROM alliance WHERE name = ?")) {
                ps.setString(1, name);
                rs = ps.executeQuery();
                if (rs.next()) {
                    ps.close();
    ...
    private static List<MapleCharacter> getPartyGuildMasters(MapleParty party) {
        List<MapleCharacter> mcl = new ArrayList<>();

        for (MaplePartyCharacter mpc : party.getMembers()) {
            if (mpc.getPlayer().getGuildRank() == 1
                    && mpc.getPlayer().getMapId() == party.getLeader().getPlayer().getMapId())
    ...
    public static MapleAlliance createAlliance(MapleParty party, String name) {
        ...
        if (alliance != null) {
            alliance.capacity = guilds.size();
            for (Integer g : guilds) alliance.addGuild(g);

            int id = alliance.allianceId;
            try {
                ...
                Server.getInstance()
                        .allianceMessage(
                                id,
                                MaplePacketCreator.updateAllianceInfo(
                                        alliance, guildMasters.get(0).getClient()),
                                -1,
                                -1);
            } catch (Exception e) {
                e.printStackTrace();
    ...
    public static MapleAlliance createAllianceOnDb(List<Integer> guilds, String name) {
        // will create an alliance, where the first guild listed is the leader and the alliance name
        // MUST BE already checked for unicity.
    ...
    public static void disbandAlliance(int allianceId) {
        ...
        try {
            ...
            Server.getInstance()
                    .allianceMessage(
                            allianceId, MaplePacketCreator.disbandAlliance(allianceId), -1, -1);
    ...
    public static boolean removeGuildFromAlliance(int allianceId, int guildId, int worldId) {
        ...
        srv.allianceMessage(
                alliance.allianceId,
                MaplePacketCreator.removeGuildFromAlliance(alliance, guildId, worldId),
                -1,
                -1);
        srv.removeGuildFromAlliance(alliance.allianceId, guildId);
        removeGuildFromAllianceOnDb(guildId);

        srv.allianceMessage(
                alliance.allianceId,
                MaplePacketCreator.getGuildAlliances(alliance, worldId),
                -1,
                -1);
        srv.allianceMessage(
                alliance.allianceId,
                MaplePacketCreator.allianceNotice(alliance.allianceId, alliance.notice),
                -1,
                -1);
        srv.guildMessage(guildId, MaplePacketCreator.disbandAlliance(alliance.allianceId));

        alliance.dropMessage(
                '[' + srv.getGuild(guildId, worldId).getName() + "] guild has left the union.");
        return true;
    }
    ...
    public void updateAlliancePackets(MapleCharacter chr) {
        if (allianceId > 0) {
            ...
            this.broadcastMessage(MaplePacketCreator.allianceNotice(allianceId, notice));
        }
    }
