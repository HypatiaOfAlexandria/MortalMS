...
public class MapleGuild {
    ...
    private final Lock membersLock =
            MonitoredReentrantLockFactory.createLock(MonitoredLockType.GUILD, true);
    ...
    public MapleGuild(int guildid, int world) {
        ...
        try {
            ...
            PreparedStatement ps =
                    con.prepareStatement("SELECT * FROM guilds WHERE guildid = " + guildid);
    ...
    public void buildNotifications() {
        ...
        synchronized (notifications) {
            if (notifications.keySet().size() != chs.size()) {
                notifications.clear();
                for (Integer ch : chs) {
                    notifications.put(ch, new ArrayList<>());
                }
            } else {
                    ...
                    for (Integer b : Server.getInstance().getOpenChannels(world)) {
                        if (!notifications.get(b).isEmpty()) {
                            if (bcop == BCOp.DISBAND) {
                                Server.getInstance()
                                        .getWorld(world)
                                        .setGuildAndRank(notifications.get(b), 0, 5, exceptionId);
                                ...
                                Server.getInstance()
                                        .getWorld(world)
                                        .sendPacket(notifications.get(b), packet, exceptionId);
                            }
                        }
                    }
                } catch (Exception re) {
                ...
                if (mgc.getId() == cid && initiator.getGuildRank() < mgc.getGuildRank()) {
                    ...
                    try {
                        if (mgc.isOnline()) {
                            ...
                            World.setOfflineGuildStatus(0, 5, cid);
                        }
                    } catch (Exception re) {
                        re.printStackTrace();
                        return;
    ...
    public void changeRank(MapleGuildCharacter mgc, int newRank) {
        try {
            if (mgc.isOnline()) {
                Server.getInstance()
                        .getWorld(mgc.getWorld())
                        .setGuildAndRank(mgc.getId(), this.id, newRank);
                mgc.setGuildRank(newRank);
            } else {
                World.setOfflineGuildStatus((short) this.id, (byte) newRank, mgc.getId());
        ...
        mc.getClient()
                .announce(
                        MaplePacketCreator.guildInvite(
                                c.getPlayer().getGuildId(), c.getPlayer().getName()));
        return null;
    }
    ...
    public static void displayGuildRanks(MapleClient c, int npcid) {
        try {
            ...
            try (PreparedStatement ps =
                    con.prepareStatement(
                            "SELECT `name`, `GP`, `logoBG`, `logoBGColor`, `logo`, `logoColor` FROM guilds WHERE NOT `guildid` = '1' ORDER BY `GP` DESC LIMIT 50")) {
                rs = ps.executeQuery();
    ...
    public void setAllianceId(int aid) {
        ...
        try {
            Connection con = DatabaseConnection.getConnection();
            try (PreparedStatement ps =
                    con.prepareStatement("UPDATE guilds SET allianceId = ? WHERE guildid = ?")) {
                ps.setInt(1, aid);
                ps.setInt(2, id);
                ps.executeUpdate();
            }
    ...
    public void resetAllianceGuildPlayersRank() {
        try {
            ...
            try (PreparedStatement ps =
                    con.prepareStatement(
                            "UPDATE characters SET allianceRank = ? WHERE guildid = ?")) {
                ps.setInt(1, 5);
                ps.setInt(2, id);
                ps.executeUpdate();
            }
    ...
    public static int getIncreaseGuildCost(int size) {
        int cost =
                ServerConstants.EXPAND_GUILD_BASE_COST
                        + Math.max(0, (size - 15) / 5) * ServerConstants.EXPAND_GUILD_TIER_COST;
