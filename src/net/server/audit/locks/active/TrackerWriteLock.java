...
/** @author RonanLana */
public class TrackerWriteLock extends ReentrantReadWriteLock.WriteLock
        implements MonitoredWriteLock {
    ...
    public void lock() {
        if (ServerConstants.USE_THREAD_TRACKER) {
            if (deadlockedState != null) {
                ...
                // FilePrinter.printError(FilePrinter.DEADLOCK_ERROR, "[CRITICAL] " +
                // dateFormat.format(new Date()) + " Deadlock occurred when trying to use the '" +
                // id.name() + "' lock resources:" + System.lineSeparator() + "" +
    ...
    public boolean tryLock() {
        if (super.tryLock()) {
            if (ServerConstants.USE_THREAD_TRACKER) {
                if (deadlockedState != null) {
                    // FilePrinter.printError(FilePrinter.DEADLOCK_ERROR, "Deadlock occurred when
                    // trying to use the '" + id.name() + "' lock resources:" +
                    // System.lineSeparator() + "" +
                    // printStackTrace(deadlockedState) + "" + System.lineSeparator() + "" +
                    // System.lineSeparator() + "");
                    ThreadTracker.getInstance().accessThreadTracker(true, true, id, hashcode);
                    deadlockedState = null;
                }
            ...
            if (reentrantCount.incrementAndGet() == 1) {
                ...
                timeoutSchedule =
                        TimerManager.getInstance()
                                .schedule(
                                        () -> issueDeadlock(t), ServerConstants.LOCK_MONITOR_TIME);
            }
        } finally {
            state.unlock();
        }
