...
public class MapleSessionCoordinator {
    ...
    private MapleSessionCoordinator() {
        for (int i = 0; i < 100; i++) {
            poolLock.add(
                    MonitoredReentrantLockFactory.createLock(MonitoredLockType.SERVER_LOGIN_COORD));
        }
    }
    ...
    private static long hwidExpirationUpdate(int relevance) {
        ...
        subdegreeTime = degree > 2 ? 10 : 1 + (3 * degree);
    ...
    private static void updateAccessAccount(
            Connection con, String remoteHwid, int accountId, int loginRelevance)
            throws SQLException {
        java.sql.Timestamp nextTimestamp =
                new java.sql.Timestamp(
                        Server.getInstance().getCurrentTime()
                                + hwidExpirationUpdate(loginRelevance));
    ...
    private static void registerAccessAccount(Connection con, String remoteHwid, int accountId)
        ...
        try (PreparedStatement ps =
                con.prepareStatement(
                        "INSERT INTO hwidaccounts (accountid, hwid, expiresat) VALUES (?, ?, ?)")) {
            ps.setInt(1, accountId);
            ps.setString(2, remoteHwid);
            ps.setTimestamp(
                    3,
                    new java.sql.Timestamp(
                            Server.getInstance().getCurrentTime() + hwidExpirationUpdate(0)));
        ...
        try {
            ...
            try (Connection con = DatabaseConnection.getConnection();
                    PreparedStatement ps =
                            con.prepareStatement(
                                    "SELECT SQL_CACHE hwid FROM hwidaccounts WHERE accountid = ?")) {
                int hwidCount = 0;
                ps.setInt(1, accountId);
    ...
    private static boolean attemptAccessAccount(
            String nibbleHwid, int accountId, boolean routineCheck) {
        try {

            try (Connection con = DatabaseConnection.getConnection();
                    PreparedStatement ps =
                            con.prepareStatement(
                                    "SELECT SQL_CACHE * FROM hwidaccounts WHERE accountid = ?")) {
                int hwidCount = 0;
                ps.setInt(1, accountId);
                try (ResultSet rs = ps.executeQuery()) {
                    while (rs.next()) {
                        ...
                        if (rsHwid.endsWith(nibbleHwid)) {
                            if (!routineCheck) {
                                // better update HWID relevance as soon as the login is
                                // authenticated
    ...
    public AntiMulticlientResult attemptLoginSession(
            IoSession session, int accountId, boolean routineCheck) {
    ...
    public AntiMulticlientResult attemptLoginSession(
            IoSession session, String nibbleHwid, int accountId, boolean routineCheck) {
        if (!ServerConstants.DETERRED_MULTICLIENT) return AntiMulticlientResult.SUCCESS;
            ...
            if (nibbleHwid != null) {
                ...
                if (remoteHwid.endsWith(nibbleHwid)) {
                    if (!onlineRemoteHwids.contains(remoteHwid)) {
                        ...
                        // updated session CLIENT_HWID attribute will be set when the player log in
                        // the game
                        onlineRemoteHwids.add(remoteHwid);

                        cachedHostHwids.put(remoteHost, remoteHwid);
                        cachedHostTimeout.put(
                                remoteHost,
                                Server.getInstance().getCurrentTime()
                                        + 604800000); // 1 week-time entry
        ...
        hwid =
                (String)
                        session.removeAttribute(
                                MapleClient.CLIENT_NIBBLEHWID); // making sure to clean up calls to
        // this function on login phase
        onlineRemoteHwids.remove(hwid);
    ...
    public void runUpdateHwidHistory() {
        try {
            try (Connection con = DatabaseConnection.getConnection();
                    PreparedStatement ps =
                            con.prepareStatement(
                                    "DELETE FROM hwidaccounts WHERE expiresat < CURRENT_TIMESTAMP")) {
                ps.execute();
            }
        } catch (SQLException ex) {
            ex.printStackTrace();
