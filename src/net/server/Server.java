...
public class Server {
    ...
    private static final List<Integer> activeCoupons = new ArrayList<>();

    private IoAcceptor acceptor;
    private List<Map<Integer, String>> channels = new ArrayList<>();
    ...
    private List<Pair<Integer, String>> worldRecommendedList = new ArrayList<>();
    ...
    private final List<MapleClient> processDiseaseAnnouncePlayers = new ArrayList<>();
    private final List<MapleClient> registeredDiseaseAnnouncePlayers = new ArrayList<>();

    private final List<List<Pair<String, Integer>>> playerRanking = new ArrayList<>();

    private final Lock srvLock = MonitoredReentrantLockFactory.createLock(MonitoredLockType.SERVER);
    private final Lock disLock =
            MonitoredReentrantLockFactory.createLock(MonitoredLockType.SERVER_DISEASES);

    private final ReentrantReadWriteLock wldLock =
    ...
    private final ReentrantReadWriteLock lgnLock =
    ...
    public long getCurrentTime() { // returns a slightly delayed time value, under frequency of
        // UPDATE_INTERVAL
    ...
    public int addChannel(int worldid) {
        ...
        try {
            ...
            // returns a slightly delayed time value, under frequency of
            // UPDATE_INTERVAL
            Channel channel = new Channel(worldid, channelid, serverCurrentTime);
    ...
    private int initWorld(Properties p) {
        ...
        try {
            ...
            // returns a slightly delayed time value, under frequency of
            // UPDATE_INTERVAL
            long bootTime = serverCurrentTime;
    ...
    public static Map<Integer, Integer> getCouponRates() {
        return couponRates;
    }

    private static void loadCouponRates(Connection c) throws SQLException {
    ...
    public static List<Integer> getActiveCoupons() {
    ...
    public static void updateActiveCoupons() throws SQLException {
        synchronized (activeCoupons) {
            activeCoupons.clear();
    ...
    public void runAnnouncePlayerDiseasesSchedule() {
        ...
        disLock.lock();
        try {
            processDiseaseAnnounceClients = new ArrayList<>(processDiseaseAnnouncePlayers);
        ...
        disLock.lock();
        try {
            // this is to force the system to wait for at least one complete tick before releasing
            // disease info for the registered clients
    ...
    public List<Pair<String, Integer>> getWorldPlayerRanking(int worldid) {
        ...
        try {
            return new ArrayList<>(
                    playerRanking.get(!ServerConstants.USE_WHOLE_SERVER_RANKING ? worldid : 0));
        } finally {
            wldRLock.unlock();
        }
    }
    ...
    private void installWorldPlayerRanking(int worldid) {
        List<Pair<Integer, List<Pair<String, Integer>>>> ranking =
                updatePlayerRankingFromDB(worldid);
        if (!ranking.isEmpty()) {
            wldWLock.lock();
            try {
    ...
    public void updateWorldPlayerRanking() {
        List<Pair<Integer, List<Pair<String, Integer>>>> rankUpdates =
                updatePlayerRankingFromDB(-1 * (this.getWorldsSize() - 1));
        if (!rankUpdates.isEmpty()) {
            wldWLock.lock();
            try {
                if (!ServerConstants.USE_WHOLE_SERVER_RANKING) {
                    for (int i = playerRanking.size();
                            i <= rankUpdates.get(rankUpdates.size() - 1).getLeft();
                            i++) {
                        playerRanking.add(new ArrayList<>(0));
                    }
    ...
    private static List<Pair<Integer, List<Pair<String, Integer>>>> updatePlayerRankingFromDB(
            int worldid) {
        ...
        try {
            ...
            if (!ServerConstants.USE_WHOLE_SERVER_RANKING) {
                worldQuery =
                        worldid >= 0
                                ? " AND `characters`.`world` = " + worldid
                                : " AND `characters`.`world` >= 0 AND `characters`.`world` <= "
                                        + -worldid;
            } else {
                worldQuery =
                        (" AND `characters`.`world` >= 0 AND `characters`.`world` <= "
            ...
            ps =
                    con.prepareStatement(
                            "SELECT `characters`.`name`, `characters`.`level`, `characters`.`world` FROM `characters` LEFT JOIN accounts ON accounts.id = characters.accountid WHERE `characters`.`gm` < 2 AND `accounts`.`banned` = '0'"
                                    + worldQuery
                                    + " ORDER BY "
                                    + (!ServerConstants.USE_WHOLE_SERVER_RANKING ? "world, " : "")
                                    + "level DESC, exp DESC LIMIT 50");
            rs = ps.executeQuery();
    ...
    public void init() {
        ...
        tMan.register(
                new CharacterDiseaseWorker(),
                ServerConstants.UPDATE_INTERVAL,
                ServerConstants.UPDATE_INTERVAL);
        ...
        try {
            int worldCount =
                    Math.min(
                            GameConstants.WORLD_NAMES.length,
    ...
    public void deleteGuildCharacter(MapleGuildCharacter mgc) {
        if (mgc.getCharacter() != null) setGuildMemberOnline(mgc.getCharacter(), false, -1);
    ...
    public static void changeFly(Integer accountid, boolean canFly) {
        if (canFly) {
    ...
    public static boolean canFly(Integer accountid) {
    ...
    public void createCharacterEntry(MapleCharacter chr) {
        ...
        lgnWLock.lock();
        try {
            accountCharacterCount.put(
                    accountid, (short) (accountCharacterCount.get(accountid) + 1));
    ...
    public void transferWorldCharacterEntry(
            MapleCharacter chr,
        ...
        try {
            Integer chrid = chr.getId(),
                    ...
                    world = worldChars.get(chr.getId());
            if (world != null) {
                World wserv = this.getWorld(world);
                if (wserv != null) wserv.unregisterAccountCharacterView(accountid, chrid);
            }
    ...
    */
    ...
    public Pair<Pair<Integer, List<MapleCharacter>>, List<Pair<Integer, List<MapleCharacter>>>>
            loadAccountCharlist(Integer accountId, int visibleWorlds) {
        List<World> wlist = this.getWorlds();
            ...
            for (World w : wlist) {
                ...
                if (wchars == null) {
                    if (!accountChars.containsKey(accountId)) {
                        ...
                        accountChars.put(
                                accountId,
                                new HashSet<>()); // not advisable at all to write on the map on
                        // a read-protected environment
    ...
    private static Pair<Short, List<List<MapleCharacter>>> loadAccountCharactersViewFromDb(
            int accId, int wlen) {
        short characterCount = 0;
        List<List<MapleCharacter>> wchars = new ArrayList<>(wlen);
        for (int i = 0; i < wlen; i++) wchars.add(i, new ArrayList<>());
        ...
        try {
            ...
            for (Pair<Item, Integer> ae : accEquips) {
                List<Item> playerEquips =
                        accPlayerEquips.computeIfAbsent(ae.getRight(), k -> new ArrayList<>());
            ...
            try (PreparedStatement ps =
                ...
                try (ResultSet rs = ps.executeQuery()) {
                    while (rs.next()) {
                        ...
                        chars.add(
                                MapleCharacter.loadCharacterEntryFromDB(
                                        rs, accPlayerEquips.get(cid)));
                    }
                }
            }
            con.close();
    ...
    private int loadAccountCharactersView(
            Integer accId, int gmLevel, int fromWorldid) { // returns the maximum gmLevel found
        List<World> wlist = this.getWorlds();
        Pair<Short, List<List<MapleCharacter>>> accCharacters =
                loadAccountCharactersViewFromDb(accId, wlist.size());
    ...
    private void disconnectIdlesOnLoginState() {
        srvLock.lock();
        try {
            List<MapleClient> toDisconnect = new ArrayList<>();
            long timeNow = System.currentTimeMillis();
    ...
    public final Runnable shutdown(
            final boolean restart) { // no player should be online when trying to shutdown!
        return () -> {
            srvLock.lock();
