    ...
    public final void handlePacket(SeekableLittleEndianAccessor slea, MapleClient c) {
        ...
        if (ServerConstants.AUTOMATIC_REGISTER && loginok == 5) {
            try {
                ...
                ps.setString(
                        3, "2018-06-20"); // Jayd's idea: was added to solve the MySQL 5.7 strict
                // checking (birthday)
                ps.setString(
                        4, "2018-06-20"); // Jayd's idea: was added to solve the MySQL 5.7 strict
                // checking (tempban)
                ps.executeUpdate();
        ...
        if (ServerConstants.BCRYPT_MIGRATION
                && (loginok
        ...
        if (tempban != null) {
            if (tempban.getTimeInMillis() > Calendar.getInstance().getTimeInMillis()) {
                c.announce(
                        MaplePacketCreator.getTempBan(tempban.getTimeInMillis(), c.getGReason()));
                return;
            }
        }
        if (loginok == 3) {
