...
public final class ViewAllCharHandler extends AbstractMaplePacketHandler {
    ...
    public final void handlePacket(SeekableLittleEndianAccessor slea, MapleClient c) {
        try {
            ...
            Pair<Pair<Integer, List<MapleCharacter>>, List<Pair<Integer, List<MapleCharacter>>>>
                    loginBlob =
                            Server.getInstance()
                                    .loadAccountCharlist(accountId, c.getVisibleWorlds());
            ...
            if (chrTotal > 9) {
                ...
                if (padRight > 0 && lastwchars != null) {
                    ...
                    for (int i = padRight;
                            i < 3;
                            i++) { // filling the remaining slots with the last character loaded
                        chrTotal++;
                        lastwchars.add(chr);
                    }
                }
            ...
            for (Pair<Integer, List<MapleCharacter>> wchars : worldChars) {
                c.announce(
                        MaplePacketCreator.showAllCharacterInfo(
                                wchars.getLeft(), wchars.getRight(), ServerConstants.ENABLE_PIC));
            }
        } catch (Exception e) {
            e.printStackTrace();
        }
