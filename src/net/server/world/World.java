...
public class World {
    ...
    private final ReentrantReadWriteLock chnLock =
            new MonitoredReentrantReadWriteLock(MonitoredLockType.WORLD_CHANNELS, true);
    private final ReadLock chnRLock = chnLock.readLock();
    private final WriteLock chnWLock = chnLock.writeLock();

    private final Map<Integer, SortedMap<Integer, MapleCharacter>> accountChars = new HashMap<>();
    private MonitoredReentrantLock accountCharsLock =
            MonitoredReentrantLockFactory.createLock(MonitoredLockType.WORLD_CHARS, true);

    private final Set<Integer> queuedGuilds = new HashSet<>();
    private final Map<Integer, Pair<Pair<Boolean, Boolean>, Pair<Integer, Integer>>>
            queuedMarriages = new HashMap<>();
    private final Map<Integer, Set<Integer>> marriageGuests = new HashMap<>();
    ...
    private MonitoredReentrantLock partyLock =
            MonitoredReentrantLockFactory.createLock(MonitoredLockType.WORLD_PARTY, true);
    ...
    private final ReentrantReadWriteLock suggestLock =
            new MonitoredReentrantReadWriteLock(MonitoredLockType.WORLD_SUGGEST, true);
    private final ReadLock suggestRLock = suggestLock.readLock();
    private final WriteLock suggestWLock = suggestLock.writeLock();

    private final Map<Integer, Integer> disabledServerMessages = new HashMap<>(); // reuse owl lock
    private MonitoredReentrantLock srvMessagesLock =
            MonitoredReentrantLockFactory.createLock(MonitoredLockType.WORLD_SRVMESSAGES);
    private ScheduledFuture<?> srvMessagesSchedule;

    private MonitoredReentrantLock activePetsLock =
            MonitoredReentrantLockFactory.createLock(MonitoredLockType.WORLD_PETS, true);
    private final Map<Integer, Integer> activePets = new LinkedHashMap<>();
    private ScheduledFuture<?> petsSchedule;
    private long petUpdate;

    private MonitoredReentrantLock activeMountsLock =
            MonitoredReentrantLockFactory.createLock(MonitoredLockType.WORLD_MOUNTS, true);
    ...
    private final Map<Integer, Pair<MapleHiredMerchant, Integer>> activeMerchants =
            new LinkedHashMap<>();
    ...
    public World(
        ...
        merchantSchedule =
                tman.register(new HiredMerchantWorker(this), 10 * 60 * 1000, 10 * 60 * 1000);
        timedMapObjectsSchedule =
                tman.register(new TimedMapObjectWorker(this), 60 * 1000, 60 * 1000);
        charactersSchedule =
                tman.register(new CharacterAutosaverWorker(this), 60 * 60 * 1000, 60 * 60 * 1000);
        marriagesSchedule =
                tman.register(
                        new WeddingReservationWorker(this),
                        ServerConstants.WEDDING_RESERVATION_INTERVAL * 60 * 1000,
                        ServerConstants.WEDDING_RESERVATION_INTERVAL * 60 * 1000);
    }
    ...
    public void setExpRate(int exp) {
        List<MapleCharacter> list = new ArrayList<>(players.getAllCharacters());
    ...
    public int getTransportationTime(int travelTime) {
        return (travelTime + travelrate - 1) / travelrate;
    }
    ...
    public void loadAccountCharactersView(Integer accountId, List<MapleCharacter> chars) {
        ...
        accountCharsLock
                .lock(); // accountCharsLock should be used after server's lgnWLock for compliance
        try {
    ...
    private static List<Entry<Integer, SortedMap<Integer, MapleCharacter>>>
            getSortedAccountCharacterView(Map<Integer, SortedMap<Integer, MapleCharacter>> map) {
    ...
    public List<MapleCharacter> getAllCharactersView() { // sorted by accountid, charid
        ...
        for (Entry<Integer, SortedMap<Integer, MapleCharacter>> e :
                getSortedAccountCharacterView(accChars)) {
    ...
    public List<MapleCharacter> getAccountCharactersView(Integer accountId) {
        ...
        try {
            ...
            if (accChars != null) {
                chrList = new ArrayList<>(accChars.values());
            } else {
    ...
    public int getWorldCapacityStatus() {
        ...
        if (num >= worldCap) {
            status = 2;
        } else if (num >= worldCap * 0.8) { // More than 80 percent o___o
            status = 1;
        } else {
            status = 0;
        }
    ...
    public static void setOfflineGuildStatus(int guildid, int guildrank, int cid) {
        try {
    ...
    public void setGuildAndRank(int cid, int guildid, int rank) {
        MapleCharacter mc = players.getCharacterById(cid);
        if (mc == null) {
            return;
        }
    ...
    public void putMarriageQueued(
            int marriageId, boolean cathedral, boolean premium, int groomId, int brideId) {
        queuedMarriages.put(
                marriageId,
                new Pair<>(new Pair<>(cathedral, premium), new Pair<>(groomId, brideId)));
    ...
    public Pair<Integer, Integer> getWeddingCoupleForGuest(int guestId, Boolean cathedral) {
        ...
        List<Integer> possibleWeddings = new ArrayList<>();
    ...
    private void updateCharacterParty(
            MapleParty party,
    ...
    private void updateParty(
            MapleParty party, PartyOperation operation, MaplePartyCharacter target) {
        Collection<MaplePartyCharacter> partyMembers = party.getMembers();
        updateCharacterParty(party, operation, target, partyMembers);

        for (MaplePartyCharacter partychar : partyMembers) {
            MapleCharacter chr = players.getCharacterById(partychar.getId());
            if (chr != null) {
        ...
        switch (operation) {
            case LEAVE:
            case EXPEL:
                MapleCharacter chr = players.getCharacterById(target.getId());
                if (chr != null) {
                    chr.getClient()
                            .announce(
                                    MaplePacketCreator.updateParty(
    ...
    public void updateParty(int partyid, PartyOperation operation, MaplePartyCharacter target) {
        ...
        assert party != null : "no party with the specified partyid exists";
        switch (operation) {
            case JOIN:
    ...
    public int find(String name) {
        int channel = -1;
        MapleCharacter chr = players.getCharacterByName(name);
        if (chr != null) {
    ...
    public void partyChat(MapleParty party, String chattext, String namefrom) {
        for (MaplePartyCharacter partychar : party.getMembers()) {
            if (!(partychar.getName().equals(namefrom))) {
                MapleCharacter chr = players.getCharacterByName(partychar.getName());
                if (chr != null) {
    ...
    public void buddyChat(
        ...
        PlayerStorage playerStorage = players;
    ...
    public void leaveMessenger(int messengerid, MapleMessengerCharacter target) {
        ...
        assert messenger != null : "No messenger with the specified messengerid exists";
    ...
    public void messengerInvite(String sender, int messengerid, String target, int fromchannel) {
        if (isConnected(target)) {
            MapleMessenger messenger = players.getCharacterByName(target).getMessenger();
            if (messenger == null) {
                players.getCharacterByName(target)
                        .getClient()
                        .announce(MaplePacketCreator.messengerInvite(sender, messengerid));
                MapleCharacter from =
                        getChannel(fromchannel).getPlayerStorage().getCharacterByName(sender);
    ...
    public void addMessengerPlayer(
            MapleMessenger messenger, String namefrom, int fromchannel, int position) {
        for (MapleMessengerCharacter messengerchar : messenger.getMembers()) {
            MapleCharacter chr = players.getCharacterByName(messengerchar.getName());
            if (chr == null) {
                continue;
            }
            if (!messengerchar.getName().equals(namefrom)) {
                MapleCharacter from =
                        getChannel(fromchannel).getPlayerStorage().getCharacterByName(namefrom);
                chr.getClient()
                        .announce(
                                MaplePacketCreator.addMessengerPlayer(
                                        namefrom, from, position, (byte) (fromchannel - 1)));
                from.getClient()
                        .announce(
                                MaplePacketCreator.addMessengerPlayer(
                                        chr.getName(),
                ...
                chr.getClient()
                        .announce(MaplePacketCreator.joinMessenger(messengerchar.getPosition()));
            }
        }
    }
    ...
    public void declineChat(String target, String namefrom) {
        if (isConnected(target)) {
            MapleCharacter chr = players.getCharacterByName(target);
    ...
    public void updateMessenger(
            MapleMessenger messenger, String namefrom, int position, int fromchannel) {
        for (MapleMessengerCharacter messengerchar : messenger.getMembers()) {
    ...
    public boolean isConnected(String charName) {
        return players.getCharacterByName(charName) != null;
    }
    ...
    public void whisper(String sender, String target, int channel, String message) {
        if (isConnected(target)) {
            players.getCharacterByName(target)
                    .getClient()
                    .announce(MaplePacketCreator.getWhisper(sender, channel, message));
        }
    }

    public BuddyAddResult requestBuddyAdd(
            String addName, int channelFrom, int cidFrom, String nameFrom) {
        MapleCharacter addChar = players.getCharacterByName(addName);
        if (addChar != null) {
    ...
    public void buddyChanged(
            int cid, int cidFrom, String name, int channel, BuddyOperation operation) {
        MapleCharacter addChar = players.getCharacterById(cid);
        if (addChar != null) {
            BuddyList buddylist = addChar.getBuddylist();
            switch (operation) {
                case ADDED:
                    if (buddylist.contains(cidFrom)) {
                        buddylist.put(
                                new BuddylistEntry(name, "Default Group", cidFrom, channel, true));
                        addChar.getClient()
                                .announce(
                                        MaplePacketCreator.updateBuddyChannel(
                                                cidFrom, (byte) (channel - 1)));
                    }
                    break;
                case DELETED:
                    if (buddylist.contains(cidFrom)) {
                        ...
                        addChar.getClient()
                                .announce(MaplePacketCreator.updateBuddyChannel(cidFrom, -1));
                    }
                    break;
            }
        }
    ...
    public void addOwlItemSearch(Integer itemid) {
        suggestWLock.lock();
        try {
            owlSearched.merge(itemid, 1, (a, b) -> a + b);
        } finally {
            suggestWLock.unlock();
        }
    }
    ...
    public void addCashItemBought(Integer snid) {
        ...
        try {
            ...
            tabItemBought.merge(snid, 1, (a, b) -> a + b);
        } finally {
            suggestWLock.unlock();
        }
    }
    ...
    private List<List<Pair<Integer, Integer>>> getBoughtCashItems() {
        ...
        suggestRLock.lock();
        try {
            List<List<Pair<Integer, Integer>>> boughtCounts =
                    new ArrayList<>(cashItemBought.size());

            for (Map<Integer, Integer> tab : cashItemBought) {
                List<Pair<Integer, Integer>> tabItems = new ArrayList<>();
    ...
    private static List<Integer> getMostSellerOnTab(List<Pair<Integer, Integer>> tabSellers) {
        List<Integer> tabLeaderboards;

        // descending order
        Comparator<Pair<Integer, Integer>> comparator =
                (p1, p2) -> p2.getRight().compareTo(p1.getRight());

        PriorityQueue<Pair<Integer, Integer>> queue =
                new PriorityQueue<>(Math.max(1, tabSellers.size()), comparator);
        queue.addAll(tabSellers);

        tabLeaderboards = new ArrayList<>();
        ...
        for (List<Pair<Integer, Integer>> tabSellers : mostSellers) {
            if (tabSellers.size() < 5) {
                if (allLeaderboards == null) {
                    List<Pair<Integer, Integer>> allSellers = new ArrayList<>();
    ...
    public void registerPetHunger(MapleCharacter chr, byte petSlot) {
        if (chr.isGM() && ServerConstants.GM_PETS_NEVER_HUNGRY
                || ServerConstants.PETS_NEVER_HUNGRY) {
            return;
        }
        ...
        try {
            int initProc;
            initProc =
                    Server.getInstance().getCurrentTime() - petUpdate > 55000
                            ? ServerConstants.PET_EXHAUST_COUNT - 2
                            : ServerConstants.PET_EXHAUST_COUNT - 1;
    ...
    public void runPetSchedule() {
        ...
        for (Map.Entry<Integer, Integer> dp : deployedPets.entrySet()) {
            MapleCharacter chr = players.getCharacterById(dp.getKey() / 4);
    ...
    public void registerMountHunger(MapleCharacter chr) {
        ...
        try {
            int initProc;
            initProc =
                    Server.getInstance().getCurrentTime() - mountUpdate > 45000
                            ? ServerConstants.MOUNT_EXHAUST_COUNT - 2
                            : ServerConstants.MOUNT_EXHAUST_COUNT - 1;
    ...
    public void runMountSchedule() {
        ...
        for (Map.Entry<Integer, Integer> dp : deployedMounts.entrySet()) {
            MapleCharacter chr = players.getCharacterById(dp.getKey());
            if (chr == null || !chr.isLoggedinWorld()) continue;
    ...
    public void registerHiredMerchant(MapleHiredMerchant hm) {
        ...
        try {
            int initProc;
            initProc =
                    Server.getInstance().getCurrentTime() - merchantUpdate > 5 * 60 * 1000 ? 1 : 0;
    ...
    public void runHiredMerchantSchedule() {
        ...
        try {
            ...
            for (Map.Entry<Integer, Pair<MapleHiredMerchant, Integer>> dm :
                    deployedMerchants.entrySet()) {
                int timeOn = dm.getValue().getRight();
                MapleHiredMerchant hm = dm.getValue().getLeft();

                if (timeOn <= 144) { // 1440 minutes == 24hrs
                    activeMerchants.put(
                            hm.getOwnerId(), new Pair<>(dm.getValue().getLeft(), timeOn + 1));
                } else {
                    hm.forceClose();
                ...
                if (chr != null && chr.isLoggedinWorld()) {
                    chr.announce(
                            MaplePacketCreator.serverMessage(
                                    chr.getClient().getChannelServer().getServerMessage()));
                }
            }
        }
    }
    ...
    private static void executePlayerNpcMapDataUpdate(
            Connection con,
            boolean isPodium,
            Map<Integer, ?> pnpcData,
            int value,
            int worldid,
            int mapid)
            throws SQLException {
        ...
        ps =
                pnpcData.containsKey(mapid)
                        ? con.prepareStatement(
                                "UPDATE playernpcs_field SET "
                                        + (isPodium ? "podium" : "step")
                                        + " = ? WHERE world = ? AND map = ?")
                        : con.prepareStatement(
                                "INSERT INTO playernpcs_field ("
                                        + (isPodium ? "podium" : "step")
                                        + ", world, map) VALUES (?, ?, ?)");
    ...
    public List<Pair<MaplePlayerShopItem, AbstractMapleMapObject>> getAvailableItemBundles(
            int itemid) {
        ...
        hmsAvailable.subList(
                0,
                Math.min(
                        hmsAvailable.size(), 200)); // truncates the list to have up to 200 elements
        return hmsAvailable;
    }

    private void pushRelationshipCouple(Pair<Integer, Pair<Integer, Integer>> couple) {
        int mid = couple.getLeft(),
                hid = couple.getRight().getLeft(),
                wid = couple.getRight().getRight();
    ...
    public Pair<Integer, Integer> getRelationshipCouple(int relationshipId) {
        ...
        if (rc == null) {
            Pair<Integer, Pair<Integer, Integer>> couple =
                    getRelationshipCoupleFromDb(relationshipId, true);
    ...
    public int getRelationshipId(int playerId) {
        ...
        if (ret == null) {
            Pair<Integer, Pair<Integer, Integer>> couple =
                    getRelationshipCoupleFromDb(playerId, false);
    ...
    private static Pair<Integer, Pair<Integer, Integer>> getRelationshipCoupleFromDb(
            int id, boolean usingMarriageId) {
        try {
            Connection con = DatabaseConnection.getConnection();
            ...
            if (usingMarriageId) {
                ...
                ps =
                        con.prepareStatement(
                                "SELECT * FROM marriages WHERE husbandid = ? OR wifeid = ?");
                ps.setInt(1, id);
                ps.setInt(2, id);
            }
    ...
    private static void deleteRelationshipFromDb(int playerId) {
        try {
            Connection con = DatabaseConnection.getConnection();
            PreparedStatement ps =
                    con.prepareStatement("DELETE FROM marriages WHERE marriageid = ?");
    ...
    public void dropMessage(int type, String message) {
        for (MapleCharacter player : players.getAllCharacters()) {
