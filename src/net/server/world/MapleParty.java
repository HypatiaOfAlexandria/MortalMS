...
public class MapleParty {
    ...
    private MonitoredReentrantLock lock =
            MonitoredReentrantLockFactory.createLock(MonitoredLockType.PARTY, true);
    ...
    // used whenever entering PQs: will draw every party member that can attempt a target PQ while
    // ingnoring those unfit.
    ...
    public List<Integer> getMembersSortedByHistory() {
        ...
        lock.lock();
        try {
            histList = new ArrayList<>(histMembers.entrySet());
        } finally {
            lock.unlock();
        }

        histList.sort(Comparator.comparing(o -> (o.getValue())));

        List<Integer> histSort = new ArrayList<>();
    ...
    public void assignNewLeader(MapleClient c) {
        ...
        if (newLeadr != null) {
            world.updateParty(id, PartyOperation.CHANGE_LEADER, newLeadr);
        }
    }
