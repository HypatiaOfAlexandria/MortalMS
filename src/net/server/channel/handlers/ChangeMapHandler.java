...
public final class ChangeMapHandler extends AbstractMaplePacketHandler {
    ...
    public final void handlePacket(SeekableLittleEndianAccessor slea, MapleClient c) {
        ...
        if (
        /* chr.isChangingMaps() || */ chr.isBanned()) {
            if (chr.isChangingMaps()) {
                FilePrinter.printError(
                        FilePrinter.PORTAL_STUCK + chr.getName() + ".txt",
                        "Player "
                                + chr.getName()
                                + " got stuck when changing maps. Timestamp: "
                                + Calendar.getInstance().getTime()
                                + " Last visited mapids: "
                                + chr.getLastVisitedMapids());
            }
                ...
                if (targetid != -1 && !chr.isAlive()) {
                    ...
                    if (executeStandardPath) {
                        ...
                        if (wheel && chr.haveItemWithId(5510000, false)) {
                            ...
                            chr.announce(
                                    MaplePacketCreator.showWheelsLeft(
                                            chr.getItemQuantity(5510000, false)));
                    ...
                    if (chr.isGM()) {
                        ...
                        if (divi == 0) {
                            ...
                            if (targetid == 104000000) {
                                ...
                                == 9130401) { // Only allow warp if player is already in Intro map,
                            // or else = hack
                            if (targetid == 130000000
                                    || targetid / 100 == 9130401) { // Cygnus introduction
                                warp = true;
                            }
                        } else if (divi == 9140900) { // Aran Introduction
                            if (targetid == 914090011
                                    || targetid == 914090012
                                    || targetid == 914090013
                                    || targetid == 140090000) {
                                warp = true;
