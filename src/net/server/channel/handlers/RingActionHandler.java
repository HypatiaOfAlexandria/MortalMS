...
public final class RingActionHandler extends AbstractMaplePacketHandler {
        ...
        return useItemId == 2240000
                ...
                : (useItemId == 2240001
                        ...
                        : (useItemId == 2240002
                                ...
                                : (useItemId == 2240003
                                        ? 4031363
                                        : (1112300 + (useItemId - 2240004)))));
    }

    public static void sendEngageProposal(
            final MapleClient c, final String name, final int itemid) {
        ...
        if (source.getPartnerId() > 0) {
            source.dropMessage(1, "You're already engaged!");
            source.announce(Wedding.OnMarriageResult((byte) 0));
            return;
        }
        if (source.getMarriageItemId() > 0) {
        ...
        if (!target.getMap().equals(source.getMap())) {
            source.dropMessage(1, "Make sure your partner is on the same map!");
            source.announce(Wedding.OnMarriageResult((byte) 0));
            return;
        }
        if (!source.haveItem(itemid) || itemid < 2240000 || itemid > 2240015) {
            source.announce(Wedding.OnMarriageResult((byte) 0));
            return;
        }
        if (target.isMarried()) {
        ...
        if (target.getPartnerId() > 0 || target.getMarriageItemId() > 0) {
        ...
        if (!MapleInventoryManipulator.checkSpace(c, newBoxId, 1, "")) {
            source.dropMessage(5, "You don't have a ETC slot available right now!");
            source.announce(Wedding.OnMarriageResult((byte) 0));
            return;
        }
        if (!MapleInventoryManipulator.checkSpace(target.getClient(), newBoxId + 1, 1, "")) {
            source.dropMessage(
                    5, "The girl you proposed doesn't have a ETC slot available right now.");
            source.announce(Wedding.OnMarriageResult((byte) 0));
            return;
        }
    ...
    private static void eraseEngagementOffline(int characterId, Connection con)
        ...
        PreparedStatement ps =
                con.prepareStatement(
                        "UPDATE characters SET marriageItemId=-1, partnerId=-1 WHERE id=?");
    ...
    private static void breakEngagementOffline(int characterId) {
        try {
            Connection con = DatabaseConnection.getConnection();
            PreparedStatement ps =
                    con.prepareStatement("SELECT marriageItemId FROM characters WHERE id=?");
            ps.setInt(1, characterId);
            ...
            if (rs.next()) {
                ...
                if (marriageItemId > 0) {
                    PreparedStatement ps2 =
                            con.prepareStatement(
                                    "UPDATE inventoryitems SET expiration=0 WHERE itemid=? AND characterid=?");
    ...
    private static synchronized void breakMarriage(MapleCharacter chr) {
        ...
        MapleCharacter partner =
                chr.getClient().getWorldServer().getPlayerStorage().getCharacterById(partnerid);
        if (partner == null) {
            eraseEngagementOffline(partnerid);
        } else {
            partner.dropMessage(5, chr.getName() + " has decided to break up the marriage.");

            // partner.announce(Wedding.OnMarriageResult((byte) 0)); ok, how to gracefully unengage
            // someone without the need to cc?
        ...
        chr.dropMessage(
                5,
                "You have successfully break the marriage with "
                        + MapleCharacter.getNameById(partnerid)
                        + '.');
    ...
    private static synchronized void breakEngagement(MapleCharacter chr) {
        int partnerid = chr.getPartnerId();
        ...
        if (chr.haveItem(marriageitemid)) {
            MapleInventoryManipulator.removeById(
                    chr.getClient(), MapleInventoryType.ETC, marriageitemid, 1, false, false);
        }
        chr.dropMessage(
                5,
                "You have successfully break the engagement with "
                        + MapleCharacter.getNameById(partnerid)
                        + '.');
    ...
    public static void breakMarriageRing(MapleCharacter chr, final int wItemId) {
        ...
        final boolean weddingToken =
                (wItem != null && type == MapleInventoryType.ETC && wItemId / 10000 == 403);
    ...
    public static void giveMarriageRings(
            MapleCharacter player, MapleCharacter partner, int marriageRingId) {
        ...
        switch (mode) {
            ...
            case 2: // Accept/Deny Proposal
                ...
                final MapleCharacter source =
                        c.getWorldServer().getPlayerStorage().getCharacterByName(name);
                ...
                if (target.getPartnerId() > 0
                        || source.getId() != id
                        || itemid <= 0
                        || !source.haveItem(itemid)
                        || source.getPartnerId() > 0
                        || !source.isAlive()
                        || !target.isAlive()) {
                    target.announce(MaplePacketCreator.enableActions());
                    return;
                ...
                if (accepted) {
                    ...
                    try {
                        ...
                        int marriageId =
                                c.getWorldServer()
                                        .createRelationship(source.getId(), target.getId());
                        source.setPartnerId(
                                target.getId()); // engage them (new marriageitemid, partnerid for
                        // both)
                        ...
                        source.announce(
                                Wedding.OnNotifyWeddingPartnerTransfer(
                                        target.getId(), target.getMapId()));
                        target.announce(
                                Wedding.OnNotifyWeddingPartnerTransfer(
                                        source.getId(), source.getMapId()));
                    } catch (Exception e) {
            ...
            case 5: // Invite %s to Wedding
                ...
                String groom = c.getPlayer().getName(),
                        bride = MapleCharacter.getNameById(c.getPlayer().getPartnerId());
                ...
                try {
                    ...
                    Pair<Boolean, Boolean> registration =
                            wserv.getMarriageQueuedLocation(marriageId);
                    ...
                    if (registration != null) {
                        if (wserv.addMarriageGuest(marriageId, guest)) {
                            ...
                            int resStatus =
                                    cserv.getWeddingReservationStatus(marriageId, cathedral);
                            if (resStatus > 0) {
                                long expiration = cserv.getWeddingTicketExpireTime(resStatus + 1);

                                MapleCharacter guestChr =
                                        c.getWorldServer()
                                                .getPlayerStorage()
                                                .getCharacterById(guest);
                                if (guestChr != null
                                        && MapleInventoryManipulator.checkSpace(
                                                guestChr.getClient(), newItemId, 1, "")
                                        && MapleInventoryManipulator.addById(
                                                guestChr.getClient(),
                                                newItemId,
                                                (short) 1,
                                                expiration)) {
                                    guestChr.dropMessage(
                                            6,
                                ...
                                c.getPlayer()
                                        .dropMessage(
                                                5,
                                                "Wedding is already under way. You cannot invite any more guests for the event.");
                            }
                        } else {
                            c.getPlayer()
                                    .dropMessage(
                                            ...
                                            '\''
                                                    + name
                                                    + "' is already invited for your marriage.");
                        }
                    } else {
                        c.getPlayer()
                                .dropMessage(
            ...
            case 6: // Open Wedding Invitation
                ...
                if (invitationid == 4031406 || invitationid == 4031407) {
                    ...
                    Pair<Integer, Integer> coupleId =
                            c.getWorldServer()
                                    .getWeddingCoupleForGuest(
                                            c.getPlayer().getId(), invitationid == 4031407);
                    if (coupleId != null) {
                        int groomId = coupleId.getLeft(), brideId = coupleId.getRight();
                        c.announce(
                                Wedding.sendWeddingInvitation(
                                        MapleCharacter.getNameById(groomId),
                                        MapleCharacter.getNameById(brideId)));
                    }
                }
