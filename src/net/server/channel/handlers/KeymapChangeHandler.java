        ...
        if (slea.available() >= 8) {
            ...
            if (mode == 0) {
                        ...
                        if (skill != null) {
                            ...
                            if (isBanndedSkill
                                    ...
                                    || (!GameConstants.isInJobTree(
                                                    skill.getId(), c.getPlayer().getJob().getId())
                                            && !c.getPlayer().isGM())) { // for those skills are are
                                // "technically" in the beginner
                                // tab, like bamboo rain in Dojo or
                                // skills you find in PYPQ
                                // AutobanFactory.PACKET_EDIT.alert(c.getPlayer(),
                                // c.getPlayer().getName() + " tried to packet edit keymapping.");
                                // FilePrinter.printError(FilePrinter.EXPLOITS +
                                // c.getPlayer().getName() + ".txt", c.getPlayer().getName() + "
                                // tried to use skill " + skill.getId() + System.lineSeparator());
                ...
                if (itemID != 0
                    ...
                    c.disconnect(
                            false,
                            false); // Don't let them send a packet with a use item they dont have.
                    return;
                }
