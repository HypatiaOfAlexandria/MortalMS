...
public final class RangedAttackHandler extends AbstractDealDamageHandler {
    ...
    public final void handlePacket(SeekableLittleEndianAccessor slea, MapleClient c) {
        ...
        if (attack.skill == Buccaneer.ENERGY_ORB
        ...
        } else if (attack.skill == ThunderBreaker.SHARK_WAVE
                && chr.getSkillLevel(ThunderBreaker.SHARK_WAVE) > 0) {
            chr.getMap()
                    .broadcastMessage(
                            chr,
                            MaplePacketCreator.rangedAttack(
            ...
            for (int i = 0; i < attack.numAttacked; i++) {
                ...
                || attack.skill == Aran.COMBO_FENRIR
            ...
            MapleWeaponType type =
                    MapleItemInformationProvider.getInstance().getWeaponType(weapon.getItemId());
            ...
            if (attack.skill != 0) {
                ...
                if (effect.getCooldown() > 0) {
                    c.announce(
                            MaplePacketCreator.skillCooldown(attack.skill, effect.getCooldown()));
                }
            ...
            for (short i = 1; i <= inv.getSlotLimit(); i++) {
                ...
                if (item != null) {
                    ...
                    if (item.getQuantity()
                            >= bulletCount) { // Fixes the bug where you can't use your last arrow.
                        if (type == MapleWeaponType.CLAW
                                && ItemConstants.isThrowingStar(id)
                                && weapon.getItemId() != 1472063) {
                            if (((id == 2070007 || id == 2070018) && chr.getLevel() < 70)
            ...
            if (projectile != 0) {
                if (!soulArrow
                    ...
                    if (effect != null && effect.getBulletConsume() != 0) {
                        bulletConsume =
                                (byte) (effect.getBulletConsume() * (hasShadowPartner ? 2 : 1));
                    }
                    ...
                    if (slot < 0)
                        ...
                        MapleInventoryManipulator.removeFromSlot(
                                c, MapleInventoryType.USE, slot, bulletConsume, false, true);
                }
            }
            ...
            if (projectile != 0
                ...
                if (attack.skill != 0) {
                    ...
                    if (effect_.getCooldown() > 0) {
                        ...
                        chr.addCooldown(
                                attack.skill, currentServerTime(), effect_.getCooldown() * 1000);
                    }
                }

                if (chr.getSkillLevel(SkillFactory.getSkill(NightWalker.VANISH)) > 0
                        && chr.getBuffedValue(MapleBuffStat.DARKSIGHT) != null
                ...
                } else if (chr.getSkillLevel(SkillFactory.getSkill(WindArcher.WIND_WALK)) > 0
                        && chr.getBuffedValue(MapleBuffStat.WIND_WALK) != null
                        && attack.numAttacked > 0) {
