...
public final class NewYearCardHandler extends AbstractMaplePacketHandler {
    ...
    public final void handlePacket(SeekableLittleEndianAccessor slea, MapleClient c) {
        ...
        if (reqMode == 0) { // card has been sent
            if (player.haveItem(2160101)) { // new year's card
                ...
                if (status == 0) {
                    if (player.canHold(4300000, 1)) {
                        String receiver = slea.readMapleAsciiString(); // [04 00 54 65 73 74] ->
                        // sReceiverName (person to send
                        // to)
                        ...
                        if (receiverid != -1) {
                            if (receiverid != c.getPlayer().getId()) {
                                String message =
                                        slea.readMapleAsciiString(); // [06 00 4C 65 74 74 65
                                // 72] -> sContent
                                // (message)

                                NewYearCardRecord newyear =
                                        new NewYearCardRecord(
                                                player.getId(),
                                                player.getName(),
                                                receiverid,
                                                receiver,
                                ...
                                player.getClient()
                                        ...
                                        .gainItem(2160101, (short) -1);
                                player.getClient()
                                        .getAbstractPlayerInteraction()
                                        .gainItem(4300000, (short) 1);

                                Server.getInstance().setNewYearCard(newyear);
                                newyear.startNewYearCardTask();
                                player.announce(
                                        MaplePacketCreator.onNewYearCardRes(
                                                player, newyear, 4, 0)); // successfully sent
                            } else {
                                player.announce(
                                        MaplePacketCreator.onNewYearCardRes(
                                                player, -1, 5, 0xF)); // cannot send to yourself
                            }
                        } else {
                            player.announce(
                                    MaplePacketCreator.onNewYearCardRes(
                                            player, -1, 5, 0x13)); // cannot find such character
                        }
                    } else {
                        player.announce(
                                MaplePacketCreator.onNewYearCardRes(
                                        player, -1, 5, 0x10)); // inventory full
                    }
                } else {
                    player.announce(
                            MaplePacketCreator.onNewYearCardRes(
                                    player, -1, 5, status)); // item and inventory errors
                }
            } else {
                player.announce(
                        MaplePacketCreator.onNewYearCardRes(
                                player, -1, 5, 0x11)); // have no card to send
            }
            ...
            if (newyear != null
                    && newyear.getReceiverId() == player.getId()
                    && !newyear.isReceiverCardReceived()) {
                if (!newyear.isSenderCardDiscarded()) {
                    if (player.canHold(4301000, 1)) {
                        ...
                        player.getClient()
                                .getAbstractPlayerInteraction()
                                .gainItem(4301000, (short) 1);
                        if (!newyear.getMessage().isEmpty())
                            player.dropMessage(
                                    6,
                                    "[NEW YEAR] "
                                            + newyear.getSenderName()
                                            + ": "
                                            + newyear.getMessage());
                        ...
                        player.announce(
                                MaplePacketCreator.onNewYearCardRes(
                                        player, newyear, 6, 0)); // successfully rcvd

                        player.getMap()
                                .broadcastMessage(
                                        MaplePacketCreator.onNewYearCardRes(
                                                player, newyear, 0xD, 0));

                        MapleCharacter sender =
                                c.getWorldServer()
                                        .getPlayerStorage()
                                        .getCharacterById(newyear.getSenderId());
                        if (sender != null && sender.isLoggedinWorld()) {
                            sender.getMap()
                                    .broadcastMessage(
                                            MaplePacketCreator.onNewYearCardRes(
                                                    sender, newyear, 0xD, 0));
                            sender.dropMessage(
                                    6,
                                    "[NEW YEAR] Your addressee successfully received the New Year card.");
                        }
                    } else {
                        player.announce(
                                MaplePacketCreator.onNewYearCardRes(
                    ...
                    player.dropMessage(
                            6,
                            "[NEW YEAR] The sender of the New Year card already dropped it. Nothing to receive.");
                }
            } else {
                if (newyear == null) {
                    player.dropMessage(
    ...
    private static int getReceiverId(String receiver, int world) {
        try (Connection con = DatabaseConnection.getConnection()) {
            try (PreparedStatement ps =
                    con.prepareStatement("SELECT id, world FROM characters WHERE name LIKE ?")) {
