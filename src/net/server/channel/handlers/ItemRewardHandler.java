...
public final class ItemRewardHandler extends AbstractMaplePacketHandler {
    ...
    public final void handlePacket(SeekableLittleEndianAccessor slea, MapleClient c) {
        ...
        if (c.getPlayer().getInventory(MapleInventoryType.USE).getItem(slot).getItemId() != itemId
                || c.getPlayer().getInventory(MapleInventoryType.USE).countById(itemId) < 1) return;
        ...
        for (RewardItem reward : rewards.getRight()) {
            ...
            if (Randomizer.nextInt(rewards.getLeft())
                    < reward.prob) { // Is it even possible to get an item with prob 1?
                ...
                if (reward.worldmsg != null) {
                    ...
                    Server.getInstance()
                            .broadcastMessage(
                                    c.getWorld(), MaplePacketCreator.serverNotice(6, msg));
                }
                break;
            }
        }
