    ...
    public final void handlePacket(SeekableLittleEndianAccessor slea, MapleClient c) {
        ...
        if (damagefrom != -3 && damagefrom != -4) {
            ...
            try {
                                        ...
                                        if (Randomizer.nextInt(101) >= loseItem.getChance()) {
                                            if (chr.haveItem(loseItem.getId())) {
                                                ...
                                                MapleInventoryManipulator.removeById(
                                                        c, type, loseItem.getId(), 1, false, false);
                                                map.spawnItemDrop(
                                                        chr,
                                                        chr,
                                                        new Item(
                                                                ...
                                                                (short) 0,
                                                                (short) 1),
                                                        map.calcDropPos(pos, chr.getPosition()),
                                                        true,
                                                        true);
                                                d++;
                                            } else {
                ...
                FilePrinter.printError(
                        ...
                        "Attacker is not a mob-type, rather is a "
                                + map.getMapObject(oid).getClass().getName()
                                + " entity.");
        ...
        if (damagefrom != -1 && damagefrom != -2 && attacker != null) {
            ...
            if (attackInfo != null) {
                ...
                if (chr.getBuffedValue(MapleBuffStat.MANA_REFLECTION) != null
                        && damage > 0
                        && !attacker.isBoss()) {
                    ...
                    if (jobid == 212 || jobid == 222 || jobid == 232) {
                        ...
                        assert manaReflectSkill != null;
                        if (chr.isBuffFrom(MapleBuffStat.MANA_REFLECTION, manaReflectSkill)
                                && chr.getSkillLevel(manaReflectSkill) > 0
                                && manaReflectSkill
                                        .getEffect(chr.getSkillLevel(manaReflectSkill))
                                        .makeChanceResult()) {
                            int bouncedamage =
                                    (damage
                                            * manaReflectSkill
                            ...
                            map.broadcastMessage(
                                    chr, MaplePacketCreator.damageMonster(oid, bouncedamage), true);
                            chr.getClient().announce(MaplePacketCreator.showOwnBuffEffect(id, 5));
                            map.broadcastMessage(
                                    chr,
                                    MaplePacketCreator.showBuffeffect(chr.getId(), id, 5),
                                    false);
                        }
                    }
                }
        ...
        if (damage > 0 && !chr.isHidden()) {
            if (attacker != null
                    ...
                    && chr.getBuffedValue(MapleBuffStat.POWERGUARD)
                            != null) { // PG works on bosses, but only at half of the rate.
                int bouncedamage =
                        (int)
                                (damage
                                        * (chr.getBuffedValue(MapleBuffStat.POWERGUARD)
                                                        .doubleValue()
                                                / (attacker.isBoss() ? 200 : 100)));
                bouncedamage = Math.min(bouncedamage, attacker.getMaxHp() / 10);
                damage -= bouncedamage;
                map.damageMonster(chr, attacker, bouncedamage);
                map.broadcastMessage(
                        chr, MaplePacketCreator.damageMonster(oid, bouncedamage), false, true);
                chr.checkMonsterAggro(attacker);
            }
            if (attacker != null
                    && damagefrom == -1
                    && chr.getBuffedValue(MapleBuffStat.BODY_PRESSURE) != null) {
                var body_pressure = SkillFactory.getSkill(Aran.BODY_PRESSURE);
                assert body_pressure != null;
                final MapleStatEffect eff =
                        body_pressure.getEffect(chr.getSkillLevel(body_pressure));
                    ...
                    if (!attacker.isBoss() && eff.makeChanceResult()) {
                        attacker.applyStatus(
                                ...
                                new MonsterStatusEffect(
                                        Collections.singletonMap(MonsterStatus.NEUTRALISE, 1),
                                        body_pressure,
                                        null,
                                        false),
                                false,
                                (eff.getDuration() / 10) * 2,
                                false);
                    }
                }
            }
            ...
            if (chr.getBuffedValue(MapleBuffStat.MAGIC_GUARD) != null && mpattack == 0) {
                ...
                damage /= 2;
                int mesoloss = (int) (damage * (mesoguard.doubleValue() / 100.0d));
        ...
        for (var player :
                banishPlayers) { // chill, if this list ever gets non-empty an attacker does exist,
            // trust me :)
            player.changeMapBanish(
                    attacker.getBanish().getMap(),
                    attacker.getBanish().getPortal(),
                    attacker.getBanish().getMsg());
        }
    }
}
