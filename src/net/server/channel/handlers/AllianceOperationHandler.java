...
public final class AllianceOperationHandler extends AbstractMaplePacketHandler {
    ...
    public final void handlePacket(SeekableLittleEndianAccessor slea, MapleClient c) {
        ...
        if (alliance == null) {
            ...
            if (chr.getMGC().getAllianceRank() > 2
                    || !alliance.getGuilds().contains(chr.getGuildId())) {
                c.announce(MaplePacketCreator.enableActions());
                return;
            }
        }
        ...
        switch (b) {
            case 0x01:
                Server.getInstance()
                        .allianceMessage(
                                alliance.getId(),
                                sendShowInfo(chr.getGuild().getAllianceId(), chr.getId()),
                                -1,
                                -1);
                break;
            case 0x02:
                ...
                MapleAlliance.removeGuildFromAlliance(
            ...
            case 0x03: // Send Invite
                ...
                if (alliance.getGuilds().size() == alliance.getCapacity()) {
                    ...
                    if (mg == null) {
                        ...
                        if (victim == null) {
                            chr.dropMessage(
                                    "The master of the guild that you offered an invitation is currently not online.");
                        } else {
                            victim.getClient()
                                    .announce(
                                            MaplePacketCreator.sendAllianceInvitation(
                                                    alliance.getId(), chr));
                        }
                    }
                }
            ...
            case 0x04:
                { // Accept Invite
                    ...
                    Server.getInstance()
                            .allianceMessage(
                                    alliance.getId(),
                                    MaplePacketCreator.addGuildToAlliance(alliance, guildid, c),
                                    -1,
                                    -1);
                    Server.getInstance()
                            .allianceMessage(
                                    alliance.getId(),
                                    MaplePacketCreator.updateAllianceInfo(alliance, c),
                                    -1,
                                    -1);
                    Server.getInstance()
                            .allianceMessage(
                                    ...
                                    MaplePacketCreator.allianceNotice(
                                            alliance.getId(), alliance.getNotice()),
                                    -1,
                                    -1);
                    chr.getGuild()
                            .dropMessage(
                                    "Your guild has joined the ["
                                            + alliance.getName()
                                            + "] union.");
            ...
            case 0x06:
                ...
                Server.getInstance()
                        .allianceMessage(
                                alliance.getId(),
                                MaplePacketCreator.removeGuildFromAlliance(
                                        alliance, guildid, c.getWorld()),
                                -1,
                                -1);
                ...
                Server.getInstance()
                        .allianceMessage(
                                alliance.getId(),
                                MaplePacketCreator.getGuildAlliances(alliance, c.getWorld()),
                                -1,
                                -1);
                Server.getInstance()
                        .allianceMessage(
                                ...
                                -1);
                Server.getInstance()
                        .guildMessage(guildid, MaplePacketCreator.disbandAlliance(allianceid));

                alliance.dropMessage(
                        '['
                                + Server.getInstance().getGuild(guildid).getName()
            ...
            case 0x07:
                { // Change Alliance Leader
                    ...
                    // sendChangeLeader(chr.getGuild().getAllianceId(), chr.getId(),
                    // slea.readInt()), -1, -1);
            ...
            case 0x08:
                ...
                Server.getInstance()
                        .allianceMessage(
                                alliance.getId(),
                                MaplePacketCreator.changeAllianceRankTitle(alliance.getId(), ranks),
                                -1,
                                -1);
                break;
            case 0x09:
                ...
                // Server.getInstance().allianceMessage(alliance.getId(),
                // sendChangeRank(chr.getGuild().getAllianceId(), chr.getId(), int1, byte1), -1,
                // -1);
                MapleCharacter player =
                        Server.getInstance()
                                .getWorld(c.getWorld())
                                .getPlayerStorage()
                                .getCharacterById(int1);
    ...
    private static void changeLeaderAllianceRank(MapleAlliance alliance, MapleCharacter newLeader) {
        MapleGuildCharacter lmgc = alliance.getLeader();
        MapleCharacter leader =
                newLeader.getWorldServer().getPlayerStorage().getCharacterById(lmgc.getId());
        ...
        Server.getInstance()
                .allianceMessage(
                        alliance.getId(),
                        MaplePacketCreator.getGuildAlliances(alliance, newLeader.getWorld()),
                        -1,
                        -1);
        alliance.dropMessage(
                '\''
                        + newLeader.getName()
                        + "' has been appointed as the new head of this Alliance.");
    }

    private static void changePlayerAllianceRank(
            MapleAlliance alliance, MapleCharacter chr, boolean raise) {
        ...
        Server.getInstance()
                .allianceMessage(
                        alliance.getId(),
                        MaplePacketCreator.getGuildAlliances(alliance, chr.getWorld()),
                        -1,
                        -1);
        alliance.dropMessage(
                '\''
