        ...
        if (action == 0x03 || action == 0x1E) {
            ...
            if (!canBuy(cItem, cs.getCash(useNX))) {
                FilePrinter.printError(
                        FilePrinter.ITEM, "Denied to sell cash item with SN " + cItem.getSN());
                c.announce(MaplePacketCreator.enableActions());
                return;
            }
            ...
            Map<String, String> recipient =
                    MapleCharacter.getCharacterFromDatabase(slea.readMapleAsciiString());
            ...
            try {
                chr.sendNote(
                        recipient.get("name"),
                        chr.getName() + " has sent you a gift! Go check out the Cash Shop.",
                        (byte) 0); // fame or not
            } catch (SQLException ex) {
                ex.printStackTrace();
            }
            MapleCharacter receiver =
                    c.getChannelServer()
                            .getPlayerStorage()
            ...
            if (mode == 0) {
                ...
                if (chr.gainSlots(type, 4, false)) {
                    c.announce(
                            MaplePacketCreator.showBoughtInventorySlots(type, chr.getSlots(type)));
            ...
            if (mode == 0) {
                ...
                if (chr.getStorage().gainSlots(4)) {
                    FilePrinter.print(
                            FilePrinter.STORAGE + c.getAccountName() + ".txt",
                            c.getPlayer().getName()
                                    + " bought 4 slots to their account storage."
                                    + System.lineSeparator());
                    chr.setUsedStorage();

                    c.announce(
                            MaplePacketCreator.showBoughtStorageSlots(chr.getStorage().getSlots()));
                    cs.gainCash(cash, -4000);
                    c.announce(MaplePacketCreator.showCash(chr));
                }
            } else {
            ...
            if (c.getPlayer().getPetIndex(item.getPetId()) > -1) {
                chr.getClient()
                        .announce(
            ...
            if (checkBirthday(c, birthday)) {
                ...
                MapleCharacter partner =
                        c.getChannelServer().getPlayerStorage().getCharacterByName(recipientName);
                if (partner == null) {
                    chr.getClient()
                            .announce(
                                    MaplePacketCreator.serverNotice(
            ...
            if (checkBirthday(c, birthday)) {
                ...
                slea.readByte();
                MapleCharacter partner =
                        c.getChannelServer().getPlayerStorage().getCharacterByName(sentTo);
                if (partner == null) {
                    chr.dropMessage(
                            "The partner you specified cannot be found."
                                    + System.lineSeparator()
