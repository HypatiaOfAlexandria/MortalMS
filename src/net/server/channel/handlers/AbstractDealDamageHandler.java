...
public abstract class AbstractDealDamageHandler extends AbstractMaplePacketHandler {
    ...
    public static class AttackInfo {
        ...
        public MapleStatEffect getAttackEffect(MapleCharacter chr, Skill theSkill) {
            ...
            if (skillLevel == 0
                    && GameConstants.isPqSkillMap(chr.getMapId())
                    && GameConstants.isPqSkill(mySkill.getId())) skillLevel = 1;
            ...
            if (display > 80) { // Hmm
                if (!mySkill.getAction()) {
                    AutobanFactory.FAST_ATTACK.autoban(
                            chr, "WZ Edit; adding action to a skill: " + display);
                    return null;
                }
            }
    ...
    protected synchronized void applyAttack(
            AttackInfo attack, final MapleCharacter player, int attackCount) {
        Skill theSkill = null;
        ...
        try {
            ...
            if (attack.skill != 0) {
                theSkill =
                        SkillFactory.getSkill(
                                GameConstants.getHiddenSkill(
                                        attack.skill)); // returns back the skill id if its not a
                // hidden skill so we are gucci
                ...
                if (attack.skill != Cleric.HEAL) {
                    if (player.isAlive()) {
                        if (attack.skill == NightWalker.POISON_BOMB) { // Poison Bomb
                            attackEffect.applyTo(
                                    player, new Point(attack.position.x, attack.position.y));
            ...
            for (Map.Entry<Integer, List<Integer>> integerListEntry : attack.allDamage.entrySet()) {
                final MapleMonster monster =
                        map.getMonsterByOid((integerListEntry.getKey()).intValue());
                if (monster != null) {
                    ...
                    else if (attack.skill == Bishop.GENESIS
                            || attack.skill == ILArchMage.BLIZZARD
                            || attack.skill == FPArchMage.METEOR_SHOWER) distanceToDetect += 275000;
                    else if (attack.skill == Hero.BRANDISH
                            || attack.skill == DragonKnight.SPEAR_CRUSHER
                            || attack.skill == DragonKnight.POLE_ARM_CRUSHER)
                        distanceToDetect += 40000;
                    else if (attack.skill == DragonKnight.DRAGON_ROAR
                    ...
                    if (distance > distanceToDetect) {
                        AutobanFactory.DISTANCE_HACK.alert(
                                player,
                                "Distance Sq to monster: "
                                        + distance
                                        + " SID: "
                                        + attack.skill
                                        + " MID: "
                    ...
                    if (player.getBuffedValue(MapleBuffStat.PICKPOCKET) != null
                        ...
                        int picklv =
                                (player.isGM())
                                        ? pickpocket.getMaxLevel()
                                        : player.getSkillLevel(pickpocket);
                        if (picklv > 0) {
                            int delay = 0;
                            final int maxmeso =
                                    player.getBuffedValue(MapleBuffStat.PICKPOCKET).intValue();
                            for (Integer eachd : onedList) {
                                eachd += Integer.MAX_VALUE;

                                if (pickpocket.getEffect(picklv).makeChanceResult()) {
                                    final int eachdf;
                                    eachdf = eachd < 0 ? eachd + Integer.MAX_VALUE : eachd;
                        ...
                        if (monster.getStolen().size() < 1) { // One steal per mob <3
                            if (steal.getEffect(player.getSkillLevel(steal)).makeChanceResult()) {
                                MapleMonsterInformationProvider mi =
                        ...
                        if (player.getBuffedValue(MapleBuffStat.WK_CHARGE) != null) {
                            ...
                            if (totDamageToOneMonster > 0) {
                                ...
                                monster.applyStatus(
                                        ...
                                        false,
                                        snowCharge
                                                        .getEffect(player.getSkillLevel(snowCharge))
                                                        .getY()
                                                * 1000);
                            }
                        }
                    ...
                    if (player.getBuffedValue(MapleBuffStat.HAMSTRING) != null) {
                        ...
                        if (hamstring
                            ...
                            monster.applyStatus(
                                    ...
                                    monsterStatusEffect,
                                    false,
                                    hamstring.getEffect(player.getSkillLevel(hamstring)).getY()
                                            * 1000);
                        }
                    }
                        ...
                        if (slow.getEffect(player.getSkillLevel(slow)).makeChanceResult()) {
                            MonsterStatusEffect monsterStatusEffect =
                                    new MonsterStatusEffect(
                                            Collections.singletonMap(
                                                    MonsterStatus.SPEED,
                                                    slow.getEffect(player.getSkillLevel(slow))
                                                            .getX()),
                                            slow,
                                            null,
                                            false);
                            monster.applyStatus(
                                    ...
                                    monsterStatusEffect,
                                    false,
                                    slow.getEffect(player.getSkillLevel(slow)).getY() * 60 * 1000);
                        }
                    }
                    if (player.getBuffedValue(MapleBuffStat.BLIND) != null) {
                        ...
                        if (blind.getEffect(player.getSkillLevel(blind)).makeChanceResult()) {
                            ...
                            monster.applyStatus(
                                    ...
                                    monsterStatusEffect,
                                    false,
                                    blind.getEffect(player.getSkillLevel(blind)).getY() * 1000);
                        }
                    }
                    if (job == 121 || job == 122) {
                        for (int charge = 1211005; charge < 1211007; charge++) {
                            ...
                            if (player.isBuffFrom(MapleBuffStat.WK_CHARGE, chargeSkill)) {
                                if (totDamageToOneMonster > 0) {
                                    if (charge == WhiteKnight.BW_ICE_CHARGE
                                            || charge == WhiteKnight.SWORD_ICE_CHARGE) {
                                        monster.setTempEffectiveness(
                                                Element.ICE,
                                                ElementalEffectiveness.WEAK,
                                                chargeSkill
                        ...
                        Skill type =
                                SkillFactory.getSkill(
                                        player.getJob().getId() == 412
                                                ? 4120005
                                                : (player.getJob().getId() == 1411
                                                        ? 14110004
                                                        : 4220005));
                        if (player.getSkillLevel(type) > 0) {
                            MapleStatEffect venomEffect =
                                    type.getEffect(player.getSkillLevel(type));
                        ...
                        if (!monster.isBoss()) {
                            ...
                            mortalBlow =
                                    job == 311 || job == 312
                                            ? SkillFactory.getSkill(Ranger.MORTAL_BLOW)
                                            : SkillFactory.getSkill(Sniper.MORTAL_BLOW);
                            if (player.getSkillLevel(mortalBlow) > 0) {
                                MapleStatEffect mortal =
                                        mortalBlow.getEffect(player.getSkillLevel(mortalBlow));
                                if (monster.getHp()
                                        <= (monster.getStats().getHp() * mortal.getX()) / 100) {
                    ...
                    if (attack.skill != 0) {
                        if (attackEffect.getFixDamage() != -1) {
                            if (totDamageToOneMonster != attackEffect.getFixDamage()
                                ...
                                AutobanFactory.FIX_DAMAGE.autoban(
                                        player, totDamageToOneMonster + " damage");
                            }
                            ...
                            if (attack.skill == threeSnailsId) {
                                if (ServerConstants.USE_ULTRA_THREE_SNAILS) {
                                    ...
                                    if (skillLv > 0) {
                                        AbstractPlayerInteraction api =
                                                player.getClient().getAbstractPlayerInteraction();
                    ...
                    if (totDamageToOneMonster > 0 && attackEffect != null) {
                        Map<MonsterStatus, Integer> attackEffectStati =
                        ...
                        if (!attackEffectStati.isEmpty()) {
                            if (attackEffect.makeChanceResult()) {
                                monster.applyStatus(
                                        player,
                                        new MonsterStatusEffect(
                                                attackEffectStati, theSkill, null, false),
                    ...
                    if (attack.skill == Paladin.HEAVENS_HAMMER) {
                        if (!monster.isBoss()) {
                            damageMonsterWithSkill(
                                    player, map, monster, monster.getHp() - 1, attack.skill, 1777);
                        } else {
                            int HHDmg =
                        ...
                        if (!monster.isBoss()) {
                            damageMonsterWithSkill(
                                    player, map, monster, monster.getHp(), attack.skill, 0);
                        } else {
                            int TmpDmg =
                                    (player.calculateMaxBaseDamage(player.getTotalWatk())
                        ...
                        if (attack.skill == Aran.BODY_PRESSURE) {
                            map.broadcastMessage(
                                    MaplePacketCreator.damageMonster(
                                            monster.getObjectId(), totDamageToOneMonster));
                        }
    ...
    private static void damageMonsterWithSkill(
        ...
        animationTime =
                fixedTime == 0 ? SkillFactory.getSkill(skillid).getAnimationTime() : fixedTime;
        ...
        if (animationTime > 0) { // be sure to only use LIMITED ATTACKS with animation time here
            ...
            map.broadcastMessage(
                    MaplePacketCreator.damageMonster(monster.getObjectId(), damage),
                    monster.getPosition());
            map.damageMonster(attacker, monster, damage);
        }
    }

    protected static AttackInfo parseDamage(
            LittleEndianAccessor lea, MapleCharacter chr, boolean ranged, boolean magic) {
        ...
        if (ret.skill > 0) {
            ...
            if (ret.skilllevel == 0
                    && GameConstants.isPqSkillMap(chr.getMapId())
                    && GameConstants.isPqSkill(ret.skill)) ret.skilllevel = 1;
        }
        ...
        if (magic && ret.skill != 0) {
            calcDmgMax =
                    (chr.getTotalMagic() * chr.getTotalMagic() / 1000 + chr.getTotalMagic()) / 30
                            + chr.getTotalInt() / 200;
        } else if (ret.skill == 4001344
                || ret.skill == NightWalker.LUCKY_SEVEN
            ...
            calcDmgMax =
                    (int) (18.5 * (chr.getTotalStr() + chr.getTotalLuk()) + chr.getTotalDex() * 2)
                            / 100
            ...
            if (magic) {
                ...
                if (chr.getJob() == MapleJob.IL_ARCHMAGE || chr.getJob() == MapleJob.IL_MAGE) {
                    ...
                    if (skillLvl > 0)
                        ...
                        || chr.getJob() == MapleJob.FP_MAGE) {
                    int skillLvl = chr.getSkillLevel(FPMage.ELEMENT_AMPLIFICATION);
                    if (skillLvl > 0)
                        calcDmgMax =
                                calcDmgMax
                                        * SkillFactory.getSkill(FPMage.ELEMENT_AMPLIFICATION)
                                                .getEffect(skillLvl)
                                                .getY()
                                        / 100;
                } else if (chr.getJob() == MapleJob.BLAZEWIZARD3
                        || chr.getJob() == MapleJob.BLAZEWIZARD4) {
                    int skillLvl = chr.getSkillLevel(BlazeWizard.ELEMENT_AMPLIFICATION);
                    if (skillLvl > 0)
                        calcDmgMax =
                                calcDmgMax
                                        * SkillFactory.getSkill(BlazeWizard.ELEMENT_AMPLIFICATION)
                                                .getEffect(skillLvl)
                                                .getY()
                                        / 100;
                } else if (chr.getJob() == MapleJob.EVAN7
                        || chr.getJob() == MapleJob.EVAN8
                        || chr.getJob() == MapleJob.EVAN9
                        || chr.getJob() == MapleJob.EVAN10) {
                    int skillLvl = chr.getSkillLevel(Evan.MAGIC_AMPLIFICATION);
                    if (skillLvl > 0)
                        calcDmgMax =
                                calcDmgMax
                                        * SkillFactory.getSkill(Evan.MAGIC_AMPLIFICATION)
                                                .getEffect(skillLvl)
                                                .getY()
                                        / 100;
                }
                ...
                if (ret.skill == Cleric.HEAL) {
                    ...
                    calcDmgMax =
                            (int)
                                    Math.round(
                                            (chr.getTotalInt() * 4.8 + chr.getTotalLuk() * 4)
                                                    * chr.getTotalMagic()
                                                    / 1000);
        ...
        if (comboBuff != null && comboBuff > 0) {
            ...
            if (comboBuff > 6) {
                ...
                MapleStatEffect ceffect =
                        SkillFactory.getSkill(advcomboid).getEffect(chr.getSkillLevel(advcomboid));
                calcDmgMax =
                        (int)
                                Math.floor(
                                        (double) (calcDmgMax * (ceffect.getDamage() + 50) / 100)
                                                + 0.20
                                                + (double) (comboBuff - 5) * 0.04);
            } else {
                // Normal Combo
                ...
                if (skillLv > 0) {
                    ...
                    calcDmgMax =
                            calcDmgMax * (ceffect.getDamage() + 50) / 100
                                    + (comboBuff - 1) * (skillLv / 6) / 100;
                }
            }
            ...
            if (GameConstants.isFinisherSkill(ret.skill)) {
                ...
                if (orbs == 2) calcDmgMax *= 1.2;
                else if (orbs == 3) calcDmgMax *= 1.54;
                else if (orbs == 4) calcDmgMax *= 2;
                else if (orbs >= 5) calcDmgMax *= 2.5;
            }
        }

        if (chr.getEnergyBar() == 15000) {
            int energycharge =
                    chr.isCygnus() ? ThunderBreaker.ENERGY_CHARGE : Marauder.ENERGY_CHARGE;
            MapleStatEffect ceffect =
                    SkillFactory.getSkill(energycharge).getEffect(chr.getSkillLevel(energycharge));
        ...
        if (chr.getBuffEffect(MapleBuffStat.SHARP_EYES) != null) {
            // Any class that has sharp eyes can crit. Also, since it stacks with normal crit go
            // ahead
            // and calc it in.
        ...
        for (int i = 0; i < ret.numAttacked; i++) {
            ...
            if (chr.getBuffEffect(MapleBuffStat.WK_CHARGE) != null) {
                ...
                if (monster != null) {
                    if (sourceID == WhiteKnight.BW_FIRE_CHARGE
                            || sourceID == WhiteKnight.SWORD_FIRE_CHARGE) {
                        if (monster.getStats().getEffectiveness(Element.FIRE)
                                == ElementalEffectiveness.WEAK) {
                            calcDmgMax *= 1.05 + level * 0.015;
                        }
                    } else if (sourceID == WhiteKnight.BW_ICE_CHARGE
                            || sourceID == WhiteKnight.SWORD_ICE_CHARGE) {
                        if (monster.getStats().getEffectiveness(Element.ICE)
                                == ElementalEffectiveness.WEAK) {
                            calcDmgMax *= 1.05 + level * 0.015;
                        }
                    } else if (sourceID == WhiteKnight.BW_LIT_CHARGE
                            || sourceID == WhiteKnight.SWORD_LIT_CHARGE) {
                        if (monster.getStats().getEffectiveness(Element.LIGHTING)
                                == ElementalEffectiveness.WEAK) {
                            calcDmgMax *= 1.05 + level * 0.015;
                        }
                    } else if (sourceID == Paladin.BW_HOLY_CHARGE
                            || sourceID == Paladin.SWORD_HOLY_CHARGE) {
                        if (monster.getStats().getEffectiveness(Element.HOLY)
                                == ElementalEffectiveness.WEAK) {
                            calcDmgMax *= 1.2 + level * 0.015;
                        }
                    }
                } else {
                    // Since we already know the skill has an elemental attribute, but we dont know
                    // if the monster is weak or not, lets
            ...
            if (ret.skill != 0) {
                ...
                if (skill.getElement() != Element.NEUTRAL
                        && chr.getBuffedValue(MapleBuffStat.ELEMENTAL_RESET) == null) {
                    // The skill has an element effect, so we need to factor that in.
                    if (monster != null) {
                        ElementalEffectiveness eff =
                                monster.getElementalEffectiveness(skill.getElement());
                        ...
                        // Since we already know the skill has an elemental attribute, but we dont
                        // know if the monster is weak or not, lets
                        // take the safe approach and just assume they are weak.
                        calcDmgMax *= 1.5;
                    }
                }
                if (ret.skill == FPWizard.POISON_BREATH
                        || ret.skill == FPMage.POISON_MIST
                        || ret.skill == FPArchMage.FIRE_DEMON
                        || ret.skill == ILArchMage.ICE_DEMON) {
                    if (monster != null) {
                        // Turns out poison is completely server side, so I don't know why I added
                        // this. >.<
            ...
            for (int j = 0; j < ret.numDamage; j++) {
                ...
                if (ret.skill == Buccaneer.BARRAGE) {
                    if (j > 3) hitDmgMax *= Math.pow(2, (j - 3));
                }
                if (shadowPartner) {
                    // For shadow partner, the second half of the hits only do 50% damage. So calc
                    // that
                ...
                if (damage > maxWithCrit * 5) {
                    AutobanFactory.DAMAGE_HACK.addPoint(
