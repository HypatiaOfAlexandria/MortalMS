...
public class BuddylistModifyHandler extends AbstractMaplePacketHandler {
    ...
    public void handlePacket(SeekableLittleEndianAccessor slea, MapleClient c) {
        ...
        if (mode == 1) { // add
            ...
            if (ble != null && !ble.isVisible() && group.equals(ble.getGroup())) {
                c.announce(
                        MaplePacketCreator.serverNotice(
                                1, "You already have \"" + ble.getName() + "\" on your Buddylist"));
            ...
            if (!buddylist.isFull()) {
                try {
                    int channel =
                            c.getWorldServer().find(otherCid); // worldInterface.find(otherCid);
                    ...
                    if (otherName != null) {
                        buddylist.put(
                                new BuddylistEntry(
                                        otherName, "Default Group", otherCid, channel, true));
                        c.announce(MaplePacketCreator.updateBuddylist(buddylist.getBuddies()));
                        notifyRemoteChannel(c, channel, otherCid, ADDED);
                    }
                } catch (SQLException e) {
    ...
    private static void notifyRemoteChannel(
        ...
        if (remoteChannel != -1) {
            c.getWorldServer()
                    .buddyChanged(
                            otherCid, player.getId(), player.getName(), c.getChannel(), operation);
        }
    }
}
