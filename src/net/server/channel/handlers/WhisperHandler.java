        ...
        if (mode == 6) { // whisper
            ...
            if (text.length() > Byte.MAX_VALUE && !player.isGM()) {
                AutobanFactory.PACKET_EDIT.alert(
                        c.getPlayer(),
                        c.getPlayer().getName() + " tried to packet edit with whispers.");
                FilePrinter.printError(
                        FilePrinter.EXPLOITS + c.getPlayer().getName() + ".txt",
                        c.getPlayer().getName()
                                + " tried to send text with length of "
                                + text.length()
            ...
            if (player != null) {
                ...
                if (world.isConnected(recipient)) {
                    ...
                    if (player.isHidden() && player.gmLevel() >= c.getPlayer().gmLevel())
                        c.announce(MaplePacketCreator.getWhisperReply(recipient, (byte) 0));
                    else c.announce(MaplePacketCreator.getWhisperReply(recipient, (byte) 1));
                } else {
                    c.announce(MaplePacketCreator.getWhisperReply(recipient, (byte) 0));
                }
            }
            ...
            if (victim != null && c.getPlayer().gmLevel() >= victim.gmLevel()) {
                ...
                try {
                    ...
                    PreparedStatement ps =
                            con.prepareStatement("SELECT gm FROM characters WHERE name = ?");
