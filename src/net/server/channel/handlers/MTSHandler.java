...
public final class MTSHandler extends AbstractMaplePacketHandler {
            ...
            if (op == 2) { // put item up for sale
                ...
                if (i != null && c.getPlayer().getMeso() >= 5000) {
                    ...
                    try {
                        ...
                        PreparedStatement ps =
                                con.prepareStatement(
                                        "SELECT COUNT(*) FROM mts_items WHERE seller = ?");
                        ps.setInt(1, c.getPlayer().getId());
                        ResultSet rs = ps.executeQuery();
                        if (rs.next()) {
                            if (rs.getInt(1)
                                    > 10) { // They have more than 10 items up for sale already!
                                c.getPlayer()
                                        .dropMessage(
                                                1, "You already have 10 items up for auction!");
                                c.announce(getMTS(1, 0, 0));
                                c.announce(
                                        MaplePacketCreator.transferInventory(
                                                getTransfer(c.getPlayer().getId())));
                                c.announce(
                                        MaplePacketCreator.notYetSoldInv(
                    ...
                    c.announce(
                            MaplePacketCreator.transferInventory(
                                    getTransfer(c.getPlayer().getId())));
                    c.announce(
                            MaplePacketCreator.notYetSoldInv(getNotYetSold(c.getPlayer().getId())));
                }
                ...
                if (tab == 4 && type == 0) {
                    ...
                    c.announce(
                            getMTSSearch(
                                    tab,
                                    type,
                                    c.getPlayer().getCurrentCI(),
                                    c.getPlayer().getSearch(),
                                    page));
                } else {
                ...
                try {
                    ...
                    if (rs.next()) {
                        ...
                        if (rs.getInt("type") != 1) {
                            ...
                            Equip equip =
                                    new Equip(
                                            rs.getInt("itemid"), (byte) rs.getInt("position"), -1);
                        ...
                        c.announce(
                                getMTS(
                                        ...
                                        c.getPlayer().getCurrentPage()));
                        c.announce(
                                MaplePacketCreator.MTSConfirmTransfer(
                ...
                try {
                    ...
                    try (PreparedStatement ps1 =
                            con.prepareStatement(
                                    "SELECT id FROM mts_items WHERE id = ? AND seller <> ?")) {
                        ...
                        try (ResultSet rs1 = ps1.executeQuery()) {
                            if (rs1.next()) {
                                PreparedStatement ps =
                                        con.prepareStatement(
                                                "SELECT cid FROM mts_cart WHERE cid = ? AND itemid = ?");
                                ps.setInt(1, c.getPlayer().getId());
                                ps.setInt(2, id);
    ...
    public static List<MTSItemInfo> getNotYetSold(int cid) {
    ...
    public static byte[] getCart(int cid) {
        List<MTSItemInfo> items = new ArrayList<>();
        Connection con = null;
        PreparedStatement ps;
        ResultSet rs;
        ...
        try {
            ...
            while (rs.next()) {
                try (PreparedStatement pse =
                        con.prepareStatement("SELECT * FROM mts_items WHERE id = ?")) {
    ...
    public static List<MTSItemInfo> getTransfer(int cid) {
        List<MTSItemInfo> items = new ArrayList<>();
        Connection con = null;
        PreparedStatement ps;
        ResultSet rs;
        try {
            ...
            while (rs.next()) {
                if (rs.getInt("type") != 1) {
                    Item i =
                            new Item(rs.getInt("itemid"), (short) 0, (short) rs.getInt("quantity"));
                    i.setOwner(rs.getString("owner"));
                    items.add(
                            new MTSItemInfo(
                                    i,
    ...
    public static byte[] getMTSSearch(int tab, int type, int cOi, String search, int page) {
        List<MTSItemInfo> items = new ArrayList<>();
        ...
        if (cOi != 0) {
            ...
            if (retItems != null && !retItems.isEmpty()) {
                for (String singleRetItem : retItems) {
                    listaitems.append(singleRetItem);
                }
                listaitems.append(" itemid=0 )");
            }
        } else {
            listaitems =
                    new StringBuilder(" AND sellername LIKE CONCAT('%','" + search + "', '%')");
        }
        Connection con = null;
        PreparedStatement ps;
        ResultSet rs;
