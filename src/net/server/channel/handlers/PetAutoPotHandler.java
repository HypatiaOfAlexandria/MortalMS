...
public final class PetAutoPotHandler extends AbstractMaplePacketHandler {
            ...
            if (toUse != null) {
                ...
                if (hasHpGain) {
                    qtyCount =
                            (int)
                                    Math.ceil(
                                            ((ServerConstants.PET_AUTOHP_RATIO * maxHp) - curHp)
                                                    / incHp);
                }
                ...
                if (hasMpGain) {
                    qtyCount =
                            Math.max(
                                    ...
                                    (int)
                                            Math.ceil(
                                                    ((ServerConstants.PET_AUTOMP_RATIO * maxMp)
                                                                    - curMp)
                                                            / incMp));
                }
                ...
                while (true) {
                    ...
                    MapleInventoryManipulator.removeFromSlot(
                            c, MapleInventoryType.USE, slot, qtyToUse, false);
    ...
    private boolean cursorOnNextAvailablePot(MapleCharacter chr) {
        if (toUseList == null) {
            toUseList = chr.getInventory(MapleInventoryType.USE).ArrayListById(itemId);
        }
