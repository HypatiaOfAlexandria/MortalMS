    ...
    public final void handlePacket(SeekableLittleEndianAccessor slea, MapleClient c) {
        ...
        if (mode == Action.CREATE.getCode()) {
                ...
                if (ServerConstants.USE_ERASE_PERMIT_ON_OPENSHOP) {
                    try {
                        MapleInventoryManipulator.removeById(
                                c, MapleInventoryType.CASH, shop.getItemId(), 1, true, false);
            ...
            if (game.getGameType().equals("matchcard")) {
                ...
                chr.getMap()
                        .broadcastMessage(
                                MaplePacketCreator.addMatchCardBox(game.getOwner(), 2, 1));
            }
            ...
            if (game.isOwner(chr)) {
                ...
                game.getOwner()
                        .getClient()
                        .announce(MaplePacketCreator.getMiniGameRequestTie(game));
            }
            ...
            int type = slea.readByte(); // piece ( 1 or 2; Owner has one piece, visitor has
            // another, it switches every game.)
            ...
            if (turn == 1) {
                ...
                if (game.isOwner(chr)) {
                    game.broadcast(
                            MaplePacketCreator.getMatchCardSelect(game, turn, slot, firstslot, 2));
                    game.setOwnerPoints();
                } else {
                    game.broadcast(
                            MaplePacketCreator.getMatchCardSelect(game, turn, slot, firstslot, 3));
                    game.setVisitorPoints();
                }
            } else if (game.isOwner(chr)) {
                game.broadcast(
                        MaplePacketCreator.getMatchCardSelect(game, turn, slot, firstslot, 0));
            } else {
                game.broadcast(
                        MaplePacketCreator.getMatchCardSelect(game, turn, slot, firstslot, 1));
            }
            ...
            if (quantity < 1 || quantity > item.getQuantity()) {
                c.announce(
                        MaplePacketCreator.serverNotice(
                                1, "You don't have enough quantity of the item."));
            ...
            if (chr.getTrade() != null) {
                if ((quantity <= item.getQuantity() && quantity >= 0)
                    ...
                    if (ii.isDropRestricted(
                            item.getItemId())) { // ensure that undroppable items do not make it
                        // to the trade window
                        if (!MapleKarmaManipulator.hasKarmaFlag(item)) {
                            c.announce(
                                    MaplePacketCreator.serverNotice(
                                            1, "That item is untradeable."));
                            c.announce(MaplePacketCreator.enableActions());
                            return;
                        }
                    }
                    ...
                    if (ItemConstants.isRechargeable(item.getItemId())) {
                        ...
                        MapleInventoryManipulator.removeFromSlot(
                                c, ivType, item.getPosition(), item.getQuantity(), true);
                    } else {
                        tradeItem.setQuantity(quantity);
                        MapleInventoryManipulator.removeFromSlot(
                                c, ivType, item.getPosition(), quantity, true);
                    }
            ...
            if (ivItem == null || ivItem.getFlag() == ItemConstants.UNTRADEABLE) {
                c.announce(
                        MaplePacketCreator.serverNotice(
                                1, "Could not perform shop operation with that item."));
                c.announce(MaplePacketCreator.enableActions());
                return;
            }
            ...
            if (perBundle <= 0
                ...
                AutobanFactory.PACKET_EDIT.alert(
                        chr, chr.getName() + " tried to packet edit with hired merchants.");
            ...
            if (ServerConstants.USE_ENFORCE_UNMERCHABLE_CASH
                    && MapleItemInformationProvider.getInstance().isCash(ivItem.getItemId())) {
                c.announce(
                        MaplePacketCreator.serverNotice(
                                1, "Cash items are not allowed to be sold on the Player Store."));
                return;
            }

            if (ServerConstants.USE_ENFORCE_UNMERCHABLE_PET
                    && ItemConstants.isPet(ivItem.getItemId())) {
                c.announce(
                        MaplePacketCreator.serverNotice(
                                1, "Pets are not allowed to be sold on the Player Store."));
                return;
            }
            ...
            if (ItemConstants.isRechargeable(ivItem.getItemId())) {
                MapleInventoryManipulator.removeFromSlot(
                        c, ivType, slot, ivItem.getQuantity(), true);
            } else {
                MapleInventoryManipulator.removeFromSlot(
                        c, ivType, slot, (short) (bundles * perBundle), true);
            }
            ...
            if (shop != null && shop.isOwner(chr)) {
                if (shop.isOpen()) {
                    c.announce(
                            MaplePacketCreator.serverNotice(
                                    1, "You can't take it with the store open."));
                    return;
                }
                ...
                if (slot >= shop.getItems().size() || slot < 0) {
                    AutobanFactory.PACKET_EDIT.alert(
                            chr, chr.getName() + " tried to packet edit with a player shop.");
                    FilePrinter.printError(
                            FilePrinter.EXPLOITS + chr.getName() + ".txt",
                            chr.getName()
            ...
            if (shop != null && shop.isVisitor(chr)) {
                ...
                merchant.broadcastToVisitorsThreadsafe(
                        MaplePacketCreator.updateHiredMerchant(merchant, chr));
            }
            ...
            if (merchant != null && merchant.isOwner(chr)) {
                ...
                if (slot >= merchant.getItems().size() || slot < 0) {
                    AutobanFactory.PACKET_EDIT.alert(
                            chr, chr.getName() + " tried to packet edit with a hired merchant.");
                    FilePrinter.printError(
                            FilePrinter.EXPLOITS + chr.getName() + ".txt",
                            chr.getName()
                                    + " tried to remove item at slot "
