    ...
    public final void handlePacket(SeekableLittleEndianAccessor slea, MapleClient c) {
        ...
        /*
          if ((!GameConstants.isPqSkillMap(chr.getMapId()) && GameConstants.isPqSkill(skillid)) || (!chr.isGM() && GameConstants.isGMSkills(skillid)) || (!GameConstants.isInJobTree(skillid, chr.getJob().getId()) && !chr.isGM())) {
          	...
          	FilePrinter.printError(FilePrinter.EXPLOITS + chr.getName() + ".txt", chr.getName() + " tried to use skill " + skillid + " without it being in their job." + System.lineSeparator() + "");
        ...
        if (effect.getCooldown() > 0) {
            ...
            if (skillid != Corsair.BATTLE_SHIP) {
                ...
                if (MapleStatEffect.isHerosWill(skillid)
                        && ServerConstants.USE_FAST_REUSE_HERO_WILL) {
                    cooldownTime /= 60;
                }
        ...
        if (skillid == Hero.MONSTER_MAGNET
            ...
            for (int i = 0; i < num; i++) {
                ...
                chr.getMap()
                        .broadcastMessage(
                                chr, MaplePacketCreator.showMagnet(mobId, success), false);
            ...
            chr.getMap()
                    .broadcastMessage(
                            ...
                            MaplePacketCreator.showBuffeffect(
                                    chr.getId(), skillid, chr.getSkillLevel(skillid), direction),
                            false);
            c.announce(MaplePacketCreator.enableActions());
            return;
        }
        if (skillid == Brawler.MP_RECOVERY) { // MP Recovery
            ...
            chr.getMap()
                    .broadcastMessage(
                            chr,
                            MaplePacketCreator.showBuffeffect(
                                    chr.getId(), skillid, chr.getSkillLevel(skillid)),
                            false);
