...
public final class MonsterCarnivalHandler extends AbstractMaplePacketHandler {
            ...
            if (chr.getCP() > getPrice(tab, number)) {
                if (tab == 0) { // SPAWNING
                    if (chr.getCarnivalParty().canSummon()) {
                        chr.getMap()
                                .spawnCPQMonster(
                                        MapleLifeFactory.getMonster(getMonster(number)),
                                        new Point(1, 1),
                                        carnival.oppositeTeam(chr.getCarnivalParty()).getTeam());
                        chr.getCarnivalParty().summon();
                    } else chr.announce(MaplePacketCreator.CPQMessage((byte) 2));
                    ...
                    MapleReactor reactor =
                            new MapleReactor(MapleReactorFactory.getReactor(rid), rid);
                    /*switch (number) {
                        case 0:
    ...
    public static int getMonster(int num) {
        int mid = 0;
        num++;
        switch (num) {
            case 1:
    ...
    public static int getPrice(int num, int tab) {
        int price = 0;
        num++;
