...
public final class CouponCodeHandler extends AbstractMaplePacketHandler {
                ...
                try {
                    ...
                    PreparedStatement ps =
                            con.prepareStatement(
                                    "UPDATE nxcode SET `valid` = 0 WHERE code = " + code);
                    ps.executeUpdate();
                    ps.close();
    ...
    private static int getNXCode(String code, String type) {
        int item = -1;
        try {
            Connection con = DatabaseConnection.getConnection();
            PreparedStatement ps =
                    con.prepareStatement("SELECT `" + type + "` FROM nxcode WHERE code = ?");
            ps.setString(1, code);
    ...
    private static boolean getNXCodeValid(String code, boolean validcode) {
        try {
            Connection con = DatabaseConnection.getConnection();
            PreparedStatement ps =
                    con.prepareStatement("SELECT `valid` FROM nxcode WHERE code = ?");
            ps.setString(1, code);
            ResultSet rs = ps.executeQuery();
            while (rs.next()) {
