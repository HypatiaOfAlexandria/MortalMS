...
public final class OwlWarpHandler extends AbstractMaplePacketHandler {
    ...
    public final void handlePacket(SeekableLittleEndianAccessor slea, MapleClient c) {
        ...
        MapleHiredMerchant hm =
                c.getWorldServer()
                        .getHiredMerchant(
                                ownerid); // if both hired merchant and player shop is on the same
        // map
        MaplePlayerShop ps;
        if (hm == null || hm.getMapId() != mapid || !hm.hasItem(c.getPlayer().getOwlSearch())) {
            ...
            if (ps.isOpen()) {
                if (GameConstants.isFreeMarketRoom(mapid)) {
                    if (ps.getChannel() == c.getChannel()) {
                        ...
                        if (ps.isOpen()) { // change map has a delay, must double check
                            ...
                            // c.announce(MaplePacketCreator.serverNotice(1, "That merchant has
                            // either been closed or is under maintenance."));
                ...
                // c.announce(MaplePacketCreator.serverNotice(1, "That merchant has either been
                // closed or is under maintenance."));
                c.announce(MaplePacketCreator.getOwlMessage(18));
            }
        } else {
            if (hm.isOpen()) {
                    ...
                    if (hm.getChannel() == c.getChannel()) {
                        ...
                        if (hm.isOpen()) { // change map has a delay, must double check
                            if (hm.addVisitor(c.getPlayer())) {
                                c.announce(
                                        MaplePacketCreator.getHiredMerchant(
                                                c.getPlayer(), hm, false));
                                c.getPlayer().setHiredMerchant(hm);
                            } else {
                                // c.announce(MaplePacketCreator.serverNotice(1, hm.getOwner() + "'s
                                // merchant is full. Wait awhile before trying again."));
                                c.announce(MaplePacketCreator.getOwlMessage(2));
                            }
                        } else {
                            // c.announce(MaplePacketCreator.serverNotice(1, "That merchant has
