...
public final class CloseRangeDamageHandler extends AbstractDealDamageHandler {
    ...
    public final void handlePacket(SeekableLittleEndianAccessor slea, MapleClient c) {
        ...
        if (GameConstants.isFinisherSkill(attack.skill)) {
            ...
            if (attack.skill != 1111008 && comboBuff != null) {
                ...
                if (advComboSkillLevel > 0) {
                    ...
                    ceffect = comboLv > 0 ? combo.getEffect(comboLv) : null;
                }
                if (ceffect != null) {
                    if (orbcount < ceffect.getX() + 1) {
                        ...
                        List<Pair<MapleBuffStat, Integer>> stat =
                                Collections.singletonList(
                                        new Pair<>(MapleBuffStat.COMBO, neworbcount));
                        chr.setBuffedValue(MapleBuffStat.COMBO, neworbcount);
                        duration -=
                                (int)
                                        (currentServerTime()
                                                - chr.getBuffedStarttime(MapleBuffStat.COMBO));
                        c.announce(MaplePacketCreator.giveBuff(oid, duration, stat));
                        chr.getMap()
                                .broadcastMessage(
                                        chr,
                                        MaplePacketCreator.giveForeignBuff(chr.getId(), stat),
                                        false);
                    }
                }
            } else if (chr.getSkillLevel(
                                    chr.isCygnus()
                                            ? SkillFactory.getSkill(15100004)
                                            : SkillFactory.getSkill(5110001))
                            > 0
                    && (chr.getJob().isA(MapleJob.MARAUDER)
                            || chr.getJob().isA(MapleJob.THUNDERBREAKER2))) {
        ...
        if (attack.numAttacked > 0 && attack.skill == DragonKnight.SACRIFICE) {
            ...
            chr.safeAddHP(
                    -1 * totDamageToOneMonster * attack.getAttackEffect(chr, null).getX() / 100);
        }
        ...
        if (attack.skill % 10000000 == 1009) { // bamboo
            ...
            c.announce(
                    MaplePacketCreator.serverNotice(
                            5, "As you used the secret skill, your energy bar has been reset."));
        ...
        if ((chr.getSkillLevel(SkillFactory.getSkill(NightWalker.VANISH)) > 0
                ...
                && chr.getBuffedValue(MapleBuffStat.DARKSIGHT)
                        != null) { // && chr.getBuffSource(MapleBuffStat.DARKSIGHT) != 9101004
