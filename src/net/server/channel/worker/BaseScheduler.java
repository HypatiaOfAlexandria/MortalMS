...
public abstract class BaseScheduler {
    ...
    private final List<MonitoredReentrantLock> externalLocks = new ArrayList<>();
    ...
    private final Runnable monitorTask = this::runBaseSchedule;

    protected BaseScheduler(MonitoredLockType lockType) {
        schedulerLock = MonitoredReentrantLockFactory.createLock(lockType, true);
    }

    // NOTE: practice EXTREME caution when adding external locks to the scheduler system, if you
            ...
            if (schedulerTask == null) {
                schedulerTask =
                        TimerManager.getInstance()
                                .register(
                                        monitorTask,
                                        ServerConstants.MOB_STATUS_MONITOR_PROC,
                                        ServerConstants.MOB_STATUS_MONITOR_PROC);
            }

            registeredEntries.put(
                    key,
                    new Pair<>(removalAction, Server.getInstance().getCurrentTime() + duration));
        } finally {
            unlockScheduler();
        }
    }
