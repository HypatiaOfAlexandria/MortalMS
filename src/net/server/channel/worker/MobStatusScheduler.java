...
public class MobStatusScheduler extends BaseScheduler {
    private final Map<MonsterStatusEffect, MobStatusOvertimeEntry> registeredMobStatusOvertime =
            new HashMap<>();
    private MonitoredReentrantLock overtimeStatusLock =
            MonitoredReentrantLockFactory.createLock(MonitoredLockType.CHANNEL_OVTSTATUS, true);
        ...
        super.addListener(
                (toRemove, update) -> {
                    ...
                    try {
                        ...
                        if (update) {
                            // it's probably ok to use one thread for both management & overtime
                            // actions
                            List<MobStatusOvertimeEntry> mdoeList =
                                    new ArrayList<>(registeredMobStatusOvertime.values());
    ...
    public void registerMobStatus(
            ...
            Runnable cancelStatus,
            long duration,
            Runnable overtimeStatus,
            int overtimeDelay) {
