...
public final class Channel {
    ...
    private final MobStatusScheduler[] mobStatusSchedulers = new MobStatusScheduler[4];
    private final MobAnimationScheduler[] mobAnimationSchedulers = new MobAnimationScheduler[4];
    private final MobClearSkillScheduler[] mobClearSkillSchedulers = new MobClearSkillScheduler[4];
    private final MobMistScheduler[] mobMistSchedulers = new MobMistScheduler[4];
    private final FaceExpressionScheduler[] faceExpressionSchedulers =
            new FaceExpressionScheduler[4];
    private final EventScheduler[] eventSchedulers = new EventScheduler[4];
    private final OverallScheduler[] channelSchedulers = new OverallScheduler[4];
    ...
    private final List<MapleExpedition> expeditions = new ArrayList<>();
    ...
    private final List<Integer> chapelReservationQueue = new ArrayList<>();
    private final List<Integer> cathedralReservationQueue = new ArrayList<>();
    ...
    private final ReentrantReadWriteLock merchantLock =
            new MonitoredReentrantReadWriteLock(MonitoredLockType.MERCHANT, true);
    private final ReadLock merchRlock = merchantLock.readLock();
    private final WriteLock merchWlock = merchantLock.writeLock();

    private final MonitoredReentrantLock[] faceLock = new MonitoredReentrantLock[4];

    private MonitoredReentrantLock lock =
            MonitoredReentrantLockFactory.createLock(MonitoredLockType.CHANNEL, true);
    ...
    public Channel(final int world, final int channel, long startTime) {
        ...
        this.ongoingStartTime =
                startTime + 10000; // rude approach to a world's last channel boot time, placeholder
        // for the 1st wedding reservation ever
        this.mapFactory =
                new MapleMapFactory(
                        null,
        ...
        try {
            ...
            respawnTask =
                    TimerManager.getInstance()
                            .register(new respawnMaps(), ServerConstants.RESPAWN_INTERVAL);
            ...
            for (int i = 0; i < 4; i++) {
                faceLock[i] =
                        MonitoredReentrantLockFactory.createLock(
                                MonitoredLockType.CHANNEL_FACEEXPRS, true);
            ...
            System.out.println("    Channel " + this.channel + ": Listening on port " + port);
        } catch (Exception e) {
            e.printStackTrace();
        }
    }
    ...
    public final void shutdown() {
        try {
            ...
            e.printStackTrace();
            System.err.println(
                    "Error while shutting down Channel "
                            + channel
                            + " on World "
                            + world
    ...
    public int getChannelCapacity() {
        return (int)
                (Math.ceil(
                        ((float) players.getAllCharacters().size() / ServerConstants.CHANNEL_LOAD)
                                * 800));
    }
    ...
    public void resetDojoMap(int fromMapId) {
        for (int i = 0; i < (((fromMapId / 100) % 100 <= 36) ? 5 : 2); i++) {
            mapFactory.getMap(fromMapId + (100 * i)).resetMapObjects();
        }
    }
    ...
    public void freeDojoSectionIfEmpty(int dojoMapId) {
        ...
        for (int i = 0; i < 5; i++) { // only 32 stages, but 38 maps
            MapleMap dojoMap = mapFactory.getMap(dojoBaseMap + (100 * (stage + i)) + delta);
        ...
        this.dojoTask[slot] =
                TimerManager.getInstance()
                        .schedule(
                                ...
                                clockTime + 3000); // let the TIMES UP display for 3 seconds, then
        // warp
    ...
    public boolean addMiniDungeon(int dungeonid) {
        ...
        try {
            ...
            MapleMiniDungeon mmd =
                    new MapleMiniDungeon(mmdi.getBase(), 30); // all minidungeons timeout on 30 mins
    ...
    public Pair<Boolean, Pair<Integer, Set<Integer>>> getNextWeddingReservation(boolean cathedral) {
        ...
        lock.lock();
        try {
            List<Integer> weddingReservationQueue =
                    (cathedral ? cathedralReservationQueue : chapelReservationQueue);
        ...
        Pair<String, String> couple =
                new Pair<>(
                        MapleCharacter.getNameById(coupleId.getLeft()),
                        MapleCharacter.getNameById(coupleId.getRight()));
        wserv.dropMessage(
                6,
                couple.getLeft()
                        + " and "
                        + couple.getRight()
                        + "'s wedding is going to be started at "
                        + (cathedral ? "Cathedral" : "Chapel")
                        + " on Channel "
                        + channel
                        + '.');
    ...
    public boolean isWeddingReserved(Integer weddingId) {
        ...
        lock.lock();
        try {
            return wserv.isMarriageQueued(weddingId)
                    || weddingId.equals(ongoingCathedral)
    ...
    public int pushWeddingReservation(
            Integer weddingId,
            boolean cathedral,
            boolean premium,
            Integer groomId,
        ...
        try {
            ...
            int delay =
                    ServerConstants.WEDDING_RESERVATION_DELAY - 1 - weddingReservationQueue.size();
    ...
    public void setOngoingWedding(
            final boolean cathedral, Boolean premium, Integer weddingId, Set<Integer> guests) {
        lock.lock();
        try {
            if (cathedral) {
    ...
    private static String getTimeLeft(long futureTime) {
        ...
        if (leftTime / (60 * 1000) > 0) {
            ...
            if (leftTime / (60 * 60 * 1000) > 0) mode++; // counts hours
        }
        ...
        try {
            ...
            return venue
                    ...
                    + getTimeLeft(
                            ongoingStartTime
                                    + (resStatus
                                            * ServerConstants.WEDDING_RESERVATION_INTERVAL
                                            * 60
                                            * 1000))
                    + " from now";
        } finally {
    ...
    public Pair<Integer, Integer> getWeddingCoupleForGuest(int guestId, boolean cathedral) {
        ...
        try {
            return (isOngoingWeddingGuest(cathedral, guestId))
                    ? Server.getInstance()
                            .getWorld(world)
                            .getRelationshipCouple(getOngoingWedding(cathedral))
                    : null;
        } finally {
            lock.unlock();
        }
    ...
    public void registerMobStatus(
            int mapid, MonsterStatusEffect mse, Runnable cancelAction, long duration) {
    ...
    public void registerMobStatus(
            ...
            long duration,
            Runnable overtimeAction,
            int overtimeDelay) {
        mobStatusSchedulers[getChannelSchedulerIndex(mapid)].registerMobStatus(
                mse, cancelAction, duration, overtimeAction, overtimeDelay);
    }
    ...
    public boolean registerMobOnAnimationEffect(int mapid, int mobHash, long delay) {
        return mobAnimationSchedulers[getChannelSchedulerIndex(mapid)].registerAnimationMode(
                mobHash, delay);
    }

    public void registerMobClearSkillAction(int mapid, Runnable runAction, long delay) {
        mobClearSkillSchedulers[getChannelSchedulerIndex(mapid)].registerClearSkillAction(
                runAction, delay);
    }

    public void registerMobMistCancelAction(int mapid, Runnable runAction, long delay) {
        mobMistSchedulers[getChannelSchedulerIndex(mapid)].registerMistCancelAction(
                runAction, delay);
    }
                ...
                () -> {
                    if (chr.isLoggedinWorld()) {
                        map.broadcastMessage(
                                chr, MaplePacketCreator.facialExpression(chr, 0), false);
                    }
                };
