...
public class NewYearCardRecord {
    ...
    public NewYearCardRecord(
            int senderid, String sender, int receiverid, String receiver, String message) {
        this.id = -1;
    ...
    private void setExtraNewYearCardRecord(
            int id,
            boolean senderDiscardCard,
            boolean receiverDiscardCard,
            boolean receiverReceivedCard,
            long dateSent,
            long dateReceived) {
        this.id = id;
        ...
        try (Connection con = DatabaseConnection.getConnection()) {
            try (PreparedStatement ps =
                    con.prepareStatement(
                            "INSERT INTO newyear VALUES (DEFAULT, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)")) {
    ...
    public static void updateNewYearCard(NewYearCardRecord newyear) {
        ...
        try (Connection con = DatabaseConnection.getConnection()) {
            try (PreparedStatement ps =
                    con.prepareStatement(
                            "UPDATE newyear SET received=1, timereceived=? WHERE id=?")) {
    ...
    public static NewYearCardRecord loadNewYearCard(int cardid) {
        ...
        try (Connection con = DatabaseConnection.getConnection()) {
            try (PreparedStatement ps =
                    con.prepareStatement("SELECT * FROM newyear WHERE id = ?")) {
                ps.setInt(1, cardid);
                try (ResultSet rs = ps.executeQuery()) {
                    if (rs.next()) {
                        ...
                        newyear.setExtraNewYearCardRecord(
                                rs.getInt("id"),
                                rs.getBoolean("senderdiscard"),
                                rs.getBoolean("receiverdiscard"),
                                rs.getBoolean("received"),
                                rs.getLong("timesent"),
                                rs.getLong("timereceived"));
                                    ...
                                    if (target != null && target.isLoggedinWorld()) {
                                        target.announce(
                                                MaplePacketCreator.onNewYearCardRes(
                                                        target, NewYearCardRecord.this, 0xC, 0));
                                    }
                                },
                                1000 * 60 * 60); // 1 Hour
    }
        ...
        for (NewYearCardRecord nyc : set) {
            if (send) {
                if (nyc.senderId == cid) {
                    ...
                    chr.getMap()
                            .broadcastMessage(
                                    MaplePacketCreator.onNewYearCardRes(chr, nyc, 0xE, 0));

                    MapleCharacter other =
                            chr.getClient()
                                    .getWorldServer()
                                    .getPlayerStorage()
                                    .getCharacterById(nyc.receiverId);
                    if (other != null && other.isLoggedinWorld()) {
                        other.removeNewYearRecord(nyc);
                        other.getMap()
                                .broadcastMessage(
                                        MaplePacketCreator.onNewYearCardRes(other, nyc, 0xE, 0));

                        other.dropMessage(
                                6,
                                "[NEW YEAR] " + chr.getName() + " threw away the New Year card.");
                    }
                }
            } else {
    ...
    public static void startPendingNewYearCardRequests() {
        try (Connection con = DatabaseConnection.getConnection()) {
            try (PreparedStatement ps =
                    con.prepareStatement(
                            "SELECT * FROM newyear WHERE timereceived = 0 AND senderdiscard = 0")) {
                try (ResultSet rs = ps.executeQuery()) {
                    while (rs.next()) {
                        NewYearCardRecord newyear =
                                new NewYearCardRecord(
