...
public class MapleInventoryManipulator {
    ...
    public static boolean addById(
            MapleClient c, int itemId, short quantity, String owner, int petid, long expiration) {
    ...
    public static boolean addById(
        ...
        if (!type.equals(MapleInventoryType.EQUIP)) {
            ...
            if (!ItemConstants.isRechargeable(itemId) && petid == -1) {
                ...
                c.announce(
                        MaplePacketCreator.modifyInventory(
                                true, Collections.singletonList(new ModifyInventory(0, nItem))));
            ...
            c.announce(
                    MaplePacketCreator.modifyInventory(
                            true, Collections.singletonList(new ModifyInventory(0, nEquip))));
    ...
    public static int checkSpaceProgressively(
        ...
        MapleInventoryType type =
                !useProofInv ? ItemConstants.getInventoryType(itemid) : MapleInventoryType.CANHOLD;
        MapleCharacter chr = c.getPlayer();
        MapleInventory inv = chr.getInventory(type);
        ...
        if (!type.equals(MapleInventoryType.EQUIP)) {
            ...
            returnValue +=
                    (numSlotsNeeded == 0
                                    || !inv.isFullAfterSomeItems(numSlotsNeeded - 1, usedSlots))
                            ? 1
                            : 0;
    ...
    public static void removeFromSlot(
            MapleClient c, MapleInventoryType type, short slot, short quantity, boolean fromDrop) {
    ...
    private static void announceModifyInventory(
            MapleClient c, Item item, boolean fromDrop, boolean allowZero) {
        if (item.getQuantity() == 0 && !allowZero) {
            c.announce(
                    MaplePacketCreator.modifyInventory(
                            fromDrop, Collections.singletonList(new ModifyInventory(3, item))));
        } else {
            c.announce(
                    MaplePacketCreator.modifyInventory(
                            fromDrop, Collections.singletonList(new ModifyInventory(1, item))));
        }
    }
        ...
        for (short i = 0; i <= slotLimit; i++) {
            ...
            if (item != null) {
                if (item.getItemId() == itemId || item.getCashId() == itemId) {
                    ...
                    removeFromSlot(
                            c, type, item.getPosition(), item.getQuantity(), fromDrop, consume);
                }
            }
        }
        assert removeQuantity <= 0 || type == MapleInventoryType.CANHOLD
                : "[HACK] Not enough items available of Item:"
                        + itemId
                        + ", Quantity (After Quantity/Over Current Quantity): "
    ...
    public static void move(MapleClient c, MapleInventoryType type, short src, short dst) {
        ...
        if (!(type.equals(MapleInventoryType.EQUIP) || type.equals(MapleInventoryType.CASH))
                && initialTarget != null
    ...
    public static void equip(MapleClient c, short src, short dst) {
        ...
        if ((((source.getItemId() >= 1902000 && source.getItemId() <= 1902002)
                                || source.getItemId() == 1912000)
                        && chr.isCygnus())
                || ((source.getItemId() >= 1902005 && source.getItemId() <= 1902007)
                                || source.getItemId() == 1912005)
    ...
    public static void unequip(MapleClient c, short src, short dst) {
        ...
        c.announce(
                MaplePacketCreator.modifyInventory(
                        true, Collections.singletonList(new ModifyInventory(2, source, src))));
        chr.equipChanged();
    }
    ...
    public static void drop(MapleClient c, MapleInventoryType type, short src, short quantity) {
        ...
        if (chr.getTrade() != null
                ...
                || source == null) { // Only check needed would prob be merchants (to see if the
            // player is in one)
            return;
        }
        ...
        if ((!ItemConstants.isRechargeable(itemId) && source.getQuantity() < quantity)
                || quantity < 0) {
            return;
        }
        ...
        if (quantity < source.getQuantity() && !ItemConstants.isRechargeable(itemId)) {
            ...
            c.announce(
                    MaplePacketCreator.modifyInventory(
                            true, Collections.singletonList(new ModifyInventory(1, source))));
            ...
            if (ItemConstants.isNewYearCardEtc(itemId)) {
                ...
                if (ii.isDropRestricted(target.getItemId())
                        || ii.isCash(target.getItemId())
                        || isDroppedItemRestricted(target)) {
                    map.disappearingItemDrop(chr, chr, target, dropPos);
                } else {
            ...
            c.announce(
                    MaplePacketCreator.modifyInventory(
                            true, Collections.singletonList(new ModifyInventory(3, source))));
            if (src < 0) {
                chr.equipChanged();
            ...
            if (map.getEverlast()) {
                if (ii.isDropRestricted(itemId)
                        || ii.isCash(itemId)
                        || isDroppedItemRestricted(source)) {
    ...
    private static boolean isDroppedItemRestricted(Item it) {
        return ServerConstants.USE_ERASE_UNTRADEABLE_DROP
                && ((it.getFlag() & ItemConstants.UNTRADEABLE) == ItemConstants.UNTRADEABLE);
    }
