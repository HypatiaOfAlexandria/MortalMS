...
public class MaplePet extends Item {
        ...
        try {
            ...
            PreparedStatement ps =
                    con.prepareStatement(
                            "SELECT name, level, closeness, fullness, summoned FROM pets WHERE petid = ?"); // Get pet details..
            ps.setInt(1, petid);
            ResultSet rs = ps.executeQuery();
            rs.next();
            ret.name = rs.getString("name");
            ret.closeness = Math.min(rs.getInt("closeness"), 30000);
            ret.level = (byte) Math.min(rs.getByte("level"), 30);
            ret.fullness = Math.min(rs.getInt("fullness"), 100);
            ret.summoned = rs.getInt("summoned") == 1;
            rs.close();
            ps.close();
            con.close();
            return ret;
    ...
    public void gainClosenessFullness(
            MapleCharacter owner, int incCloseness, int incFullness, int type) {
        ...
        owner.getMap()
                .broadcastMessage(
                        MaplePacketCreator.commandResponse(owner.getId(), slot, type, enjoyed));
        saveToDb();

        Item petz = owner.getInventory(MapleInventoryType.CASH).getItem(getPosition());
        if (petz != null) owner.forceUpdateItem(petz);
    }
        ...
        for (LifeMovementFragment move : movement) {
            if (move instanceof LifeMovement) {
                if (move instanceof AbsoluteLifeMovement) {
                    this.pos = move.getPosition();
                }
                this.stance = ((LifeMovement) move).getNewstate();
            }
        }
    }
}
