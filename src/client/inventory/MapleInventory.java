...
public class MapleInventory implements Iterable<Item> {
    ...
    protected Lock lock =
            MonitoredReentrantLockFactory.createLock(MonitoredLockType.INVENTORY, true);
    ...
    public boolean
            isExtendableInventory() { // not sure about cash, basing this on the previous one.
        return !(type.equals(MapleInventoryType.UNDEFINED)
        ...
        for (Item item : list()) {
            ...
            if (itemName == null) {
                FilePrinter.printError(
                        ...
                        "[CRITICAL] Item " + item.getItemId() + " has no name.");
                continue;
            }
    ...
    public int countNotOwnedById(int itemId) {
        int qty = 0;
        for (Item item : list()) {
            if (item.getItemId() == itemId && item.getOwner().isEmpty()) {
                qty += item.getQuantity();
            }
        }
        return qty;
    ...
    public List<Item> ArrayListById(int itemId) {
        List<Item> ret = new ArrayList<>();
        for (Item item : list()) {
            if (item.getItemId() == itemId) {
                ret.add(item);
    ...
    public static boolean checkSpots(
            MapleCharacter chr, List<Pair<Item, MapleInventoryType>> items) {
        return checkSpots(chr, items, false);
    }

    public static boolean checkSpots(
            MapleCharacter chr, List<Pair<Item, MapleInventoryType>> items, boolean useProofInv) {
        ...
        for (Entry<Integer, Short> it : rcvItems.entrySet()) {
            ...
            int result =
                    MapleInventoryManipulator.checkSpaceProgressively(
                            c, it.getKey(), it.getValue(), "", usedSlots, useProofInv);
    ...
    public static boolean checkSpotsAndOwnership(
            ...
            boolean useProofInv) {
        // assumption: no "UNDEFINED" or "EQUIPPED" items shall be tested here, all counts are >= 0
        // and item list to be checked is a legal one.
        ...
        for (Entry<Long, Short> it : rcvItems.entrySet()) {
            ...
            // System.out.print("inserting " + itemId.intValue() + " with type " + itemType + " qty
            // " + it.getValue() + " owner '" + rcvOwners.get(it.getKey()) + "' current
            // usedSlots:");
