...
public class Equip extends Item {
    ...
    private boolean isUpgradeable,
            isElemental =
                    false; // timeless or reverse, or any equip that could levelup on GMS for all
    // effects
    ...
    public Equip(int id, short position, int slots) {
        ...
        this.isElemental =
                (MapleItemInformationProvider.getInstance().getEquipLevel(id, false) > 1);
    }

    @Override
    public Item copy() {
        Equip ret = new Equip(getItemId(), getPosition(), upgradeSlots);
        ret.str = str;
        ret.dex = dex;
        ret._int = _int;
        ret.luk = luk;
    ...
    private void getUnitStatUpgrade(
            List<Pair<StatUpgrade, Integer>> stats,
            StatUpgrade name,
            int curStat,
            boolean isAttribute) {
        isUpgradeable = true;

        int maxUpgrade = randomizeStatUpgrade((1 + (curStat / getStatModifier(isAttribute))));
        if (maxUpgrade == 0) return;

        stats.add(new Pair<>(name, maxUpgrade));
    }

    private static void getUnitSlotUpgrade(
            List<Pair<StatUpgrade, Integer>> stats, StatUpgrade name) {
        if (Math.random() < 0.1) {
    ...
    private void gainLevel(MapleClient c) {
        List<Pair<StatUpgrade, Integer>> stats = new ArrayList<>();
        ...
        StringBuilder lvupStr =
                new StringBuilder(
                        '\''
                                + MapleItemInformationProvider.getInstance()
                                        .getName(this.getItemId())
                                + "' is now level "
                                + itemLevel
                                + "! ");
        String showStr =
                "#e'"
                        + MapleItemInformationProvider.getInstance().getName(this.getItemId())
                        + "'#b is now #elevel #r"
                        + itemLevel
                        + "#k#b!";

        int statUp, maxStat = ServerConstants.MAX_EQUIPMNT_STAT;
        ...
        if (gotVicious) {
            // c.getPlayer().dropMessage(6, "A new Vicious Hammer opportunity has been found on the
            // '" + MapleItemInformationProvider.getInstance().getName(getItemId()) + "'!");
            lvupStr.append("+VICIOUS ");
        }
        if (gotSlot) {
            // c.getPlayer().dropMessage(6, "A new upgrade slot has been found on the '" +
            // MapleItemInformationProvider.getInstance().getName(getItemId()) + "'!");
            lvupStr.append("+UPGSLOT ");
        }

        showLevelupMessage(showStr, c); // thanks to Polaris dev team !
        c.getPlayer().dropMessage(6, lvupStr.toString());
        ...
        c.getPlayer()
                ...
                .broadcastMessage(
                        c.getPlayer(),
                        MaplePacketCreator.showForeignEffect(c.getPlayer().getId(), 15));
    ...
    private static double normalizedMasteryExp(int reqLevel) {
        // Conversion factor between mob exp and equip exp gain. Through many calculations, the
        // expected for equipment levelup
        // from level 1 to 2 is killing about 100~200 mobs of the same level range, on a 1x EXP rate
        // scenario.
    ...
    public synchronized void gainItemExp(MapleClient c, int gain) { // Ronan's Equip Exp gain method
        ...
        int equipMaxLevel =
                Math.min(
                        30,
                        Math.max(
                                ii.getEquipLevel(this.getItemId(), true),
                                ServerConstants.USE_EQUIPMNT_LVLUP));
        ...
        float masteryModifier =
                (float) (ServerConstants.EQUIP_EXP_RATE * ExpTable.getExpNeededForLevel(1))
        ...
        c.getPlayer().forceUpdateItem(this);
        // if(ServerConstants.USE_DEBUG) c.getPlayer().dropMessage("'" +
    ...
    private boolean reachedMaxLevel() {
        if (isElemental) {
            if (itemLevel
                    < MapleItemInformationProvider.getInstance().getEquipLevel(getItemId(), true)) {
                return false;
            }
        }
        ...
        String eqpInfo =
                reachedMaxLevel()
                        ...
                        : (" EXP: #e#b"
                                + (int) itemExp
                                + "#k#n / "
                                + ExpTable.getEquipExpNeededForLevel(itemLevel));
