...
public abstract class AbstractMapleCharacterObject extends AbstractAnimatedMapleMapObject {
    ...
    protected Lock effLock =
            MonitoredReentrantLockFactory.createLock(MonitoredLockType.CHARACTER_EFF, true);
    ...
    protected AbstractMapleCharacterObject() {
        ReentrantReadWriteLock locks =
                new MonitoredReentrantReadWriteLock(MonitoredLockType.CHARACTER_STA, true);
    ...
    private void changeStatPool(
            Long hpMpPool, Long strDexIntLuk, Long newSp, int newAp, boolean silent) {
        effLock.lock();
        statWlock.lock();
        try {
            statUpdates.clear();
            ...
            if (hpMpPool != null) {
                var newHp = (short) (hpMpPool >> 48);
                var newMp = (short) (hpMpPool >> 32);
                var newMaxHp = (short) (hpMpPool >> 16);
                var newMaxMp = hpMpPool.shortValue();
            ...
            if (strDexIntLuk != null) {
                var newStr = (short) (strDexIntLuk >> 48);
                var newDex = (short) (strDexIntLuk >> 32);
                var newInt = (short) (strDexIntLuk >> 16);
                var newLuk = strDexIntLuk.shortValue();
    ...
    protected void addMaxMPMaxHP(int hpdelta, int mpdelta, boolean silent) {
        ...
        try {
            changeHpMpPool(
                    Short.MIN_VALUE, Short.MIN_VALUE, maxhp + hpdelta, maxmp + mpdelta, silent);
        } finally {
            statWlock.unlock();
            effLock.unlock();
        }
    ...
    public boolean assignMP(int deltaMP, int deltaAp) {
        ...
        try {
            ...
            long hpMpPool =
                    calcStatPoolLong(Short.MIN_VALUE, Short.MIN_VALUE, maxhp, maxmp + deltaMP);
            long strDexIntLuk = calcStatPoolLong(str, dex, int_, luk);
    ...
    public boolean assignStrDexIntLuk(int deltaStr, int deltaDex, int deltaInt, int deltaLuk) {
        ...
        try {
            ...
            int newStr = str + deltaStr,
                    newDex = dex + deltaDex,
                    newInt = int_ + deltaInt,
                    newLuk = luk + deltaLuk;
    ...
    private void changeStrDexIntLuk(
            int str, int dex, int int_, int luk, int remainingAp, boolean silent) {
        long strDexIntLuk = calcStatPoolLong(str, dex, int_, luk);
        changeStatPool(null, strDexIntLuk, null, remainingAp, silent);
    }

    private void changeStrDexIntLukSp(
            int str,
            int dex,
            int int_,
            int luk,
    ...
    protected void updateStrDexIntLukSp(
            int str, int dex, int int_, int luk, int remainingAp, int remainingSp, int skillbook) {
