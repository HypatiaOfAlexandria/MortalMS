...
public class MapleFamily {
    ...
    private static final Map<Integer, MapleFamilyEntry> members = new HashMap<>();

    public MapleFamily(int cid) {
        try {
            Connection con = DatabaseConnection.getConnection();
            PreparedStatement ps =
                    con.prepareStatement("SELECT familyid FROM family_character WHERE cid = ?");
            ps.setInt(1, cid);
    ...
    private static void getMapleFamily() {
        try {
            Connection con = DatabaseConnection.getConnection();
            PreparedStatement ps =
                    con.prepareStatement("SELECT * FROM family_character WHERE familyid = ?");
            ps.setInt(1, id);
            ResultSet rs = ps.executeQuery();
            while (rs.next()) {
    ...
    public static MapleFamilyEntry getMember(int cid) {
    ...
    public static Map<Integer, MapleFamilyEntry> getMembers() {
        return members;
    }
