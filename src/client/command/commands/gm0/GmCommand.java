...
public class GmCommand extends Command {
    ...
    public void execute(MapleClient c, String[] params) {
        ...
        if (params[0].length() < 3) { // #goodbye 'hi'
            player.dropMessage(
                    5, "Your message was too short. Please provide as much detail as possible.");
            return;
        }
        ...
        Server.getInstance()
                .broadcastGMMessage(
                        ...
                        MaplePacketCreator.sendYellowTip(
                                "[GM MESSAGE]:"
                                        + MapleCharacter.makeMapleReadable(player.getName())
                                        + ": "
                                        + message));
        Server.getInstance()
                .broadcastGMMessage(c.getWorld(), MaplePacketCreator.serverNotice(1, message));
