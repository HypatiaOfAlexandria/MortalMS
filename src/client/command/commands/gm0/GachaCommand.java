...
public class GachaCommand extends Command {
    ...
    public void execute(MapleClient c, String[] params) {
        ...
        int[] ids = {
            9100100, 9100101, 9100102, 9100103, 9100104, 9100105, 9100106, 9100107, 9100109, 9100117
        };
        ...
        if (gacha == null) {
            c.getPlayer()
                    .yellowMessage(
                            "Please use @gacha <name> where name corresponds to one of the below:");
            for (String name : names) {
                c.getPlayer().yellowMessage(name);
            }
            return;
        }
        StringBuilder talkStr =
                new StringBuilder(
                        "The #b" + gachaName + "#k Gachapon contains the following items.\n\n");
        for (int i = 0; i < 2; i++) {
            for (int id : gacha.getItems(i)) {
                talkStr.append('-')
                        .append(MapleItemInformationProvider.getInstance().getName(id))
                        .append('\n');
            }
        }
        talkStr.append(
        ...
        c.announce(
                MaplePacketCreator.getNPCTalk(
                        9010000, (byte) 0, talkStr.toString(), "00 00", (byte) 0));
    }
}
