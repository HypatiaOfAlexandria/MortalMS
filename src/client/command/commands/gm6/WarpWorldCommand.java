    ...
    public void execute(MapleClient c, String[] params) {
        ...
        if (worldb <= (server.getWorldsSize() - 1)) {
            try {
                ...
                player.saveCharToDB(); // To set the new world :O (true because else 2 player
                // instances are created, one in both worlds)
                c.announce(
                ...
                ex.printStackTrace();
                player.message(
                        "Unexpected error when changing worlds, are you sure the world you are trying to warp to has the same amount of channels?");
            }

        } else {
            player.message(
                    "Invalid world; highest number available: " + (server.getWorldsSize() - 1));
        }
    }
}
