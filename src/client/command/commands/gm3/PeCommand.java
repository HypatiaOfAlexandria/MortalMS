...
public class PeCommand extends Command {
    ...
    public void execute(MapleClient c, String[] params) {
        ...
        String packet;
        try {
            var is = new InputStreamReader(new FileInputStream("pe.txt"), StandardCharsets.UTF_8);
            var packetProps = new Properties();
            packetProps.load(is);
            is.close();
        ...
        SeekableLittleEndianAccessor slea =
                new GenericSeekableLittleEndianAccessor(new ByteArrayByteStream(mplew.getPacket()));
        short packetId = slea.readShort();
        final MaplePacketHandler packetHandler =
                PacketProcessor.getProcessor(0, c.getChannel()).getHandler(packetId);
