...
public class FlyCommand extends Command {
    ...
    public void execute(
            MapleClient c,
            String[] params) { // fly option will become available for any character of that account
        ...
        if (params[0].equalsIgnoreCase("on")) {
            ...
            if (!Server.canFly(accid)) sendStr += " Re-login to take effect.";

            Server.changeFly(c.getAccID(), true);
        } else {
            sendStr += "Disabled Fly feature. You can now attack.";
            if (Server.canFly(accid)) sendStr += " Re-login to take effect.";

            Server.changeFly(c.getAccID(), false);
        }
