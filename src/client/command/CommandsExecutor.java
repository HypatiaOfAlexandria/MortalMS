...
public class CommandsExecutor {
    ...
    private final HashMap<String, RegisteredCommand> registeredCommands = new HashMap<>();
    ...
    public void handle(MapleClient client, String message) {
        ...
        if (command == null) {
            client.getPlayer()
                    .yellowMessage(
                            "Command '"
                                    + commandName
                                    + "' is not available. See @commands for a list of available commands.");
            return;
        }
        ...
        String[] params;
        params =
                spitedMessage.length > 1
                        ? Arrays.copyOfRange(spitedMessage, 1, spitedMessage.length)
                        : new String[] {};
        try {
            Command commandInstance = command.getCommandClass().getConstructor().newInstance();
    ...
    private static void writeLog(MapleClient client, String command) {
        SimpleDateFormat sdf = new SimpleDateFormat("dd-MM-yyyy HH:mm");
        FilePrinter.print(
                FilePrinter.USED_COMMANDS,
    ...
    private void addCommandInfo(String name, Class<? extends Command> commandClass) {
        try {
            levelCommandsCursor
                    .getRight()
                    .add(commandClass.getConstructor().newInstance().getDescription());
    ...
    private void addCommand(String syntax, int rank, Class<? extends Command> commandClass) {
        if (registeredCommands.containsKey(syntax.toLowerCase())) {
            System.out.println(
                    "Error on register command with name: " + syntax + ". Already exists.");
            return;
        }
