...
public class MapleClient {
    ...
    private final Lock lock =
            MonitoredReentrantLockFactory.createLock(MonitoredLockType.CLIENT, true);
    private final Lock encoderLock =
            MonitoredReentrantLockFactory.createLock(MonitoredLockType.CLIENT_ENCODER, true);
    private static final Lock loginLock =
            MonitoredReentrantLockFactory.createLock(MonitoredLockType.CLIENT_LOGIN, true);
    ...
    private void loadHWIDIfNescessary() throws SQLException {
        if (hwid == null) {
            try (Connection con = DatabaseConnection.getConnection();
                    PreparedStatement ps =
                            con.prepareStatement("SELECT hwid FROM accounts WHERE id = ?")) {
                ps.setInt(1, accId);
                try (ResultSet rs = ps.executeQuery()) {
                    if (rs.next()) {
    ...
    private void loadMacsIfNescessary() throws SQLException {
        if (macs.isEmpty()) {
            try (Connection con = DatabaseConnection.getConnection();
                    PreparedStatement ps =
                            con.prepareStatement("SELECT macs FROM accounts WHERE id = ?")) {
                ps.setInt(1, accId);
                try (ResultSet rs = ps.executeQuery()) {
                    if (rs.next()) {
    ...
    public void banMacs() {
        ...
        try {
            ...
            try (PreparedStatement ps =
                    con.prepareStatement("INSERT INTO macbans (mac, aid) VALUES (?, ?)")) {
                for (String mac : macs) {
    ...
    public int finishLogin() {
        ...
        try {
            if (getLoginState()
                    > LOGIN_NOTLOGGEDIN) { // 0 = LOGIN_NOTLOGGEDIN, 1= LOGIN_SERVER_TRANSITION, 2 =
                // LOGIN_LOGGEDIN
                loggedIn = false;
                return 7;
            }
    ...
    public void setPin(String pin) {
        ...
        try {
            try (Connection con = DatabaseConnection.getConnection();
                    PreparedStatement ps =
                            con.prepareStatement("UPDATE accounts SET pin = ? WHERE id = ?")) {
                ps.setString(1, pin);
                ps.setInt(2, accId);
                ps.executeUpdate();
            }
    ...
    public void setPic(String pic) {
        ...
        try {
            try (Connection con = DatabaseConnection.getConnection();
                    PreparedStatement ps =
                            con.prepareStatement("UPDATE accounts SET pic = ? WHERE id = ?")) {
                ps.setString(1, pic);
                ps.setInt(2, accId);
                ps.executeUpdate();
            }
    ...
    public int login(String login, String pwd) {
        ...
        try {
            ...
            ps =
                    con.prepareStatement(
                            "SELECT id, password, salt, gender, banned, pin, pic, characterslots, tos FROM accounts WHERE name = ?");
        ...
        if (loginok == 0 || loginok == 4) {
            AntiMulticlientResult res =
                    MapleSessionCoordinator.getInstance()
                            .attemptLoginSession(session, accId, loginok == 4);
    ...
    public static long dottedQuadToLong(String dottedQuad) throws RuntimeException {
        ...
        assert quads.length == 4 : "Invalid IP Address format.";
        long ipAddress = 0;
        for (int i = 0; i < 4; i++) {
    ...
    public void updateLoginState(int newstate) {
        try {
            ...
            try (PreparedStatement ps =
                ...
                // using sql currenttime here could potentially break the login, thanks Arnah for
                // pointing this out
    ...
    public int getLoginState() { // 0 = LOGIN_NOTLOGGEDIN, 1= LOGIN_SERVER_TRANSITION, 2 =
        // LOGIN_LOGGEDIN
        try {
            Connection con = DatabaseConnection.getConnection();
            PreparedStatement ps =
            ...
            if (state == LOGIN_SERVER_TRANSITION) {
                if (rs.getTimestamp("lastlogin").getTime() + 30000
    ...
    public final synchronized void disconnect(
            boolean shutdown, boolean cashshop) { // once per MapleClient instance
        if (disconnecting) {
            return;
        }
        disconnecting = true;
        if (player != null && player.isLoggedin() && player.getClient() != null) {
            ...
            final World wserv =
                    getWorldServer(); // obviously wserv is NOT null if this player was online on it
            try {
                removePlayer(wserv);
                ...
                if (!(channel == -1 || shutdown)) {
                    if (!cashshop) {
                        if (!this.serverTransition) { // meaning not changing channels
                            ...
                            */
                            for (MapleQuestStatus status :
                                    player.getStartedQuests()) { // This is for those quests that
                                // you have to stay logged in for
                            ...
                            if (guild != null) {
                                ...
                                server.setGuildMemberOnline(
                                        player, false, player.getClient().channel);
                                player.getClient()
    ...
    public boolean deleteCharacter(int cid, int senderAccId) {
        try {
            return MapleCharacter.deleteCharFromDB(
                    MapleCharacter.loadCharFromDB(cid, this, false), senderAccId);
        } catch (SQLException ex) {
            ex.printStackTrace();
            return false;
        }
    ...
    public int getVotePoints() {
        ...
        try {
            Connection con = DatabaseConnection.getConnection();
            PreparedStatement ps =
                    con.prepareStatement("SELECT `votes` FROM accounts WHERE id = ?");
            ps.setInt(1, accId);
            ResultSet rs = ps.executeQuery();
    ...
    private void saveVotePoints() {
        try {
            Connection con = DatabaseConnection.getConnection();
            try (PreparedStatement ps =
                    con.prepareStatement("UPDATE accounts SET votes = ? WHERE id = ?")) {
    ...
    public short getAvailableCharacterSlots() {
        return (short)
                Math.max(0, characterSlots - Server.getInstance().getAccountCharacterCount(accId));
    }
    ...
    public short getAvailableCharacterWorldSlots() {
        return (short)
                Math.max(
                        0,
                        characterSlots
                                - Server.getInstance().getAccountWorldCharacterCount(accId, world));
    }
    ...
    public void setGender(byte m) {
        ...
        try {
            con = DatabaseConnection.getConnection();
            try (PreparedStatement ps =
                    con.prepareStatement("UPDATE accounts SET gender = ? WHERE id = ?")) {
                ps.setByte(1, gender);
                ps.setInt(2, accId);
                ps.executeUpdate();
            }
    ...
    public synchronized void announceBossHpBar(
            MapleMonster mm, final int mobHash, final byte[] packet) {
        long timeNow = System.currentTimeMillis();
        int targetHash = player.getTargetHpBarHash();

        if (mobHash != targetHash) {
            if (timeNow - player.getTargetHpBarTime() >= 5 * 1000) {
                // is there a way to INTERRUPT this annoying thread running on the client that drops
                // the boss bar after some time at every attack?
    ...
    public synchronized void announce(
            final byte[] packet) { // MINA CORE IS A FUCKING BITCH AND I HATE IT <3
    ...
    public void changeChannel(int channel) {
        ...
        if (MapleMiniDungeonInfo.isDungeonMap(player.getMapId())) {
            announce(
        ...
        String[] socket = Server.getInstance().getInetSocket(world, channel);
        if (socket == null) {
            announce(
                    MaplePacketCreator.serverNotice(
                            1,
