...
public class DueyProcessor {
    ...
    private static void removeItemFromDB(int packageid) {
        ...
        try {
            ...
            PreparedStatement ps =
                    con.prepareStatement("DELETE FROM dueypackages WHERE PackageId = ?");
    ...
    public static void addItemToDB(
            Item item, int quantity, int mesos, String sName, int recipientID) {
        Connection con = null;
        try {
            con = DatabaseConnection.getConnection();
                ...
                if (item == null) {
                    ...
                    try (ResultSet rs = ps.getGeneratedKeys()) {
                        ...
                        if (item.getInventoryType().equals(MapleInventoryType.EQUIP)) {
                            ps2 =
                                    con.prepareStatement(
                                            "INSERT INTO dueyitems (PackageId, itemid, quantity, upgradeslots, level, itemlevel, itemexp, str, dex, `int`, luk, hp, mp, watk, matk, wdef, mdef, acc, avoid, hands, speed, jump, flag, owner) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)");
                            Equip eq = (Equip) item;
                            ...
                            ps2 =
                                    con.prepareStatement(
                                            "INSERT INTO dueyitems (PackageId, itemid, quantity, flag, owner) VALUES (?, ?, ?, ?, ?)");
        ...
        try {
            ...
            if (mesos < 0 || sendMesos > Integer.MAX_VALUE || (amount < 1 && mesos == 0)) {
                AutobanFactory.PACKET_EDIT.alert(
                        c.getPlayer(),
                        c.getPlayer().getName() + " tried to packet edit with duey.");
                FilePrinter.printError(
                        FilePrinter.EXPLOITS + c.getPlayer().getName() + ".txt",
                        c.getPlayer().getName()
            ...
            if (c.getPlayer().getMeso() >= finalcost) {
                ...
                if (accid != -1) {
                    if (accid == c.getAccID()) {
                        c.announce(
                                MaplePacketCreator.sendDueyMSG(
                                        DueyProcessor.Actions.TOCLIENT_SEND_SAMEACC_ERROR
                                                .getCode()));
                        return;
                    }
                } else {
                    c.announce(
                            MaplePacketCreator.sendDueyMSG(
                                    DueyProcessor.Actions.TOCLIENT_SEND_NAME_DOES_NOT_EXIST
                                            .getCode()));
                    return;
                }
            } else {
            ...
            if (inventId > 0) {
                ...
                if (item != null
                    ...
                    c.announce(
                            MaplePacketCreator.sendDueyMSG(
                                    DueyProcessor.Actions.TOCLIENT_SEND_SUCCESSFULLY_SENT
                                            .getCode()));

                    if (ItemConstants.isRechargeable(item.getItemId())) {
                        MapleInventoryManipulator.removeFromSlot(
                                c, inv, itemPos, item.getQuantity(), true);
                    } else {
                        MapleInventoryManipulator.removeFromSlot(
                                c, inv, itemPos, amount, true, false);
                    }
            ...
            try {
                ...
                if (dp.getItem() != null) {
                    if (!MapleInventoryManipulator.checkSpace(
                        ...
                        if (MapleItemInformationProvider.getInstance().isPickupRestricted(itemid)
                                && c.getPlayer()
                                                .getInventory(
