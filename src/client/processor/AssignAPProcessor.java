...
public class AssignAPProcessor {
    ...
    public static void APAutoAssignAction(SeekableLittleEndianAccessor slea, MapleClient c) {
        ...
        try {
            ...
            if (ServerConstants.USE_SERVER_AUTOASSIGNER) {
                ...
                // This method excels for assigning APs in such a way to cover all equipments AP
                // requirements.
                ...
                for (Item item :
                        equippedC) { // selecting the biggest AP value of each stat from each
                    // equipped item.
                    nEquip = (Equip) item;
                ...
                // Autoassigner looks up the 1st/2nd placed equips for their stats to calculate the
                // optimal upgrade.
                int eqpStr =
                        getNthHighestStat(eqpStrList, (short) 0)
                                + getNthHighestStat(eqpStrList, (short) 1);
                int eqpDex =
                        getNthHighestStat(eqpDexList, (short) 0)
                                + getNthHighestStat(eqpDexList, (short) 1);
                int eqpLuk =
                        getNthHighestStat(eqpLukList, (short) 0)
                                + getNthHighestStat(eqpLukList, (short) 1);

                // c.getPlayer().message("----------------------------------------");
                // c.getPlayer().message("SDL: s" + eqpStr + " d" + eqpDex + " l" + eqpLuk + " BASE
                // STATS --> STR: " + chr.getStr() + " DEX: " + chr.getDex() + " INT: " +
                // chr.getInt() + " LUK: " + chr.getLuk());
                // c.getPlayer().message("SUM EQUIP STATS -> STR: " + str + " DEX: " + dex + " LUK:
                // " + luk + " INT: " + int_);
                ...
                switch (stance) {
                    ...
                    case THIEF:
                        ...
                        temp =
                                (chr.getLevel() + 40)
                                        - Math.max(80, scStat + chr.getDex() + dex - eqpDex);
                        if (temp < 0) temp = 0;
                        temp = Math.min(tempAp, temp);
                        scStat += temp;
                        tempAp -= temp;

                        // thieves will upgrade STR as well only if a level-based threshold is
                        // reached.
                        if (chr.getStr() >= Math.max(13, (int) (0.4 * chr.getLevel()))) {
                            ...
                            temp =
                                    (20 + (chr.getLevel() / 2))
                                            - Math.max(50, trStat + chr.getStr() + str - eqpStr);
                            if (temp < 0) temp = 0;
                            temp = Math.min(tempAp, temp);
                            trStat += temp;
                            tempAp -= temp;
                ...
                c.announce(
                        MaplePacketCreator.serverNotice(
                                1,
                                "Better AP applications detected:\nSTR: +"
                                        + statGain[0]
                                        + "\nDEX: +"
                                        + statGain[1]
                                        + "\nINT: +"
                ...
                if (slea.available() < 16) {
                    AutobanFactory.PACKET_EDIT.alert(
                            chr, "Didn't send full packet for Auto Assign.");

                    final MapleClient client = c;
                    Thread t = new Thread(() -> client.disconnect(false, false));
                    t.start();
                ...
                for (int i = 0; i < 2; ++i) {
                    ...
                    gainStatByType(
                            MapleStat.getBy5ByteEncoding(type), statGain, tempVal, statUpdate);
                }
            ...
            switch (APFrom) {
                ...
                case 2048: // HP
                    ...
                    if (player.getHpMpApUsed() < 1) {
                        player.message(
                                "You don't have enough HPMP stat points to spend on AP Reset.");
                        c.announce(MaplePacketCreator.enableActions());
                        return false;
                    }
                ...
                default:
                    c.announce(
                            MaplePacketCreator.updatePlayerStats(
                                    MaplePacketCreator.EMPTY_STATUPDATE, true, player));
                    return false;
            }
    ...
    private static boolean addStat(MapleCharacter chr, int apTo, boolean usedAPReset) {
        switch (apTo) {
            ...
            default:
                chr.announce(
                        MaplePacketCreator.updatePlayerStats(
                                MaplePacketCreator.EMPTY_STATUPDATE, true, chr));
                return false;
        }
        return true;
    }
    ...
    private static int calcHpChange(MapleCharacter player, boolean usedAPReset) {
        ...
        if (job.isA(MapleJob.WARRIOR) || job.isA(MapleJob.DAWNWARRIOR1)) {
            ...
            if (ServerConstants.USE_RANDOMIZE_HPMP_GAIN) {
                MaxHP += usedAPReset ? 20 : Randomizer.rand(18, 22);
            } else {
                MaxHP += 20;
            }
        } else if (job.isA(MapleJob.ARAN1)) {
            if (ServerConstants.USE_RANDOMIZE_HPMP_GAIN) {
                MaxHP += usedAPReset ? 20 : Randomizer.rand(26, 30);
            } else {
                MaxHP += 28;
            }
        } else if (job.isA(MapleJob.MAGICIAN) || job.isA(MapleJob.BLAZEWIZARD1)) {
            if (ServerConstants.USE_RANDOMIZE_HPMP_GAIN) {
                MaxHP += usedAPReset ? 6 : Randomizer.rand(5, 9);
            } else {
                MaxHP += 6;
            }
        } else if (job.isA(MapleJob.THIEF) || job.isA(MapleJob.NIGHTWALKER1)) {
            if (ServerConstants.USE_RANDOMIZE_HPMP_GAIN) {
                MaxHP += usedAPReset ? 16 : Randomizer.rand(14, 18);
            } else {
                MaxHP += 16;
            }
            ...
            if (ServerConstants.USE_RANDOMIZE_HPMP_GAIN) {
                MaxHP += usedAPReset ? 18 : Randomizer.rand(16, 20);
            } else {
                MaxHP += 18;
            }
        } else if (usedAPReset) {
            ...
            MaxHP += ServerConstants.USE_RANDOMIZE_HPMP_GAIN ? Randomizer.rand(8, 12) : 10;
        }
    ...
    private static int calcMpChange(MapleCharacter player, boolean usedAPReset) {
        ...
        if (job.isA(MapleJob.WARRIOR)
            ...
            if (ServerConstants.USE_RANDOMIZE_HPMP_GAIN) {
                MaxMP += !usedAPReset ? Randomizer.rand(2, 4) + (player.getInt() / 10) : 2;
            } else {
                MaxMP += 3;
            }
        } else if (job.isA(MapleJob.MAGICIAN) || job.isA(MapleJob.BLAZEWIZARD1)) {
            ...
            if (ServerConstants.USE_RANDOMIZE_HPMP_GAIN) {
                MaxMP += !usedAPReset ? Randomizer.rand(12, 16) + (player.getInt() / 20) : 18;
            } else {
                MaxMP += 18;
            }
        } else if (job.isA(MapleJob.BOWMAN) || job.isA(MapleJob.WINDARCHER1)) {
            if (ServerConstants.USE_RANDOMIZE_HPMP_GAIN) {
                MaxMP += !usedAPReset ? Randomizer.rand(6, 8) + (player.getInt() / 10) : 10;
            } else {
                MaxMP += 10;
            }
        } else if (job.isA(MapleJob.THIEF) || job.isA(MapleJob.NIGHTWALKER1)) {
            ...
            if (ServerConstants.USE_RANDOMIZE_HPMP_GAIN) {
                MaxMP += !usedAPReset ? Randomizer.rand(7, 9) + (player.getInt() / 10) : 14;
            } else {
                MaxMP += 14;
            }
        } else {
            if (ServerConstants.USE_RANDOMIZE_HPMP_GAIN) {
                MaxMP += !usedAPReset ? Randomizer.rand(4, 6) + (player.getInt() / 10) : 6;
            } else {
                MaxMP += 6;
            }
        }
