...
public class BuybackProcessor {
    ...
    public static void processBuyback(MapleClient c) {
        ...
        if (buyback) {
            ...
            List<LifeMovementFragment> moveUpdate = Collections.singletonList(alm);

            MapleMap map = chr.getMap();
            map.broadcastMessage(
                    chr, MaplePacketCreator.movePlayer(c.getPlayer().getId(), moveUpdate), false);

            map.broadcastMessage(MaplePacketCreator.playSound("Buyback/" + jobString));
            map.broadcastMessage(
                    MaplePacketCreator.earnTitleMessage(
                            chr.getName() + " just bought back into the game!"));

            chr.announce(MaplePacketCreator.showBuybackEffect());
            map.broadcastMessage(
                    chr, MaplePacketCreator.showForeignBuybackEffect(chr.getId()), false);
        }
    }
}
