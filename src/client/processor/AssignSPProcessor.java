...
public class AssignSPProcessor {
    ...
    public static void SPAssignAction(MapleClient c, int skillid) {
        c.lockClient();
        try {
            if (skillid == Aran.HIDDEN_FULL_DOUBLE
                    || skillid == Aran.HIDDEN_FULL_TRIPLE
                    || skillid == Aran.HIDDEN_OVER_DOUBLE
            ...
            if ((!GameConstants.isPqSkillMap(player.getMapId()) && GameConstants.isPqSkill(skillid))
                ...
                AutobanFactory.PACKET_EDIT.alert(
                        player, "tried to packet edit in distributing sp.");
                ...
                Thread t = new Thread(() -> client.disconnect(true, false));
                t.start();
                    ...
                    total +=
                            player.getSkillLevel(
                                    SkillFactory.getSkill(
                                            player.getJobType() * 10000000 + 1000 + i));
                }
            ...
            if ((remainingSp > 0
                ...
                if (skill.getId() == Aran.FULL_SWING) {
                    ...
                    player.changeSkillLevel(
                            SkillFactory.getSkill(Aran.HIDDEN_FULL_DOUBLE),
                            player.getSkillLevel(skill),
                            player.getMasterLevel(skill),
                            player.getSkillExpiration(skill));
                    player.changeSkillLevel(
                            SkillFactory.getSkill(Aran.HIDDEN_FULL_TRIPLE),
                            player.getSkillLevel(skill),
                            player.getMasterLevel(skill),
                            player.getSkillExpiration(skill));
                    ...
                    player.changeSkillLevel(
                            ...
                            player.getSkillExpiration(skill));
                    player.changeSkillLevel(
                            SkillFactory.getSkill(Aran.HIDDEN_OVER_DOUBLE),
                            player.getSkillLevel(skill),
                            player.getMasterLevel(skill),
                            player.getSkillExpiration(skill));
                    player.changeSkillLevel(
                            SkillFactory.getSkill(Aran.HIDDEN_OVER_TRIPLE),
                            player.getSkillLevel(skill),
                            player.getMasterLevel(skill),
                            player.getSkillExpiration(skill));
                } else {
