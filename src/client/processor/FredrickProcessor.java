...
public class FredrickProcessor {
    private static boolean canRetrieveFromFredrick(
        ...
        return chr.getMeso() + chr.getMerchantMeso() >= 0
                && MapleInventory.checkSpotsAndOwnership(chr, items);
    }
    ...
    public static void fredrickRetrieveItems(
            MapleClient c) { // thanks Gustav for pointing out the dupe on Fredrick handling
        c.lockClient();
        try {
            ...
            try {
                ...
                if (deleteFredrickItems(chr.getId())) {
                    ...
                    for (Pair<Item, MapleInventoryType> it : items) {
                        ...
                        FilePrinter.print(
                                FilePrinter.FREDRICK + chr.getName() + ".txt",
                                chr.getName()
                                        + " gained "
                                        + item.getQuantity()
                                        + ' '
