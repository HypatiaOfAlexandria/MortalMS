...
public class MaplePlayerNPC extends AbstractMapleMapObject {
    ...
    private static final Map<Pair<Integer, Integer>, AtomicInteger> runningWorldJobRank =
            new HashMap<>();
    ...
    private static void getRunningWorldRanks(Connection con) throws SQLException {
        ...
        PreparedStatement ps =
                con.prepareStatement(
                        "SELECT world, max(worldrank) FROM playernpcs GROUP BY world ORDER BY world");
        ResultSet rs = ps.executeQuery();
    ...
    private static void getRunningWorldJobRanks(Connection con) throws SQLException {
        PreparedStatement ps =
                con.prepareStatement(
                        "SELECT world, job, max(worldjobrank) FROM playernpcs GROUP BY world, job ORDER BY world, job");
        ResultSet rs = ps.executeQuery();

        while (rs.next()) {
            runningWorldJobRank.put(
                    new Pair<>(rs.getInt(1), rs.getInt(2)), new AtomicInteger(rs.getInt(3) + 1));
        }
    ...
    public static boolean canSpawnPlayerNpc(String name, int mapid) {
        ...
        try {
            ...
            PreparedStatement ps =
                    con.prepareStatement(
                            "SELECT name FROM playernpcs WHERE name LIKE ? AND map = ?");
            ps.setString(1, name);
            ps.setInt(2, mapid);
    ...
    public void updatePlayerNPCPosition(MapleMap map, Point newPos) {
        ...
        try {
            ...
            PreparedStatement ps =
                    con.prepareStatement(
                            "UPDATE playernpcs SET x = ?, cy = ?, fh = ?, rx0 = ?, rx1 = ? WHERE id = ?");
    ...
    private static void fetchAvailableScriptIdsFromDb(byte branch, List<Integer> list) {
        try {
            ...
            PreparedStatement ps =
                    con.prepareStatement(
                            "SELECT scriptid FROM playernpcs WHERE scriptid >= ? AND scriptid < ? ORDER BY scriptid");
    ...
    private static int getNextScriptId(byte branch) {
        List<Integer> availablesBranch =
                availablePlayerNpcScriptIds.computeIfAbsent(branch, k -> new ArrayList<>(20));
    ...
    private static MaplePlayerNPC createPlayerNPCInternal(
            MapleMap map, Point pos, MapleCharacter chr) {
        int mapId = map.getId();
        ...
        if (pos == null) {
            pos =
                    GameConstants.isPodiumHallOfFameMap(map.getId())
                            ? MaplePlayerNPCPodium.getNextPlayerNpcPosition(map)
                            : MaplePlayerNPCPositioner.getNextPlayerNpcPosition(map);
        ...
        try {
            Connection con = DatabaseConnection.getConnection();
            PreparedStatement ps =
                    con.prepareStatement("SELECT * FROM playernpcs WHERE scriptid = ?");
    ...
    private static List<Integer> removePlayerNPCInternal(MapleMap map, MapleCharacter chr) {
        ...
        try {
            ...
            PreparedStatement ps =
                    con.prepareStatement(
                            "SELECT id, map FROM playernpcs WHERE name LIKE ?"
                                    + (map != null ? " AND map = ?" : ""));
            ps.setString(1, chr.getName());
    ...
    private static synchronized Pair<MaplePlayerNPC, List<Integer>> processPlayerNPCInternal(
            MapleMap map, Point pos, MapleCharacter chr, boolean create) {
        if (create) {
    ...
    public static boolean spawnPlayerNPC(int mapid, Point pos, MapleCharacter chr) {
        ...
        MaplePlayerNPC pn =
                processPlayerNPCInternal(
                                chr.getClient().getChannelServer().getMapFactory().getMap(mapid),
                                pos,
                                chr,
                                true)
                        .getLeft();
    ...
    private static MaplePlayerNPC getPlayerNPCFromWorldMap(String name, int world, int map) {
        ...
        for (MapleMapObject pnpcObj :
            ...
            if (name.contentEquals(pn.name) && pn.scriptId < 9977777) {
                return pn;
            }
        }
    ...
    public static void removeAllPlayerNPC() {
        try {
            ...
            PreparedStatement ps =
                    con.prepareStatement("SELECT DISTINCT world, map FROM playernpcs");
            ResultSet rs = ps.executeQuery();
            ...
            while (rs.next()) {
                ...
                for (Channel channel : w.getChannels()) {
                    ...
                    for (MapleMapObject pnpcObj :
                        ...
                        m.removeMapObject(pn);
                        m.broadcastMessage(
                                MaplePacketCreator.removeNPCController(pn.getObjectId()));
                        m.broadcastMessage(MaplePacketCreator.removePlayerNPC(pn.getObjectId()));
                    }
                }
            }
