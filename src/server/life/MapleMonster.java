...
public class MapleMonster extends AbstractLoadedMapleLife {
    private ChangeableStats ostats =
            null; // unused, v83 WZs offers no support for changeable stats.
    ...
    private final EnumMap<MonsterStatus, MonsterStatusEffect> stati =
            new EnumMap<>(MonsterStatus.class);
    ...
    private MonitoredReentrantLock externalLock =
            MonitoredReentrantLockFactory.createLock(MonitoredLockType.MOB_EXT);
    private MonitoredReentrantLock monsterLock =
            MonitoredReentrantLockFactory.createLock(MonitoredLockType.MOB, true);
    private MonitoredReentrantLock statiLock =
            MonitoredReentrantLockFactory.createLock(MonitoredLockType.MOB_STATI);
    private MonitoredReentrantLock animationLock =
            MonitoredReentrantLockFactory.createLock(MonitoredLockType.MOB_ANI);
    ...
    private boolean applyAnimationIfRoaming(
            int attackPos, MobSkill skill) { // roam: not casting attack or skill animations
            ...
            animationTime =
                    skill == null
                            ? MapleMonsterInformationProvider.getInstance()
                                    .getMobAttackAnimationTime(this.getId(), attackPos)
                            : MapleMonsterInformationProvider.getInstance()
                                    .getMobSkillAnimationTime(skill);

            return animationTime <= 0
                    || map.getChannelServer()
                            .registerMobOnAnimationEffect(
                                    map.getId(), this.hashCode(), animationTime);
        } finally {
    ...
    public synchronized void
            disposeMapObject() { // mob is no longer associated with the map it was in
        hp.set(-1);
    }
        ...
        if (hasBossHPBar()) {
            ...
            from.getMap()
                    .broadcastBossHpMessage(
                            this, this.hashCode(), makeBossHPBarPacket(), getPosition());
        } else if (!isBoss()) {
            int remainingHP = (int) Math.max(1, hp.get() * 100.0f / getMaxHp());
            ...
            if (from.getParty() != null) {
                for (MaplePartyCharacter mpc : from.getParty().getMembers()) {
                    MapleCharacter member =
                            from.getMap().getCharacterById(mpc.getId()); // god bless
                    if (member != null) {
    ...
    public void heal(int hp, int mp) {
        ...
        if (hp > 0) {
            map.broadcastMessage(
                    MaplePacketCreator.healMonster(getObjectId(), hp, getHp(), getMaxHp()));
        }
    ...
    private void distributeExperienceToParty(
            ...
            float exp,
            int killer,
            Set<MapleCharacter> underleveled,
            int minThresholdLevel) {
        List<MapleCharacter> members = new ArrayList<>();
        MapleCharacter pchar = map.getAnyCharacterFromParty(pid);
        if (pchar != null) {
            members.addAll(pchar.getPartyMembersOnSameMap());
        } else {
            MapleCharacter chr = map.getCharacterById(killer);
            if (chr == null) return;
        ...
        for (MapleCharacter mc : members) {
            if (mc.getLevel()
                    >= minThresholdLevel) { // NO EXP WILL BE GIVEN for those who are underleveled!
    ...
    private int calcThresholdLevel(boolean isPqMob) {
        ...
        return getLevel()
                - (!isBoss()
                        ? ServerConstants.MIN_UNDERLEVEL_TO_EXP_GAIN
                        : 2 * ServerConstants.MIN_UNDERLEVEL_TO_EXP_GAIN);
    }
    ...
    private void distributeExperience(int killerId) {
        ...
        float exp8perHp =
                (0.8f * exp) / totalHealth; // 80% of pool is split amongst all the damagers
        ...
        for (Entry<Integer, Float> party : partyExp.entrySet()) {
            distributeExperienceToParty(
                    party.getKey(), party.getValue(), killerId, underleveled, minThresholdLevel);
        }
    ...
    private void giveExpToCharacter(
            MapleCharacter attacker, float exp, boolean isKiller, int numExpSharers) {
            ...
            if (exp <= Integer.MAX_VALUE) { // assuming no negative xp here
                if (partyModifier > 0.0f) {
                    partyExp =
                            (int)
                                    (personalExp
                                            * partyModifier
                                            * ServerConstants.PARTY_BONUS_EXP_RATE);
                }
                ...
                try {
                    if (stati.containsKey(MonsterStatus.SHOWDOWN)) {
                        personalExp *=
                                (stati.get(MonsterStatus.SHOWDOWN)
                                                        .getStati()
    ...
    public MapleCharacter killBy(final MapleCharacter killer) {
        ...
        if (chrController
                != null) { // this can/should only happen when a hidden gm attacks the monster
        ...
        if (toSpawn != null) {
            ...
            if (timeMob != null) {
                if (toSpawn.contains(timeMob.getLeft())) {
                    reviveMap.broadcastMessage(
                            MaplePacketCreator.serverNotice(6, timeMob.getRight()));
                }

                if (timeMob.getLeft() == 9300338
                        && (reviveMap.getId() >= 922240100 && reviveMap.getId() <= 922240119)) {
            ...
            if (!toSpawn.isEmpty()) {
                final EventInstanceManager eim = map.getEventInstance();
                        ...
                        .schedule(
                                () -> {
                                    for (Integer mid : toSpawn) {
                                        ...
                                        if (mob.getId() >= 8810010
                                                && mob.getId() <= 8810017
                                                && reviveMap.isHorntailDefeated()) {
                                            ...
                                            for (int i = 8810017; i >= 8810010; i--)
                                                reviveMap.killMonster(
                                                        reviveMap.getMonsterById(i), killer, true);
                                        }
    ...
    public void switchController(MapleCharacter newController, boolean immediateAggro) {
        ...
        if (controllers != null) {
            ...
            controllers
                    .getClient()
                    .announce(MaplePacketCreator.stopControllingMonster(getObjectId()));
        }
    ...
    public byte[] makeBossHPBarPacket() {
        return MaplePacketCreator.showBossHP(
                getId(), getHp(), getMaxHp(), getTagColor(), getTagBgColor());
    }
    ...
    public boolean applyStatus(
            MapleCharacter from, final MonsterStatusEffect status, boolean poison, long duration) {
    ...
    public boolean applyStatus(
        ...
        } else if (status.getSkill().getId() == NightLord.VENOMOUS_STAR
                || status.getSkill().getId() == Shadower.VENOMOUS_STAB
        ...
        final Runnable cancelTask =
                () -> {
                    if (isAlive()) {
                        byte[] packet =
                                MaplePacketCreator.cancelMonsterStatus(
                                        getObjectId(), status.getStati());
                        map.broadcastMessage(packet, getPosition());
        ...
        if (poison) {
            ...
            int poisonDamage =
                    Math.min(Short.MAX_VALUE, (int) (getMaxHp() / (70.0 - poisonLevel) + 0.999));
            ...
            if (from.getJob() == MapleJob.NIGHTLORD
                ...
                int skillid =
                        (jobid == 412
                                ? NightLord.VENOMOUS_STAR
                                : (jobid == 422 ? Shadower.VENOMOUS_STAB : NightWalker.VENOM));
                ...
                int poisonDamage = 0;
                for (int i = 0; i < this.VenomMultiplier; i++) {
            ...
            final int damage =
                    (int)
                            ((from.getStr() + from.getLuk())
                                    * ((3.7 * skill.getEffect(level).getDamage()) / 100));
    ...
    public void applyMonsterBuff(
        ...
        final Runnable cancelTask =
                () -> {
                    if (isAlive()) {
                        byte[] packet =
                                MaplePacketCreator.cancelMonsterStatus(getObjectId(), stats);
                        map.broadcastMessage(packet, getPosition());
    ...
    private void debuffMobStat(MonsterStatus stat) {
        ...
        try {
            if (isBuffed(stat)) {
                final MonsterStatusEffect oldEffect = stati.get(stat);
                byte[] packet =
                        MaplePacketCreator.cancelMonsterStatus(getObjectId(), oldEffect.getStati());
                map.broadcastMessage(packet, getPosition());
            ...
            if (skillid == Hermit.SHADOW_MESO) {
                ...
                int i =
                        (skillid == Crusader.ARMOR_CRASH
                                ? 1
                                : (skillid == WhiteKnight.MAGIC_CRASH ? 2 : 0));
    ...
    public boolean canUseSkill(MobSkill toUse) {
        ...
        if (toUse.getLimit() > 0) {
            ...
            try {
                Integer times =
                        this.skillsUsed.get(new Pair<>(toUse.getSkillId(), toUse.getSkillLevel()));
    ...
    public int canUseAttack(int attackPos) {
        ...
        try {
            ...
            Pair<Integer, Integer> attackInfo =
                    MapleMonsterInformationProvider.getInstance()
                            .getMobAttackInfo(this.getId(), attackPos);
            if (attackInfo == null) {
    ...
    private final class DamageTask implements Runnable {
        ...
        private DamageTask(
                int dealDamage, MapleCharacter chr, MonsterStatusEffect status, int type) {
        ...
        public void run() {
            ...
            if (damage > 0) {
                ...
                if (type == 1) {
                    map.broadcastMessage(
                            MaplePacketCreator.damageMonster(getObjectId(), damage), getPosition());
                } else if (type == 2) {
                    if (damage < dealDamage) { // ninja ambush (type 2) is already displaying DOT to
                        // the caster
                        map.broadcastMessage(
    ...
    private static float getDifficultyRate(final int difficulty) {
        switch (difficulty) {
            case 6:
                return (7.7f);
            case 5:
