...
public class MobSkillFactory {
    ...
    private static final ReentrantReadWriteLock dataLock =
            new MonitoredReentrantReadWriteLock(MonitoredLockType.MOBSKILL_FACTORY);
    private static final ReadLock rL = dataLock.readLock();
    private static final WriteLock wL = dataLock.writeLock();

    public static MobSkill getMobSkill(final int skillId, final int level) {
        final String key = skillId + String.valueOf(level);
        rL.lock();
        try {
        ...
        try {
            ...
            if (ret == null) {
                ...
                mobSkills.put(skillId + String.valueOf(level), ret);
            }
            return ret;
        } finally {
            wL.unlock();
