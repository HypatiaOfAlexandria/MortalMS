...
public class ChangeableStats extends OverrideMonsterStats {
    ...
    public ChangeableStats(
            MapleMonsterStats stats, int newLevel, boolean pqMob) { // here we go i think
        final double mod = (double) newLevel / stats.getLevel();
        final double hpRatio = (double) stats.getHp() / stats.getExp();
                        ...
                        (int)
                                Math.round(
                                        (!stats.isBoss()
                                                        ...
                                                        : (stats.getHp() * mod))
                                                * pqMod),
                        Integer.MAX_VALUE); // right here lol
        exp =
                Math.min(
                        (int)
                                Math.round(
