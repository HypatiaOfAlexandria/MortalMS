...
public class MaplePlayerNPCPositioner {
    ...
    private static boolean isPlayerNpcNearby(
            List<Point> otherPos, Point searchPos, int xLimit, int yLimit) {
        int xLimit2 = xLimit / 2, yLimit2 = yLimit / 2;

        Rectangle searchRect =
                new Rectangle(searchPos.x - xLimit2, searchPos.y - yLimit2, xLimit, yLimit);
    ...
    private static int calcDy(int newStep) {
        return (ServerConstants.PLAYERNPC_AREA_Y / 2)
                + (ServerConstants.PLAYERNPC_AREA_Y / (1 << (newStep + 1)));
    }

    private static List<Point> rearrangePlayerNpcPositions(
            MapleMap map, int newStep, int pnpcsSize) {
        Rectangle mapArea = map.getMapArea();

        int leftPx = mapArea.x + ServerConstants.PLAYERNPC_INITIAL_X,
                px,
                py = mapArea.y + ServerConstants.PLAYERNPC_INITIAL_Y;
        int outx = mapArea.x + mapArea.width - ServerConstants.PLAYERNPC_INITIAL_X,
                outy = mapArea.y + mapArea.height;
        int cx = calcDx(newStep), cy = calcDy(newStep);

        List<Point> otherPlayerNpcs = new ArrayList<>();
        while (py < outy) {
            px = leftPx;
    ...
    private static Point rearrangePlayerNpcs(
            MapleMap map, int newStep, List<MaplePlayerNPC> pnpcs) {
        Rectangle mapArea = map.getMapArea();
    ...
    private static Point reorganizePlayerNpcs(
            MapleMap map, int newStep, List<MapleMapObject> mmoList) {
        if (!mmoList.isEmpty()) {
    ...
    private static Point getNextPlayerNpcPosition(
            MapleMap map, int initStep) { // automated playernpc position thanks to Ronan
        ...
        if (i > initStep) {
            map.getWorldServer()
                    .setPlayerNpcMapStep(map.getId(), ServerConstants.PLAYERNPC_AREA_STEPS - 1);
        }
        return null;
    }
