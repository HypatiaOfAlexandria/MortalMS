...
public class MapleMonsterInformationProvider {
    ...
    public List<MonsterDropEntry> retrieveEffectiveDrop(final int monsterId) {
        ...
        if (hasNoMultiEquipDrops.contains(monsterId)
                || !ServerConstants.USE_MULTIPLE_SAME_EQUIP_DROP) {
            return list;
        }

        List<MonsterDropEntry> multiDrops = extraMultiEquipDrops.get(monsterId),
                extra = new ArrayList<>();
        if (multiDrops == null) {
            multiDrops = new ArrayList<>();
            ...
            for (MonsterDropEntry mde : list) {
                if (ItemConstants.isEquipment(mde.itemId) && mde.Maximum > 1) {
                    ...
                    for (int i = 0; i < rnd - 1; i++) {
                        extra.add(
                                mde); // this passes copies of the equips' MDE with min/max quantity
                        // > 1, but idc it'll be unused anyways
                    }
                }
            }
        ...
        List<MonsterDropEntry> ret = new ArrayList<>(list);
        ret.addAll(extra);
    ...
    public final List<MonsterDropEntry> retrieveDrop(final int monsterId) {
        ...
        final List<MonsterDropEntry> ret = new ArrayList<>();
    ...
    public final List<Integer> retrieveDropPool(
            final int monsterId) { // ignores Quest and Party Quest items
    ...
    public static ArrayList<Pair<Integer, String>> getMobsIDsFromName(String search) {
        MapleDataProvider dataProvider =
                MapleDataProviderFactory.getDataProvider(new File("wz/String.wz"));
