...
public class MobAttackInfoFactory {
    ...
    public static MobAttackInfo getMobAttackInfo(MapleMonster mob, int attack) {
        MobAttackInfo ret = mobAttacks.get(mob.getId() + String.valueOf(attack));
        if (ret != null) {
            return ret;
        }
        synchronized (mobAttacks) {
            ret = mobAttacks.get(mob.getId() + String.valueOf(attack));
            if (ret == null) {
                MapleData mobData =
                        dataSource.getData(
                                StringUtil.getLeftPaddedStr(mob.getId() + ".img", '0', 11));
                if (mobData != null) {
                    ...
                    MapleData attackData =
                            mobData.getChildByPath("attack" + (attack + 1) + "/info");
                ...
                mobAttacks.put(mob.getId() + String.valueOf(attack), ret);
            }
            return ret;
        }
    }
