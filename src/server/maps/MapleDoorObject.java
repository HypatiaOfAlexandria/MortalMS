...
public class MapleDoorObject extends AbstractMapleMapObject {
    ...
    private final ReentrantReadWriteLock locks =
            new MonitoredReentrantReadWriteLock(MonitoredLockType.PLAYER_DOOR, true);
    private final ReentrantReadWriteLock.ReadLock rlock = locks.readLock();
    private final ReentrantReadWriteLock.WriteLock wlock = locks.writeLock();

    public MapleDoorObject(
            int owner,
            ...
            if (client.getPlayer().getParty() != null
                ...
                client.announce(
                        MaplePacketCreator.partyPortal(
                                from.getId(), to.getId(), this.toPosition()));
            }

            client.announce(
                    MaplePacketCreator.spawnPortal(from.getId(), to.getId(), this.toPosition()));
            if (!this.inTown()) {
                client.announce(MaplePacketCreator.spawnDoor(ownerId, this.getPosition(), true));
            }
        }
    }
        ...
        if (from.getId() == client.getPlayer().getMapId()) {
            if (client.getPlayer().getParty() != null
                    && (ownerId == client.getPlayer().getId()
                            || client.getPlayer().getParty().getMemberById(ownerId) != null)) {
                client.announce(
                        MaplePacketCreator.partyPortal(999999999, 999999999, new Point(-1, -1)));
            }
