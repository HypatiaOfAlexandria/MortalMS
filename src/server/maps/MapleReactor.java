...
public class MapleReactor extends AbstractMapleMapObject {
    ...
    private final Lock reactorLock =
            MonitoredReentrantLockFactory.createLock(MonitoredLockType.REACTOR, true);
    private final Lock hitLock =
            MonitoredReentrantLockFactory.createLock(MonitoredLockType.REACTOR_HIT, true);
    ...
    public void forceHitReactor(final byte newState) {
        ...
        try {
            ...
            map.broadcastMessage(MaplePacketCreator.triggerReactor(this, 0));
        } finally {
            this.unlockReactor();
        }
    }
    ...
    private void tryForceHitReactor(
            final byte newState) { // weak hit state signal, if already changed reactor state before
        // timeout then drop this
        ...
        try {
            ...
            if (hitLock.tryLock()) {
                ...
                try {
                    ...
                    if (reactorType < 999
                            && reactorType
                                    != -1) { // type 2 = only hit from right (kerning swamp plants),
                        // 00 is air left 02 is ground left
                            ...
                            for (byte b = 0; b < stats.getStateSize(state); b++) { // YAY?
                                ...
                                if (stats.getNextState(state, b) == -1) { // end of reactor
                                    ...
                                    if (state
                                            == stats.getNextState(
                                                    state,
                                                    b)) { // current state = next state, looping
                                        // reactor
                                        ReactorScriptManager.getInstance().act(c, this);
                                    }

                                    // refresh collectability on item drop-based
                                    this.shouldCollect = true;
                                    // reactors
                                    refreshReactorTimeout();
