...
public class MapleMiniGame extends AbstractMapleMapObject {
    ...
    public boolean checkPassword(String sentPw) {
        return this.password.isEmpty()
                || sentPw.toLowerCase().contentEquals(this.password.toLowerCase());
    }
    ...
    public void addVisitor(MapleCharacter challenger) {
        visitor = challenger;
        MapleCharacter owner = this.owner;
    ...
    public void removeVisitor(MapleCharacter challenger) {
        if (visitor == challenger) {
            visitor = null;
            owner.getClient().announce(MaplePacketCreator.getMiniGameRemoveVisitor());
            if (GameType == MiniGameType.OMOK) {
                owner.getMap().broadcastMessage(MaplePacketCreator.addOmokBox(owner, 1, 0));
            } else if (GameType == MiniGameType.MATCH_CARD) {
                owner.getMap().broadcastMessage(MaplePacketCreator.addMatchCardBox(owner, 1, 0));
            }
        }
    }
    ...
    public void chat(MapleClient c, String chat) {
        broadcast(
                MaplePacketCreator.getPlayerShopChat(c.getPlayer(), chat, isOwner(c.getPlayer())));
    }
