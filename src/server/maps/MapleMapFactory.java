...
public class MapleMapFactory {
    private static final Map<Integer, Float> mapRecoveryRate = new HashMap<>();

    private final MapleDataProvider source;
    private final MapleData nameData;
    private EventInstanceManager event;
    private final Map<Integer, MapleMap> maps = new HashMap<>();
    private final ReadLock mapsRLock;
    private final WriteLock mapsWLock;
    private int channel;
    ...
    public MapleMapFactory(
        ...
        ReentrantReadWriteLock rrwl =
                new MonitoredReentrantReadWriteLock(MonitoredLockType.MAP_FACTORY);
    ...
    private synchronized MapleMap loadMapFromWz(int mapid, Integer omapid) {
        ...
        if (!link.isEmpty()) { // nexon made hundreds of dojo maps so to reduce the size they added
            // links.
        ...
        map =
                new MapleMap(
                        ...
                        world,
                        channel,
                        MapleDataTool.getInt("returnMap", infoData),
                        monsterRate);
        map.setEventInstance(event);

        String onFirstEnter =
                MapleDataTool.getString(
                        infoData.getChildByPath("onFirstUserEnter"), String.valueOf(mapid));
        map.setOnFirstUserEnter(onFirstEnter.isEmpty() ? String.valueOf(mapid) : onFirstEnter);

        String onEnter =
                MapleDataTool.getString(
                        infoData.getChildByPath("onUserEnter"), String.valueOf(mapid));
        ...
        map.setMobInterval(
                (short) MapleDataTool.getInt(infoData.getChildByPath("createMobInterval"), 5000));
        ...
        if (timeMob != null) {
            map.timeMob(
                    MapleDataTool.getInt(timeMob.getChildByPath("id")),
            ...
            try {
                ...
                try (PreparedStatement ps =
                        con.prepareStatement(
                                "SELECT * FROM playernpcs WHERE map = ? AND world = ?")) {
                    ps.setInt(1, omapid);
                    ps.setInt(2, world);
        ...
        try {
            map.setMapName(
                    MapleDataTool.getString(
                            "mapName", nameData.getChildByPath(getMapStringName(omapid)), ""));
            map.setStreetName(
                    MapleDataTool.getString(
        ...
        map.setForcedReturnMap(
                MapleDataTool.getInt(infoData.getChildByPath("forcedReturn"), 999999999));
        ...
        map.setMobCapacity(
                MapleDataTool.getIntConvert(
                        "fixedMobCapacity",
                        infoData,
                        500)); // Is there a map that contains more than 500 mobs?
    ...
    private static AbstractLoadedMapleLife loadLife(MapleData life, String id, String type) {
    ...
    private static MapleReactor loadReactor(MapleData reactor, String id) {
        MapleReactor myReactor =
                new MapleReactor(
                        MapleReactorFactory.getReactor(Integer.parseInt(id)), Integer.parseInt(id));
    ...
    private static String getMapName(int mapid) {
        ...
        String builder = "Map/Map" + area + '/' + mapName + ".img";
        mapName = builder;
        return mapName;
    }
    ...
    private static String getMapStringName(int mapid) {
        ...
        if (mapid < 100000000) {
            ...
            if ((mapid >= 674030000 && mapid < 674040000)
                    || (mapid >= 680100000 && mapid < 680200000)) {
                builder.append("etc");
            } else {
