                            ...
                            for (MapleData fknexon : eventData.getChildren()) {
                                if (fknexon.getName().equals("timeOut")) {
                                    ...
                                    if (type == 100) { // reactor waits for item
                                        reactItem =
                                                new Pair<>(
                                                        MapleDataTool.getIntConvert("0", fknexon),
                                                        MapleDataTool.getIntConvert("1", fknexon));
                                        if (!areaSet || loadArea) { // only set area of effect for
                                    ...
                                    MapleData activeSkillID =
                                            fknexon.getChildByPath("activeSkillID");
                                    ...
                                    byte nextState =
                                            (byte) MapleDataTool.getIntConvert("state", fknexon);
                                    statedatas.add(
                                            new StateData(type, reactItem, skillids, nextState));
                                }
                            }
