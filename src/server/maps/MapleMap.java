...
public class MapleMap {
    ...
    private final Collection<SpawnPoint> monsterSpawn =
            Collections.synchronizedList(new ArrayList<>());
    private final Collection<SpawnPoint> allMonsterSpawn =
            Collections.synchronizedList(new ArrayList<>());
    ...
    private final ArrayList<WeakReference<MapleMapObject>> registeredDrops = new ArrayList<>();
    ...
    private final Lock lootLock =
            MonitoredReentrantLockFactory.createLock(MonitoredLockType.MAP_LOOT, true);
    ...
    private static final Lock bndLock =
            MonitoredReentrantLockFactory.createLock(MonitoredLockType.MAP_BOUNDS, true);
    ...
    public MapleMap(int mapid, int world, int channel, int returnMapId, float monsterRate) {
        ...
        final ReentrantReadWriteLock chrLock =
                new MonitoredReentrantReadWriteLock(MonitoredLockType.MAP_CHRS, true);
        chrRLock = chrLock.readLock();
        chrWLock = chrLock.writeLock();

        final ReentrantReadWriteLock objectLock =
                new MonitoredReentrantReadWriteLock(MonitoredLockType.MAP_OBJS, true);
    ...
    private void spawnAndAddRangedMapObject(
            MapleMapObject mapobject, DelayedPacketCreation packetbakery) {
    ...
    private void spawnRangedMapObject(
            MapleMapObject mapobject,
            DelayedPacketCreation packetbakery,
            SpawnCondition condition) {
        chrRLock.lock();
    ...
    private int getUsableOID() {
        ...
        try {
            ...
            do {
                if ((curOid = runningOid.incrementAndGet())
                        < 0) { // clashes with playernpc on curOid >= 2147000000, developernpc uses
                    // >= 2147483000
    ...
    public void generateMapDropRangeCache() {
        ...
        try {
            ...
            if (bounds != null) {
                ...
                // assuming MINIMAP always have an equal-greater picture representation of the map
                // area (players won't walk beyond the area known by the minimap).
                Point lp = new Point(mapArea.x, mapArea.y),
                        rp = new Point(mapArea.x + mapArea.width, mapArea.y),
                        fallback = new Point(mapArea.x + (mapArea.width / 2), mapArea.y);
    ...
    /**
     * Fetches angle relative between spawn and door points where 3 O'Clock is 0 and 12 O'Clock is
     * 270 degrees
     *
     * @param spawnPoint
     * @param doorPoint
    ...
    private byte dropItemsFromMonsterOnMap(
        ...
        for (final MonsterDropEntry de : dropEntry) {
            ...
            if (Randomizer.nextInt(999999) < dropChance) {
                pos.x =
                        droptype == 3
                                ? mobpos + ((d % 2 == 0) ? (40 * ((d + 1) / 2)) : -(40 * (d / 2)))
                                : mobpos + ((d % 2 == 0) ? (25 * ((d + 1) / 2)) : -(25 * (d / 2)));
                if (de.itemId == 0) { // meso
                    ...
                    idrop =
                            ItemConstants.getInventoryType(de.itemId) == MapleInventoryType.EQUIP
                                    ? MapleItemInformationProvider.randomizeStats(
    ...
    private void dropFromMonster(
            final MapleCharacter chr, final MapleMonster mob, final boolean useBaseRate) {
        ...
        sortDropEntries(
                ServerConstants.USE_SPAWN_RELEVANT_LOOT
                        ? chr.retrieveRelevantDrops(mob.getId())
    ...
    public void dropItemsFromMonster(
            List<MonsterDropEntry> list, final MapleCharacter chr, final MapleMonster mob) {
        if (mob.dropsDisabled() || !dropsOn) {
            return;
        }
    ...
    private void startItemMonitor() {
        ...
        try {
            ...
            itemMonitor =
                    TimerManager.getInstance()
                            .register(
                                    ...
                                    ServerConstants.ITEM_MONITOR_TIME,
                                    ServerConstants.ITEM_MONITOR_TIME);

            expireItemsTask =
                    TimerManager.getInstance()
                            .register(
                                    this::makeDisappearExpiredItemDrops,
                                    ServerConstants.ITEM_EXPIRE_CHECK,
                                    ServerConstants.ITEM_EXPIRE_CHECK);
            ...
            if (ServerConstants.USE_SPAWN_LOOT_ON_ANIMATION) {
                ...
                mobSpawnLootTask =
                        TimerManager.getInstance().register(this::spawnMobItemDrops, 200, 200);
            }
    ...
    private void registerItemDrop(MapleMapItem mdrop) {
        droppedItems.put(
                mdrop,
                !everlast
                        ? Server.getInstance().getCurrentTime() + ServerConstants.ITEM_EXPIRE_TIME
                        : Long.MAX_VALUE);
    }
    ...
    private void makeDisappearExpiredItemDrops() {
        List<MapleMapItem> toDisappear = new ArrayList<>();
    ...
    private void registerMobItemDrops(
            ...
            Point pos,
            List<MonsterDropEntry> dropEntry,
            List<MonsterDropEntry> visibleQuestEntry,
            List<MonsterDropEntry> otherQuestEntry,
            List<MonsterGlobalDropEntry> globalEntry,
            MapleCharacter chr,
            MapleMonster mob) {
        MobLootEntry mle =
    ...
    private void spawnMobItemDrops() {
        ...
        long timeNow = Server.getInstance().getCurrentTime();
        List<MobLootEntry> toRemove = new ArrayList<>();
        ...
        if (!toRemove.isEmpty()) {
            List<MobLootEntry> toSpawnLoot = new ArrayList<>();
    ...
    private List<MapleMapItem> getDroppedItems() {
        objectRLock.lock();
        try {
            return new ArrayList<>(droppedItems.keySet());
        } finally {
            objectRLock.unlock();
        }
    }

    public void pickItemDrop(
            byte[] pickupPacket,
            MapleMapItem mdrop) { // mdrop must be already locked and not-pickedup checked by now
    ...
    public void updatePlayerItemDrops(
            int partyid,
            int charid,
        ...
        for (MapleMapItem mdrop : getDroppedItems()) {
            if (mdrop.getOwnerId() == charid) {
                ...
                try {
                    ...
                    byte[] removePacket =
                            MaplePacketCreator.silentRemoveItemFromMap(mdrop.getObjectId());
                    byte[] updatePacket =
                            MaplePacketCreator.updateMapItemObject(mdrop, partyLeaver == null);
                    ...
                    if (partyLeaver != null) {
                        if (this.equals(partyLeaver.getMap())) {
                            ...
                            if (partyLeaver.needQuestItem(mdrop.getQuest(), mdrop.getItemId())) {
                                partyLeaver.announce(
                                        MaplePacketCreator.updateMapItemObject(mdrop, true));
                            }
                        }
                    }
                } finally {
    ...
    private void spawnDrop(
        ...
        final MapleMapItem mdrop =
                new MapleMapItem(
                        idrop, dropPos, dropper, chr, chr.getClient(), droptype, false, questid);
    ...
    public final void spawnMesoDrop(
        ...
        final MapleMapItem mdrop =
                new MapleMapItem(
                        meso, droppos, dropper, owner, owner.getClient(), droptype, playerDrop);
        mdrop.setDropTime(Server.getInstance().getCurrentTime());
    ...
    public final void disappearingItemDrop(
        ...
        final MapleMapItem mdrop =
                new MapleMapItem(item, droppos, dropper, owner, owner.getClient(), (byte) 1, false);

        mdrop.lockItem();
        try {
            broadcastItemDropMessage(
                    mdrop, dropper.getPosition(), droppos, (byte) 3, mdrop.getPosition());
        } finally {
            mdrop.unlockItem();
        }
    }
    ...
    public boolean damageMonster(
            final MapleCharacter chr, final MapleMonster monster, final int damage) {
        ...
        if (monster.isAlive()) {
            ...
            if (monster.getStats().selfDestruction() != null
                    && monster.getStats().selfDestruction().getHp() > -1) { // should work ;p
                if (monster.getHp() <= monster.getStats().selfDestruction().getHp()) {
                    killMonster(
                            monster, chr, true, monster.getStats().selfDestruction().getAction());
                    return true;
                }
            }
            if (killed) {
    ...
    public void broadcastPinkBeanVictory(int channel) {
        getWorldServer()
                .dropMessage(
                        6,
                        "[VICTORY] In a swift stroke of sorts, the crew that has attempted Pink Bean at channel "
                                + channel
    ...
    public void killMonster(
            final MapleMonster monster, final MapleCharacter chr, final boolean withDrops) {
    ...
    public void killMonster(
        ...
        if (chr == null) {
            if (removeKilledMonsterObject(monster)) {
                ...
                broadcastMessage(
                        MaplePacketCreator.killMonster(monster.getObjectId(), animation),
                        monster.getPosition());
            }
        } else {
            if (removeKilledMonsterObject(monster)) {
                if (monster.getStats().getLevel() >= chr.getLevel() + 30 && !chr.isGM()) {
                    AutobanFactory.GENERAL.alert(
                            chr,
                            " for killing a "
                                    + monster.getName()
                ...
                if (buff > -1) {
                    ...
                    for (MapleMapObject mmo : this.getPlayers()) {
                        ...
                        if (character.isAlive()) {
                            ...
                            character
                                    .getClient()
                                    .announce(MaplePacketCreator.showOwnBuffEffect(buff, 1));
                            broadcastMessage(
                                    character,
                                    MaplePacketCreator.showBuffeffect(character.getId(), buff, 1),
                                    false);
            ...
            for (Object ob : list) {
                if (ob instanceof MapleMapObject) {
                    ...
                    if (mapobjects.containsValue(mmo)
                            && mmo.getType() == MapleMapObjectType.REACTOR) {
                        listObjects.add(mmo);
                    }
                }
            }
    ...
    public void updateMonsterController(MapleMonster monster) {
        ...
        try {
            ...
            if (newController
                    != null) { // was a new controller found? (if not no one is on the map)
    ...
    public MapleMonster getMonsterByOid(int oid) {
        ...
        return (mmo != null && mmo.getType() == MapleMapObjectType.MONSTER)
                ? (MapleMonster) mmo
                : null;
    }
    ...
    public MapleReactor getReactorByOid(int oid) {
        MapleMapObject mmo = getMapObject(oid);
        return (mmo != null && mmo.getType() == MapleMapObjectType.REACTOR)
                ? (MapleReactor) mmo
                : null;
    }
    ...
    private void monsterItemDrop(final MapleMonster m, long delay) {
        final ScheduledFuture<?> monsterItemDrop =
                TimerManager.getInstance()
                        .register(
                                () -> {
    ...
    public Point getGroundBelow(Point pos) {
        Point spos =
                new Point(
                        pos.x,
                        pos.y - 14); // Using -14 fixes spawning pets causing a lot of issues.
        spos = calcPointBelow(spos);
    ...
    public void spawnRevives(final MapleMonster monster) {
        monster.setMap(this);
        if (event != null) {
            event.registerMonster(monster);
        }

        spawnAndAddRangedMapObject(
                monster, c -> c.announce(MaplePacketCreator.spawnMonster(monster, false)));
    ...
    public void spawnMonster(final MapleMonster monster, int difficulty, boolean isPq) {
        ...
        spawnAndAddRangedMapObject(
                monster, c -> c.announce(MaplePacketCreator.spawnMonster(monster, true)), null);
        ...
        if (monster.getDropPeriodTime()
                > 0) { // 9300102 - Watchhog, 9300061 - Moon Bunny (HPQ), 9300093 - Tylus
            if (monster.getId() == 9300102) {
                ...
                monsterItemDrop(monster, monster.getDropPeriodTime());
            } else if (monster.getId() == 9400326
                    || monster.getId() == 9400331
                    || monster.getId() == 9400336) {
                monsterItemDrop(monster, monster.getDropPeriodTime());
            } else {
                FilePrinter.printError(
                        FilePrinter.UNHANDLED_EVENT,
                        "UNCODED TIMED MOB DETECTED: " + monster.getId() + System.lineSeparator());
            }
        }

        spawnedMonstersOnMap.incrementAndGet();
        applyRemoveAfter(
                monster); // thanks LightRyuzaki for pointing issues with spawned CWKPQ mobs not
        // applying this
    }
    ...
    public void spawnMonsterWithEffect(final MapleMonster monster, final int effect, Point pos) {
        ...
        spawnAndAddRangedMapObject(
                monster, c -> c.announce(MaplePacketCreator.spawnMonster(monster, true, effect)));
        updateMonsterController(monster);
    ...
    public void spawnFakeMonster(final MapleMonster monster) {
        ...
        spawnAndAddRangedMapObject(
                monster, c -> c.announce(MaplePacketCreator.spawnFakeMonster(monster, 0)));
                    ...
                    if (door.getFrom().getId() == c.getPlayer().getMapId()) {
                        if (c.getPlayer().getParty() != null
                                && (door.getOwnerId() == c.getPlayer().getId()
                                        || c.getPlayer().getParty().getMemberById(door.getOwnerId())
                                                != null)) {
                            c.announce(
                        ...
                        if (!door.inTown())
                            c.announce(
                                    MaplePacketCreator.spawnDoor(
                                            door.getOwnerId(), door.getPosition(), false));
                    }
    ...
    public MaplePortal getDoorPortal(int doorid) {
        ...
        if (doorPortal == null) {
            FilePrinter.printError(
                    FilePrinter.EXCEPTION,
                    "[DOOR] "
                            + mapName
                            + '('
                            + mapid
                            + ") does not contain door portalid "
                            + doorid);
                            ...
                            if (mist.makeChanceResult()) {
                                MonsterStatusEffect poisonEffect =
                                        new MonsterStatusEffect(
                                                ...
                                                false);
                                ((MapleMonster) mo)
                                        .applyStatus(mist.getOwner(), poisonEffect, true, duration);
                            }
                        }
                    };
    ...
    public final void spawnItemDrop(
        ...
        final MapleMapItem mdrop =
                new MapleMapItem(
                        item, droppos, dropper, owner, owner.getClient(), dropType, playerDrop);
        mdrop.setDropTime(Server.getInstance().getCurrentTime());
    ...
    public final void spawnItemDropList(
            List<Integer> list,
            final MapleMapObject dropper,
            final MapleCharacter owner,
            Point pos) {
    ...
    public final void spawnItemDropList(
        ...
        for (Integer aList : list) {
            if (aList == 0) {
                spawnMesoDrop(
                        owner != null ? 10 * owner.getMesoRate() : 10,
                        calcDropPos(dropPos, pos),
                        dropper,
                        owner,
                        playerDrop,
                        (byte) (ffaDrop ? 2 : 0));
                ...
                final Item drop;
                int randomedId = aList;

                drop =
                        ItemConstants.getInventoryType(randomedId) != MapleInventoryType.EQUIP
                                ? new Item(
                                        randomedId,
                                        (short) 0,
                                        (short) (rnd.nextInt(copies) + minCopies))
                                : MapleItemInformationProvider.randomizeStats(
                                        (Equip) ii.getEquipById(randomedId));
                    ...
                    if (!drop.isPickedUp()) {
                        ...
                        if (item != null
                            ...
                            if (reactArea.contains(drop.getPosition())) {
                                ...
                                if (owner != null) {
                                    registerMapSchedule(
                                            new ActivateItemReactor(drop, react, owner), 5000);
                                }
                            }
                        }
                    }
    ...
    public void startMapEffect(String msg, int itemId, long time) {
        ...
        Runnable r =
                () -> {
                    broadcastMessage(MapleMapEffect.makeDestroyData());
                    mapEffect = null;
                };
    ...
    public void addPlayer(final MapleCharacter chr) {
        ...
        if (chrSize == 1) {
            ...
            if (!onFirstUserEnter.isEmpty()
                    ...
                    && MapScriptManager.scriptExists(onFirstUserEnter, true)) {
                chr.enteredScript(onFirstUserEnter, mapid);
                MapScriptManager.getInstance()
                        .getMapScript(chr.getClient(), onFirstUserEnter, true);
            }
        }
        ...
        if (chr.getFitness() != null && chr.getFitness().isTimerStarted()) {
            chr.getClient()
                    .announce(
                            MaplePacketCreator.getClock(
                                    (int) (chr.getFitness().getTimeLeft() / 1000)));
        }
        ...
        if (carnival != null
                ...
                && (mapid == 980000101
                        ...
                        || mapid == 980000601)) {
            chr.getClient()
                    .announce(MaplePacketCreator.getClock((int) (carnival.getTimeLeft() / 1000)));
            chr.getClient()
                    .announce(MaplePacketCreator.startCPQ(chr, carnival.oppositeTeam(cparty)));
        }
        if (hasClock()) {
    ...
    */
     ...
     */
    public void broadcastMessage(
            MapleCharacter source, final byte[] packet, boolean repeatToSource, boolean ranged) {
        broadcastMessage(
                repeatToSource ? null : source,
                packet,
    ...
    private void broadcastMessage(
            MapleCharacter source, final byte[] packet, double rangeSq, Point rangedFrom) {
        chrRLock.lock();
        try {
            for (MapleCharacter chr : characters) {
                if (chr != source) {
    ...
    public void broadcastBossHpMessage(
            MapleMonster mm, int bossHash, final byte[] packet, Point rangedFrom) {
    ...
    private void broadcastItemDropMessage(
            MapleMapItem mdrop, Point dropperPos, Point dropPos, byte mod, Point rangedFrom) {
        broadcastItemDropMessage(mdrop, dropperPos, dropPos, mod, getRangedDistance(), rangedFrom);
    }

    private void broadcastItemDropMessage(
            MapleMapItem mdrop, Point dropperPos, Point dropPos, byte mod) {
    ...
    private void broadcastItemDropMessage(
        ...
        try {
            for (MapleCharacter chr : characters) {
                final byte[] packet =
                        MaplePacketCreator.dropItemFromMapObject(
                                chr.getParty() != null, mdrop, dropperPos, dropPos, mod);
    ...
    private void sendObjectPlacement(MapleClient mapleClient) {
        ...
        if (chr != null) {
            for (MapleMapObject o :
                    getMapObjectsInRange(
                            chr.getPosition(), getRangedDistance(), rangedMapobjectTypes)) {
                if (o.getType() == MapleMapObjectType.REACTOR) {
    ...
    public List<MapleMapObject> getMapObjectsInRange(
            Point from, double rangeSq, List<MapleMapObjectType> types) {
    ...
    public void addMonsterSpawn(MapleMonster monster, int mobTime, int team) {
        ...
        newpos.y -= 1;
        SpawnPoint sp =
                new SpawnPoint(monster, newpos, !monster.isMobile(), mobTime, mobInterval, team);
        monsterSpawn.add(sp);
        if (sp.shouldSpawn()
                || mobTime == -1) { // -1 does not respawn and should not either but force ONE spawn
    ...
    public void movePlayer(MapleCharacter player, Point newPosition) {
        ...
        for (MapleMapObject mo :
                getMapObjectsInRange(
                        player.getPosition(), getRangedDistance(), rangedMapobjectTypes)) {
    ...
    public boolean makeDisappearItemFromMap(MapleMapItem mapitem) {
        if (mapitem != null && mapitem == getMapObject(mapitem.getObjectId())) {
            ...
            try {
                ...
                MapleMap.this.pickItemDrop(
                        MaplePacketCreator.removeItemFromMap(mapitem.getObjectId(), 0, 0), mapitem);
                return true;
            } finally {
                mapitem.unlockItem();
            }
        ...
        public void run() {
            ...
            d =
                    dropItemsFromMonsterOnMap(
                            visibleQuestEntry, pos, d, chRate, droptype, mobpos, chr, mob);
        ...
        public void run() {
            ...
            try {
                if (reactor.getReactorType() == 100) {
                    if (reactor.getShouldCollect() == true
                            && mapitem != null
    ...
    public void instanceMapRespawn() {
        ...
        final int numShouldSpawn =
                (short) ((monsterSpawn.size() - spawnedMonstersOnMap.get())); // Fking lol'd
        if (numShouldSpawn > 0) {
    ...
    private int getNumShouldSpawn(int numPlayers) {
        ...
        */
        ...
        int maxNumShouldSpawn =
                (int) Math.ceil(getCurrentSpawnRate(numPlayers) * monsterSpawn.size());
    ...
    public final int getNumPlayersItemsInRect(final Rectangle rect) {
        ...
        int retI =
                getMapObjectsInBox(rect, Collections.singletonList(MapleMapObjectType.ITEM)).size();
    ...
    public void clearDrops(MapleCharacter player) {
        for (MapleMapObject i :
            ...
            this.broadcastMessage(
                    MaplePacketCreator.removeItemFromMap(i.getObjectId(), 0, player.getId()));
        }
    }
    ...
    public void startEvent(final MapleCharacter chr) {
        if (this.mapid == 109080000 && coconut == null) {
            this.coconut = new MapleCoconut(this);
            coconut.startEvent();
        } else if (this.mapid == 109040000) {
            chr.setFitness(new MapleFitness(chr));
            chr.getFitness().startFitness();
        } else if (this.mapid == 109030101
                || this.mapid == 109030201
                || this.mapid == 109030301
                || this.mapid == 109030401) {
            chr.setOla(new MapleOla(chr));
            chr.getOla().startOla();
        } else if (this.mapid == 109020001 && ox == null) {
            this.ox = new MapleOxQuiz(this);
            ox.sendQuestion();
            this.isOxQuiz = true;
    ...
    public boolean hasEventNPC() {
        return this.mapid == 60000
                || this.mapid == 104000000
                || this.mapid == 200000000
                || this.mapid == 220000000;
    }

    public boolean isStartingEventMap() {
        return this.mapid == 109040000
                || this.mapid == 109020001
                || this.mapid == 109010000
                || this.mapid == 109030001
