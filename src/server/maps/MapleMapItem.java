...
public class MapleMapItem extends AbstractMapleMapObject {
    ...
    private final Lock itemLock =
            MonitoredReentrantLockFactory.createLock(MonitoredLockType.MAP_ITEM);
    ...
    public final boolean canBePickedBy(MapleCharacter chr) {
        ...
        if (party_ownerid == -1) {
            ...
            if (chr.isPartyMember(character_ownerid)) {
    ...
    public final MapleClient getOwnerClient() {
        return (ownerClient.isLoggedIn() && !ownerClient.getPlayer().isAwayFromWorld())
                ? ownerClient
                : null;
    }
            ...
            try {
                client.announce(
                        MaplePacketCreator.dropItemFromMapObject(
                                chr.getParty() != null, this, null, getPosition(), (byte) 2));
            } finally {
                this.unlockItem();
            }
        }
