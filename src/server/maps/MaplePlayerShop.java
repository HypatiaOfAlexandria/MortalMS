...
public class MaplePlayerShop extends AbstractMapleMapObject {
    ...
    private final List<SoldItem> sold = new ArrayList<>();
    private String description;
    private int boughtnumber = 0;
    private final List<String> bannedList = new ArrayList<>();
    private final List<Pair<MapleCharacter, String>> chatLog = new ArrayList<>();
    private final Map<Integer, Byte> chatSlot = new LinkedHashMap<>();
    private final Lock visitorLock =
            MonitoredReentrantLockFactory.createLock(MonitoredLockType.VISITOR_PSHOP, true);
        ...
        if (visitor == owner) {
            ...
            try {
                for (int i = 0; i < 3; i++) {
                    if (visitors[i] != null && visitors[i].getId() == visitor.getId()) {
                        visitor.setSlot(
                                -1); // absolutely cant remove player slot for late players without
                        // dc'ing them... heh

                        for (int j = i; j < 2; j++) {
                            if (visitors[j] != null)
                                owner.announce(
                                        MaplePacketCreator.getPlayerShopRemoveVisitor(j + 1));
                        ...
                        for (int j = i; j < 2; j++) {
                            if (visitors[j] != null)
                                owner.announce(
                                        MaplePacketCreator.getPlayerShopNewVisitor(
                                                visitors[j], j + 1));
                        }

                        this.broadcastRestoreToVisitors();
                        owner.getMap()
                                .broadcastMessage(MaplePacketCreator.updatePlayerShopBox(this));
                        return;
                    }
                }
            } finally {
        ...
        synchronized (items) {
            ...
            if (shopItem.isExist()) {
                if (shopItem.getBundles() > 0) {
                    ...
                    iitem.setQuantity(
                            (short) (shopItem.getItem().getQuantity() * shopItem.getBundles()));
    ...
    public void buy(MapleClient c, int item, short quantity) {
        synchronized (items) {
            if (isVisitor(c.getPlayer())) {
                ...
                if (newItem.getInventoryType().equals(MapleInventoryType.EQUIP)
                ...
                try {
                    int price =
                            (int) Math.min((float) pItem.getPrice() * quantity, Integer.MAX_VALUE);
                    ...
                    if (c.getPlayer().getMeso() >= price) {
                        if (canBuy(c, newItem)) {
                            ...
                            SoldItem soldItem =
                                    new SoldItem(
                                            ...
                                            price);
                            owner.announce(
                                    MaplePacketCreator.getPlayerShopOwnerUpdate(soldItem, item));
                        ...
                        c.getPlayer()
                                .dropMessage(
                                        1, "You don't have enough mesos to purchase this item.");
                    }
                } finally {
                    visitorLock.unlock();
                }
        ...
        try {
            for (int i = 0; i < 3; i++) {
                if (visitors[i] != null) {
                    visitors[i]
                            .getClient()
                            .announce(MaplePacketCreator.getPlayerShopRemoveVisitor(i + 1));
                }
            }
            ...
            try {
                for (int i = 0; i < 3; i++) {
                    if (visitors[i] != null) {
                        visitors[i]
                                .getClient()
                                .announce(MaplePacketCreator.shopErrorMessage(10, 1));
    ...
    public List<MaplePlayerShopItem> sendAvailableBundles(int itemid) {
        List<MaplePlayerShopItem> list = new ArrayList<>();
