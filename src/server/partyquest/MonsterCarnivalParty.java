...
public class MonsterCarnivalParty {
    ...
    public MonsterCarnivalParty(
            final MapleCharacter owner, final List<MapleCharacter> members1, final byte team1) {
        leader = owner;
        members = members1;
        team = team1;
    ...
    public void warpOut() {
        if (winner == true) warpOut(980000003 + (leader.getCarnival().getRoom() * 100));
        else warpOut(980000004 + (leader.getCarnival().getRoom() * 100));
    }
