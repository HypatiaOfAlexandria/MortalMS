...
public class ItemRequirement extends MapleQuestRequirement {
    ...
    public boolean check(MapleCharacter chr, Integer npcid) {
        ...
        for (Map.Entry<Integer, Integer> integerIntegerEntry : items.entrySet()) {
            int countNeeded = integerIntegerEntry.getValue();
            int count = 0;

            MapleInventoryType iType = ItemConstants.getInventoryType(integerIntegerEntry.getKey());

            if (iType.equals(MapleInventoryType.UNDEFINED)) {
                return false;
            }
            for (Item item : chr.getInventory(iType).listById(integerIntegerEntry.getKey())) {
                count += item.getQuantity();
            }
            // Weird stuff, nexon made some quests only available when wearing gm clothes. This
            // enables us to accept it ><
            if (iType.equals(MapleInventoryType.EQUIP)
                    && !ItemConstants.isMedal(integerIntegerEntry.getKey())) {
                if (chr.isGM()) {
                    for (Item item :
                            chr.getInventory(MapleInventoryType.EQUIPPED)
                                    .listById(integerIntegerEntry.getKey())) {
                        count += item.getQuantity();
                    }
                } else {
                    if (count < countNeeded) {
                        if (chr.getInventory(MapleInventoryType.EQUIPPED)
                                                .countById(integerIntegerEntry.getKey())
                                        + count
                                >= countNeeded) {
                            chr.dropMessage(
                                    5,
                                    "Unequip the required "
                                            + ii.getName(integerIntegerEntry.getKey())
                                            + " before trying this quest operation.");
                            return false;
                        }
                    }
                }
