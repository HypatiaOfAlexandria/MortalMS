...
public class ItemAction extends MapleQuestAction {
    ...
    public void processData(MapleData data) {
        for (MapleData iEntry : data.getChildren()) {
            ...
            if (propData != null) prop = MapleDataTool.getInt(propData);
            ...
            items.add(
                    new ItemData(Integer.parseInt(iEntry.getName()), id, count, prop, job, gender));
        }
    ...
    public void run(MapleCharacter chr, Integer extSelection) {
        List<Pair<Integer, Integer>> takeItem = new ArrayList<>();
        List<Pair<Integer, Integer>> giveItem = new ArrayList<>();
        ...
        for (Pair<Integer, Integer> iPair : takeItem) {
            ...
            if (type.equals(MapleInventoryType.EQUIP)) {
                if (chr.getInventory(type).countById(iPair.getLeft()) < quantity) {
                    ...
                    if (chr.getInventory(MapleInventoryType.EQUIPPED).countById(iPair.getLeft())
                            > quantity) {
            ...
            MapleInventoryManipulator.removeById(
                    chr.getClient(), type, iPair.getLeft(), quantity, true, false);
            chr.announce(
                    MaplePacketCreator.getShowItemGain(
                            iPair.getLeft(), iPair.getRight().shortValue(), true));
        }

        for (Pair<Integer, Integer> iPair : giveItem) {
            MapleInventoryManipulator.addById(
                    chr.getClient(), iPair.getLeft(), iPair.getRight().shortValue());
            chr.announce(
                    MaplePacketCreator.getShowItemGain(
                            iPair.getLeft(), iPair.getRight().shortValue(), true));
        }
    ...
    public boolean check(MapleCharacter chr, Integer extSelection) {
        List<Pair<Item, MapleInventoryType>> gainList = new ArrayList<>();
        List<Pair<Item, MapleInventoryType>> selectList = new ArrayList<>();
        List<Pair<Item, MapleInventoryType>> randomList = new ArrayList<>();
        ...
        for (ItemData item : items) {
            ...
            if (item.getProp() != null) {
                ...
                if (item.getCount() > 0) {
                    ...
                    int freeSlotCount =
                            chr.getInventory(type).freeSlotCountById(item.getId(), quantity);
                    if (freeSlotCount == -1) {
                        if (type.equals(MapleInventoryType.EQUIP)
                        ...
                        chr.dropMessage(
                                1, "Please check if you have enough items in your inventory.");
                        return false;
                    }
        ...
        if (!randomList.isEmpty()) {
            ...
            for (Pair<Item, MapleInventoryType> it : randomList) {
                ...
                result =
                        MapleInventoryManipulator.checkSpaceProgressively(
                                c,
