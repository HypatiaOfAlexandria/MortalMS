...
public class MapleQuest {
    ...
    static {
        exploitableQuests.add(
                (short) 2338); // there are a lot more exploitable quests, they need to be
        // nit-picked
    ...
    protected Map<MapleQuestRequirementType, MapleQuestRequirement> startReqs =
            new EnumMap<>(MapleQuestRequirementType.class);
    protected Map<MapleQuestRequirementType, MapleQuestRequirement> completeReqs =
            new EnumMap<>(MapleQuestRequirementType.class);
    protected Map<MapleQuestActionType, MapleQuestAction> startActs =
            new EnumMap<>(MapleQuestActionType.class);
    protected Map<MapleQuestActionType, MapleQuestAction> completeActs =
            new EnumMap<>(MapleQuestActionType.class);
    protected List<Integer> relevantMobs = new ArrayList<>();
    ...
    private MapleQuest(int id) {
        ...
        if (startActData != null) {
            for (MapleData startAct : startActData.getChildren()) {
                MapleQuestActionType questActionType =
    ...
    private String getIntervalTimeLeft(MapleCharacter c, IntervalRequirement r) {
        ...
        long futureTime =
                c.getQuest(MapleQuest.getInstance(id)).getCompletionTime() + r.getInterval();
    ...
    public boolean isSameDayRepeatable() {
        ...
        IntervalRequirement ir =
                (IntervalRequirement) startReqs.get(MapleQuestRequirementType.INTERVAL);
    ...
    public boolean canStartWithoutRequirements(MapleCharacter c) {
        ...
        return !(mqs.getStatus() != Status.NOT_STARTED
    ...
    public boolean canComplete(MapleCharacter c, Integer npcid) {
        ...
        for (MapleQuestRequirement r : completeReqs.values()) {
            ...
            if (!r.check(c, npcid)) {
                if (r.getType()
                        == MapleQuestRequirementType
                                .MESO) { // TODO: find a way to tell the client about the new MESO
                    // requirement type.
    ...
    public boolean forceStart(MapleCharacter c, int npc) {
        MapleQuestStatus newStatus =
                new MapleQuestStatus(this, MapleQuestStatus.Status.STARTED, npc);
    ...
    public boolean forceComplete(MapleCharacter c, int npc) {
        ...
        MapleQuestStatus newStatus =
                new MapleQuestStatus(this, MapleQuestStatus.Status.COMPLETED, npc);
        newStatus.setForfeited(c.getQuest(this).getForfeited());
        ...
        c.getMap()
                .broadcastMessage(
                        c,
                        MaplePacketCreator.showForeignEffect(c.getId(), 9),
                        false); // use 9 instead of 12 for both
        return true;
    }
    ...
    private MapleQuestRequirement getRequirement(MapleQuestRequirementType type, MapleData data) {
        ...
        switch (type) {
            ...
            case INFO_NUMBER:
                break;
            default:
                // FilePrinter.printError(FilePrinter.EXCEPTION_CAUGHT, "Unhandled Requirement Type:
    ...
    public int getNpcRequirement(boolean complete) {
        Map<MapleQuestRequirementType, MapleQuestRequirement> reqs =
                !complete ? startReqs : completeReqs;
