...
public class CashShop {
    public static class CashItem {
        ...
        public Item toItem() {
            ...
            item =
                    ItemConstants.getInventoryType(itemId).equals(MapleInventoryType.EQUIP)
                            ? MapleItemInformationProvider.getInstance().getEquipById(itemId)
                            : new Item(itemId, (byte) 0, count, petid);

            if (ItemConstants.EXPIRING_ITEMS) {
                if (period == 1) {
                    if (itemId == 5211048
                            || itemId == 5360042) { // 4 Hour 2X coupons, the period is 1, but we
                        // don't want them to last a day.
                        item.setExpiration(
                                Server.getInstance().getCurrentTime() + (1000 * 60 * 60 * 4));
                        /*
                        ...
                        */
                        ...
                        item.setExpiration(
                                Server.getInstance().getCurrentTime() + (1000 * 60 * 60 * 2));
                    } else {
                        item.setExpiration(
                                Server.getInstance().getCurrentTime() + (1000 * 60 * 60 * 24));
                    }
                } else {
                    item.setExpiration(
                            Server.getInstance().getCurrentTime() + (1000 * 60 * 60 * 24 * period));
                }
            }
    ...
    public static class CashItemFactory {
        ...
        static {
            ...
            try {
                ...
                rs = ps.executeQuery();
                while (rs.next()) {
                    specialcashitems.add(
                            new SpecialCashItem(
                                    rs.getInt("sn"), rs.getInt("modifier"), rs.getByte("info")));
                }
        ...
        try {
            ps =
                    con.prepareStatement(
                            "SELECT `nxCredit`, `maplePoint`, `nxPrepaid` FROM `accounts` WHERE `id` = ?");
    ...
    public List<Pair<Item, String>> loadGifts() {
        ...
        try {
            ...
            while (rs.next()) {
                ...
                } else gifts.add(new Pair<>(item, rs.getString("message")));

                if (CashItemFactory.isPackage(
                        cItem.getItemId())) { // Packages never contains a ring
