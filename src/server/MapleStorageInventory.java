...
class PairedQuicksort {
    ...
    public PairedQuicksort(ArrayList<Item> A, int primarySort, int secondarySort) {
        ...
        if (!A.isEmpty()) MapleQuicksort(0, A.size() - 1, A, primarySort);
...
public class MapleStorageInventory {
        ...
        if (target == null) {
            ...
            inventory.remove(sSlot);
                ...
                && !MapleItemInformationProvider.getInstance()
                        .isPickupRestricted(source.getItemId())
    ...
    private void moveItem(short src, short dst) {
        ...
        short slotMax =
                MapleItemInformationProvider.getInstance().getSlotMax(c, source.getItemId());
    ...
    public void mergeItems() {
        ...
        for (short dst = 1; dst <= slotLimit; dst++) {
            dstItem = this.getItem(dst);
            if (dstItem == null) continue;

            for (short src = (short) (dst + 1); src <= slotLimit; src++) {
        ...
        while (!sorted) {
            ...
            if (freeSlot != -1) {
                short itemSlot = -1;
                for (short i = (short) (freeSlot + 1); i <= slotLimit; i += 1) {
