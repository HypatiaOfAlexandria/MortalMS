...
public class MakerItemFactory {
    ...
    public static MakerItemCreateEntry getItemCreateEntry(
            int toCreate, int stimulantid, Map<Integer, Short> reagentids) {
        ...
        if (!reagentids.isEmpty()) {
            for (Entry<Integer, Short> r : reagentids.entrySet()) {
                makerEntry.addCost(
                        (getMakerReagentFee(toCreate, ((r.getKey() % 10) + 1))) * r.getValue());
            }
        }
    ...
    public static MakerItemCreateEntry generateDisassemblyCrystalEntry(
            int cost, int crystalGain) { // equipment at specific position already taken
