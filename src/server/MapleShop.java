...
public class MapleShop {
    ...
    public void sendShop(MapleClient c) {
        ...
        c.announce(MaplePacketCreator.getNPCShop(c, npcId, items));
    }
    ...
    public void buy(MapleClient c, short slot, int itemId, short quantity) {
        ...
        if (item.getPrice() > 0) {
            ...
            if (c.getPlayer().getMeso() >= amount) {
                ...
                } else c.announce(MaplePacketCreator.shopTransaction((byte) 3));

            } else c.announce(MaplePacketCreator.shopTransaction((byte) 2));
                ...
                if (MapleInventoryManipulator.checkSpace(c, itemId, quantity, "")) {
                    if (!ItemConstants.isRechargeable(itemId)) {
                        ...
                        MapleInventoryManipulator.removeById(
                                c, MapleInventoryType.ETC, 4310000, amount, false, false);
                    } else {
                        short slotMax = ii.getSlotMax(c, item.getItemId());
                        quantity = slotMax;
                        MapleInventoryManipulator.addById(c, itemId, quantity);
    ...
    private static boolean canSell(Item item, short quantity) {
        ...
        return ItemConstants.isRechargeable(item.getItemId()) || iQuant != 0 && quantity <= iQuant;
    }
    ...
    public static void sell(MapleClient c, MapleInventoryType type, short slot, short quantity) {
    ...
    public static void recharge(MapleClient c, short slot) {
        MapleItemInformationProvider ii = MapleItemInformationProvider.getInstance();
        ...
        if (item.getQuantity() < slotMax) {
            int price =
                    (int)
                            Math.ceil(
                                    ii.getUnitPrice(item.getItemId())
    ...
    public static MapleShop createFromDB(int id, boolean isShopId) {
        ...
        try {
            ...
            ps =
                    isShopId
                            ? con.prepareStatement("SELECT * FROM shops WHERE shopid = ?")
                            : con.prepareStatement("SELECT * FROM shops WHERE npcid = ?");
            ps.setInt(1, id);
