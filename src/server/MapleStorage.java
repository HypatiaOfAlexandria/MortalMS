...
public class MapleStorage {
    ...
    private final Lock lock =
            MonitoredReentrantLockFactory.createLock(MonitoredLockType.STORAGE, true);
    ...
    public static MapleStorage loadOrCreateFromDB(int id, int world) {
        ...
        try {
            ...
            PreparedStatement ps =
                    con.prepareStatement(
                            "SELECT storageid, slots, meso FROM storages WHERE accountid = ? AND world = ?");
            ps.setInt(1, id);
            ps.setInt(2, world);
    ...
    public void sendStorage(MapleClient c, int npcId) {
        if (c.getPlayer().getLevel() < 15) {
            c.getPlayer()
                    .dropMessage(1, "You may only use the storage once you have reached level 15.");
        ...
        try {
            items.sort(
                    (o1, o2) -> {
                        ...
                        if (o1.getInventoryType() == o2.getInventoryType()) {
                            return 0;
                        }
                        return 1;
                    });
