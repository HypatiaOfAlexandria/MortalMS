...
public class MapleLootManager {
    ...
    private static boolean isRelevantDrop(
            MonsterDropEntry dropEntry,
            List<MapleCharacter> partyMembers,
            List<MapleLootInventory> partyInv) {
        ...
        boolean restricted =
                MapleItemInformationProvider.getInstance().isLootRestricted(dropEntry.itemId);
    ...
    public static List<MonsterDropEntry> retrieveRelevantDrops(
            int monsterId, List<MapleCharacter> partyMembers) {
        List<MonsterDropEntry> loots =
                MapleMonsterInformationProvider.getInstance().retrieveEffectiveDrop(monsterId);
        if (loots.isEmpty()) return loots;

        List<MapleLootInventory> partyInv = new ArrayList<>();
        for (MapleCharacter chr : partyMembers) {
            MapleLootInventory lootInv = new MapleLootInventory(chr);
            partyInv.add(lootInv);
        }

        List<MonsterDropEntry> effectiveLoot = new ArrayList<>();
